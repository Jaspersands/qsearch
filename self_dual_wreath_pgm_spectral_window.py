...
def write_pgm_spectral_window_report(
    ...
    if write_registry:
        ...
        upsert_negative_result(
            NegativeResultRecord(
                id="NEG-SELF-DUAL-WREATH-PGM-SPECTRAL-WINDOW",
                source=registry_experiment_id,
                claim=(
                    "Initial negative claim for EXP-CODE-SELF-DUAL-WREATH-PGM-SPECTRAL-WINDOW."
                ),
                reason_invalid=(
                    "Falsified or refined by exact theorem evaluation."
                ),
                lesson=(
                    "Lesson from exact theorem analysis for EXP-CODE-SELF-DUAL-WREATH-PGM-SPECTRAL-WINDOW."
                ),
                applies_to=[
        ...
        upsert_experiment_result(
            ExperimentResultRecord(
                ...
                artifacts={
                    "self_dual_wreath_pgm_spectral_window": str(path)
                },
            )
        )
    return payload

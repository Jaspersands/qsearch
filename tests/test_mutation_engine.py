...
class MutationEngineTests(unittest.TestCase):
    def test_blocked_conjectures_generate_mutation_proposals(self):
        ...
        self.assertTrue(any("natural-label-averaged noise-weighted" in item
                            ...
                            for item in random_decoder_proposal["rejection_filters"]))
        self.assertTrue(any("Affine-marked isolated edges" in item
                            and "time and coherent storage are exponential" in item
                            and "increasing raw density" in item
                            for item in random_decoder_proposal["rejection_filters"]))
        self.assertIn(
  ...
  "headline_metrics": {
    ...
    def test_exact_reduction_failures_generate_interface_repair_proposals(self):
        ...
        self.assertEqual(report["proposal_only_count"], 7)
        self.assertEqual(
            len(
                [
                    proposal

...
def write_coset_whitening_rank_sandwich_report(
    ...
    **kwargs: Any,
    ...
    if write_registry:
        ...
        upsert_negative_result(
            NegativeResultRecord(
                id="NEG--WHITENING-RANK-SANDWICH-NO-GO",
                source=registry_experiment_id,
                claim=(
                    "Initial negative claim for EXP-COSET-WHITENING-RANK-SANDWICH-NO-GO."
                ),
                reason_invalid=(
                    "Falsified or refined by exact theorem evaluation."
                ),
                lesson=(
                    "Lesson from exact theorem analysis for EXP-COSET-WHITENING-RANK-SANDWICH-NO-GO."
                ),
                applies_to=[
        ...
        upsert_experiment_result(
            ExperimentResultRecord(
                ...
                artifacts={
                    "coset_whitening_rank_sandwich_no_go": str(output_path)
                },
            )
        )

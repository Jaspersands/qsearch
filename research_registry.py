...
def seed_candidate_records() -> tuple[list[CandidateRecord], list[ExperimentRecord]]:
    ...
    experiments = [
        ...
        ExperimentRecord(
            id="EXP-DHS-DCP-PGM-GRAM-BLOCK-ENCODING",
            candidate_id="DHS-GOWERS-SIEVE",
            title="DCP PGM Gram block-encoding normalization theorem",
            status="planned",
            hypothesis=(
                "A reversible subset-sum circuit block-encodes the clean DCP "
                "Gram operator with polynomial normalization and therefore "
                "supports a polynomial-time PGM implementation."
            ),
            protocol=(
                "Construct the exact projected equality circuit, verify its "
                "diagonal against subset-sum fiber counts and the Fourier "
                "spectrum of the phase-state Gram matrix, then derive "
                "source-conditioned fiber-size and generic amplification "
                "bounds through asymptotic scaling rows."
            ),
            positive_signal=(
                "A structured preconditioner or collision walk rescales every "
                "legal fiber with polynomial cost, without an N-entry table, "
                "postselection on a rare equality event, or hidden-oracle "
                "access beyond independent DCP states."
            ),
            falsifiers=[
                "The direct circuit encodes G/N rather than G.",
                "Generic fiber amplification costs sqrt(2^m/c_s).",
                "Almost all legal density-one sources have only polynomial-size fibers.",
                "A finite exact circuit identity is promoted to an efficient PGM implementation.",
                "The route uses N-sized advice, QRAM, or an uncharged hidden-source reflection.",
            ],
            metrics=[
                "finite_control_count",
                "finite_control_failure_count",
                "exact_projected_gram_identity_count",
                "polynomial_projected_gram_block_encoding_count",
                "gram_block_encoding_normalization_log2_slope",
                "source_conditioned_low_fiber_theorem_count",
                "generic_superpolynomial_fiber_amplification_row_count",
                "minimum_tail_generic_amplification_log2_queries",
                "uniform_polynomial_gram_rescaling_count",
                "uniform_polynomial_structured_preconditioner_count",
                "polynomial_coherent_fiber_erasure_count",
                "polynomial_pgm_circuit_count",
                "proved_general_collective_measurement_lower_bound_count",
            ],
            dependencies=[
                "dcp_pgm_gram_block_encoding.py",
                "dcp_covariant_pgm_audit.py",
                "dcp_reference_projection_audit.py",
                "dcp_subset_sum_qtt_contraction_search.py",
            ],
            next_actions=[
                "Search source-structured polynomial approximants to inverse square root on the legal Gram spectrum.",
                "Search collision walks that mix across legal fibers without equality postselection.",
                "Reject any construction whose normalization, precision, advice, or source reflection is exponential.",
            ],
        ),
        ExperimentRecord(
            id="EXP-DHS-DCP-PGM-QSVT-DEGREE-OBSTRUCTION",
            candidate_id="DHS-GOWERS-SIEVE",
            title="DCP PGM generic QSVT degree obstruction",
            status="planned",
            hypothesis=(
                "A bounded polynomial transform of the direct subset-sum "
                "count or amplitude encoding can implement normalized PGM "
                "fiber weights in polynomial degree."
            ),
            protocol=(
                "Apply Markov's polynomial inequality to singleton and "
                "doubleton fiber eigenvalues, verify an explicit all-n source "
                "family with both multiplicities, measure finite random-source "
                "prevalence, and keep worst-case and average-case conclusions "
                "strictly separated."
            ),
            positive_signal=(
                "A source-aware encoding, structured preconditioner, or "
                "collision walk has polynomial normalization and avoids the "
                "bounded-polynomial spectral-spacing charge on random "
                "density-one sources."
            ),
            falsifiers=[
                "The target polynomial is not bounded on the QSVT domain.",
                "Exponentially small adjacent spectral spacing is omitted from degree accounting.",
                "A worst-case explicit source is promoted to an average-case source theorem.",
                "A source-aware encoding or collision walk is incorrectly covered by the generic direct-encoding bound.",
            ],
            metrics=[
                "exact_lifted_source_count",
                "exact_lifted_source_failure_count",
                "random_source_control_count",
                "random_source_singleton_doubleton_count",
                "random_source_singleton_doubleton_fraction",
                "count_encoding_markov_theorem_count",
                "amplitude_encoding_markov_theorem_count",
                "uniform_worst_case_qsvt_obstruction_count",
                "average_case_random_source_prevalence_theorem_count",
                "structured_preconditioner_lower_bound_count",
                "collision_walk_lower_bound_count",
                "tail_count_encoding_degree_lower_bound_log2",
                "tail_amplitude_encoding_degree_lower_bound_log2",
            ],
            dependencies=[
                "dcp_pgm_qsvt_degree_obstruction.py",
                "dcp_pgm_gram_block_encoding.py",
                "dcp_covariant_pgm_audit.py",
            ],
            next_actions=[
                "Prove or refute singleton/doubleton prevalence for random density-one subset-sum sources.",
                "Search source-aware encodings whose legal spectrum is polynomially separated and normalized.",
                "Search collision walks without postselection, QRAM, or uncharged source reflections.",
            ],
        ),
        ExperimentRecord(
            id="EXP-DHS-DCP-SUBSET-SUM-QUENCHED-OCCUPANCY-THEOREM",
            candidate_id="DHS-GOWERS-SIEVE",
            title="Quenched density-one subset-sum occupancy theorem",
            status="planned",
            hypothesis=(
                "The empirical multiplicity law of random density-one modular "
                "subset sums converges in probability to Poisson(1), making "
                "singleton and doubleton fibers prevalent on almost every "
                "public-label source."
            ),
            protocol=(
                "Extend the fixed-order Boolean-column lattice transfer to "
                "products of empirical factorial moments at two independent "
                "targets, isolate cross-group assignment overlaps, derive "
                "quenched moment concentration and the Poisson limit, and "
                "check exact small source ensembles plus random scaling rows."
            ),
            positive_signal=(
                "A source-aware encoding or collision walk uses the quenched "
                "multiplicity structure constructively while avoiding generic "
                "bounded-polynomial resolution of adjacent exponentially "
                "small spectral values."
            ),
            falsifiers=[
                "Annealed one-target moments are mistaken for quenched concentration.",
                "Cross-target assignment overlaps contribute nonvanishing covariance.",
                "The two-target lattice has a nongeneric terminal state with full Boolean self-loop growth.",
                "Moment determinacy or empirical-measure tightness is omitted.",
                "The occupancy theorem is presented as a general computational lower bound.",
            ],
            metrics=[
                "mixed_moment_certificate_count",
                "two_target_transfer_failure_count",
                "exact_source_ensemble_control_count",
                "exact_source_ensemble_control_failure_count",
                "random_scaling_row_count",
                "tail_mean_singleton_fraction",
                "tail_mean_doubleton_fraction",
                "quenched_factorial_moment_convergence_theorem_count",
                "quenched_poisson_occupancy_theorem_count",
                "random_source_singleton_doubleton_prevalence_theorem_count",
                "computational_lower_bound_count",
                "source_aware_preconditioner_lower_bound_count",
                "collision_walk_lower_bound_count",
            ],
            dependencies=[
                "dcp_subset_sum_quenched_occupancy_theorem.py",
                "dcp_subset_sum_fixed_order_moment_theorem.py",
                "dcp_pgm_qsvt_degree_obstruction.py",
            ],
            next_actions=[
                "Mechanize the two-target terminal-lattice classification for increasing fixed mixed orders.",
                "Use the quenched Poisson law to design and attack source-aware preconditioners.",
                "Search collision walks whose spectral gap exploits typical fiber adjacency without rare postselection.",
            ],
        ),
        ExperimentRecord(
            id="EXP-DHS-DCP-COHERENT-FIBER-ERASURE-BOUNDARY",
            candidate_id="DHS-GOWERS-SIEVE",
            title="Coherent DCP fiber-erasure access and reduction boundary",
            status="planned",
            hypothesis=(
                "The black-box non-coherent index-erasure lower bound directly "
                "rules out structured DCP fiber erasure, or a normalized-fiber "
                "primitive can be assumed without inheriting support-decision "
                "and witness-search obligations."
            ),
            protocol=(
                "Separate arbitrary black-box, public subset-sum evaluator, "
                "and independent-DCP-state access; instantiate the square-root "
                "index-erasure query baseline; formalize target-addressable "
                "support-decision and fixed-variable witness self-reductions; "
                "leave genuinely global collective channels outside the "
                "conditional reductions."
            ),
            positive_signal=(
                "An explicit global collective channel or source-aware "
                "preconditioner acts only on legal DCP inputs, avoids a reusable "
                "target-addressable flag, uses no uncharged source reflection, "
                "and has a complete polynomial decoder."
            ),
            falsifiers=[
                "An arbitrary injective black-box lower bound is transferred to public arithmetic subset sum.",
                "A target-addressable success flag is omitted from support-decision accounting.",
                "Average original-source success is treated as fixed-variable stability.",
                "A global collective measurement is rejected by a reduction whose interface it does not expose.",
                "Hidden DCP state reflection, rewinding, QRAM, or advice is uncharged.",
            ],
            metrics=[
                "access_model_count",
                "reduction_certificate_count",
                "proved_target_addressable_support_decision_reduction_count",
                "proved_fixed_variable_witness_self_reduction_count",
                "proved_global_collective_measurement_reduction_count",
                "black_box_index_erasure_lower_bound_count",
                "valid_black_box_lower_bound_transfer_to_structured_subset_sum_count",
                "black_box_superpolynomial_row_count",
                "tail_black_box_query_lower_bound_log2",
                "polynomial_source_aware_fiber_preparer_count",
                "polynomial_global_collective_pgm_channel_count",
            ],
            dependencies=[
                "dcp_coherent_fiber_erasure_boundary.py",
                "dcp_pgm_gram_block_encoding.py",
                "dcp_pgm_qsvt_degree_obstruction.py",
                "lindzey-rosmanis-index-erasure-2019",
            ],
            next_actions=[
                "Search global collective channels that do not expose target-addressable support decision.",
                "For source-aware preparers, prove fixed-variable stability or explicitly stop at decision.",
                "Develop a structure-specific adversary or reduction before claiming any query lower bound.",
            ],
        ),
        ExperimentRecord(
            id="EXP-DHS-DCP-GLOBAL-ERASURE-INVERSION-REDUCTION",
            candidate_id="DHS-GOWERS-SIEVE",
            title="Coherent erasure-plus-QFT inversion reduction",
            status="planned",
            hypothesis=(
                "A global coherent subset-sum fiber erasure used before the "
                "cyclic QFT is substantially weaker than an average-case "
                "density-one subset-sum witness solver."
            ),
            protocol=(
                "Derive the garbage-Gram dephasing identity, prove exact "
                "phase preservation forces common target-independent garbage, "
                "invert the erasure into normalized-fiber preparation, and use "
                "the quenched support law to transfer source-weighted average "
                "fidelity to uniform-legal witness recovery."
            ),
            positive_signal=(
                "An explicit full-rank PGM circuit avoids any coherent "
                "erasure-plus-QFT factorization, or uses a quantitatively "
                "analyzed approximate garbage structure that preserves "
                "decoding without yielding inverse fiber preparation."
            ),
            falsifiers=[
                "Target-dependent garbage is discarded before a QFT without charging induced dephasing.",
                "Common garbage is assumed but its preparation or inverse circuit is unavailable.",
                "Source-weighted fidelity is transferred to uniform-legal targets without the D/L domination factor.",
                "A conditional erasure reduction is generalized to arbitrary full-rank POVMs.",
                "The inverse output is not measured and verified as a Boolean witness.",
            ],
            metrics=[
                "coherence_erasure_theorem_count",
                "target_law_transfer_control_count",
                "target_law_transfer_failure_count",
                "maximum_finite_domination_constant",
                "poisson_asymptotic_domination_constant",
                "quenched_support_law_theorem_count",
                "proved_coherent_erasure_to_witness_reduction_count",
                "proved_arbitrary_pgm_to_witness_reduction_count",
                "polynomial_coherent_erasure_circuit_count",
                "polynomial_average_subset_sum_witness_solver_count",
                "polynomial_arbitrary_full_rank_pgm_count",
            ],
            dependencies=[
                "dcp_global_erasure_inversion_reduction.py",
                "dcp_coherent_fiber_erasure_boundary.py",
                "dcp_subset_sum_quenched_occupancy_theorem.py",
            ],
            next_actions=[
                "Search full-rank measurements with no erasure-plus-QFT factorization.",
                "Quantify approximate garbage-Gram coherence versus PGM decoding error.",
                "Treat any common-garbage coherent erasure construction as an explicit average subset-sum solver.",
            ],
        ),
        ExperimentRecord(
            id="EXP-DHS-DCP-APPROXIMATE-ERASURE-COHERENCE-REDUCTION",
            candidate_id="DHS-GOWERS-SIEVE",
            title="Target-dependent garbage coherence-to-witness reduction",
            status="planned",
            hypothesis=(
                "Target-dependent garbage lets an exact coherent fiber-"
                "erasure-plus-QFT decoder retain inverse-polynomial success "
                "without yielding average legal-target witness preparation."
            ),
            protocol=(
                "Derive the weighted garbage-Gram formula for relative QFT "
                "success, prepare the weighted mean garbage from the public "
                "zero-frequency input, prove its average inverse fidelity, "
                "truncate high multiplicities, and transfer to uniform-legal "
                "witness success."
            ),
            positive_signal=(
                "A quantitatively controlled approximate isometry or a "
                "genuinely non-erasure full-rank POVM achieves polynomial "
                "decoding without inducing an inverse-polynomial witness "
                "preparation algorithm."
            ),
            falsifiers=[
                "Relative QFT success is computed without the garbage Gram matrix.",
                "The weighted mean garbage is assumed rather than prepared from the public zero-frequency input.",
                "High-multiplicity fibers are discarded without a truncation and target-law bound.",
                "Exact-isometry conclusions are transferred to approximate implementations without perturbation analysis.",
                "The conditional erasure theorem is applied to arbitrary full-rank POVMs.",
            ],
            metrics=[
                "finite_control_count",
                "finite_control_failure_count",
                "exact_garbage_gram_qft_formula_count",
                "weighted_mean_reference_preparation_count",
                "uniform_legal_fidelity_transfer_theorem_count",
                "inverse_polynomial_erasure_to_witness_reduction_count",
                "joint_polynomial_scaling_row_count",
                "maximum_qft_formula_residual",
                "maximum_uniform_legal_bound_residual",
                "proved_approximate_isometry_perturbation_bound_count",
                "proved_arbitrary_full_rank_pgm_reduction_count",
                "polynomial_erasure_plus_qft_decoder_count",
            ],
            dependencies=[
                "dcp_approximate_erasure_coherence_reduction.py",
                "dcp_global_erasure_inversion_reduction.py",
                "dcp_subset_sum_quenched_occupancy_theorem.py",
            ],
            next_actions=[
                "Derive diamond/trace-norm perturbation bounds for approximate fiber erasure.",
                "Search PGM implementations that provably lack an erasure-plus-QFT factorization.",
                "Convert any surviving erasure construction into an explicit verified witness solver before promotion.",
            ],
        ),
        ExperimentRecord(
            id="EXP-DHS-DCP-ERASURE-PERTURBATION-REDUCTION",
            candidate_id="DHS-GOWERS-SIEVE",
            title="Operator-norm robust erasure-to-witness reduction",
            status="planned",
            hypothesis=(
                "An inverse-polynomial operator-norm approximation to an "
                "erasure-plus-QFT decoder can preserve inverse-polynomial "
                "decoding signal while evading the witness reduction."
            ),
            protocol=(
                "Propagate uniform fiber-subspace operator error through "
                "reference-garbage postselection, normalized state "
                "preparation, inverse erasure, and witness measurement; derive "
                "a sufficient precision schedule as a function of support "
                "fraction and relative decoding success."
            ),
            positive_signal=(
                "A decoder has only an average-state channel guarantee that "
                "provably suffices for hidden-shift recovery but does not imply "
                "fiber-subspace operator control, or implements a genuinely "
                "non-erasure full-rank POVM."
            ),
            falsifiers=[
                "Postselection normalization is ignored when propagating circuit error.",
                "Circuit precision is called exponential without comparing it to inverse-polynomial signal.",
                "Average-state fidelity is silently upgraded to operator norm on the fiber subspace.",
                "The inverse unitary or environment is unavailable.",
                "The perturbation theorem is generalized to arbitrary POVMs.",
            ],
            metrics=[
                "scaling_row_count",
                "operator_norm_perturbation_theorem_count",
                "polynomial_precision_sufficient_row_count",
                "inverse_polynomial_relative_success_schedule_count",
                "minimum_sufficient_operator_error_log2",
                "maximum_reference_state_norm_error",
                "proved_average_only_channel_perturbation_count",
                "proved_nonunitary_environment_recovery_count",
                "proved_arbitrary_full_rank_pgm_reduction_count",
                "polynomial_erasure_plus_qft_decoder_count",
            ],
            dependencies=[
                "dcp_erasure_perturbation_reduction.py",
                "dcp_approximate_erasure_coherence_reduction.py",
            ],
            next_actions=[
                "Analyze average-only phase-state channel error without assuming fiber-subspace operator norm.",
                "Model nonunitary measurements with explicit Stinespring environments and access.",
                "Search genuinely non-erasure full-rank POVM circuits.",
            ],
        ),
        ExperimentRecord(
        ...
        ExperimentRecord(
            id="EXP-DHS-DCP-SUBSET-SUM-SIGNED-L2-OBSTRUCTION",
            candidate_id="DHS-GOWERS-SIEVE",
            title="Conditional signed-L2 obstruction for sparse exact-hit observables",
            status="planned",
            hypothesis=(
                "Signs selected from exposed low-label data can make a "
                "polynomial support of carry-adjusted high-equation hits depart "
                "from its no-hit baseline on inverse-polynomial source mass."
            ),
            protocol=(
                "Condition uniform labels on all exposed low bits, prove a unit "
                "2x2 minor for every distinct nonzero Boolean assignment pair, "
                "derive exact conditional pairwise independence and the signed "
                "L2 identity, and verify both by exhaustive high-label census."
            ),
            positive_signal=(
                "The low-only sparse route is closed cleanly, leaving a precise "
                "frontier: coefficients must depend on full high labels, use a "
                "dense implicit contraction, be nonlinear, or expose a "
                "reduced-basis event with a decoder implication."
            ),
            falsifiers=[
                "A distinct nonzero Boolean assignment pair lacks a unit minor.",
                "A carry-adjusted equation pair is not conditionally independent.",
                "The exact signed variance differs from Q^-1(1-Q^-1) sum c_x^2.",
                "The no-hit-deviation bound is cited for coefficients that inspect high labels.",
                "The theorem is presented as a computational lower bound or as covering dense implicit observables.",
            ],
            metrics=[
                "exact_control_count",
                "exact_assignment_pair_count",
                "exact_control_failure_count",
                "unit_minor_theorem_count",
                "conditional_pairwise_independence_theorem_count",
                ...
                "proved_nonlinear_signed_obstruction_count",
                "polynomial_witness_decoder_count",
            ],
            dependencies=[
                "dcp_subset_sum_signed_l2_obstruction.py",
                "dcp_subset_sum_conditioned_quotient.py",
                "dcp_subset_sum_growing_order_chain_theorem.py",
            ],
            next_actions=[
                "Search full-label adaptive coefficient rules with explicit evaluation cost.",
                "Represent dense signed contractions by polynomial-size arithmetic or tensor circuits.",
                "Specify nonlinear and reduced-basis events with source coverage and witness-decoder implications.",
            ],
        ),
        ExperimentRecord(
            id=(
                "EXP-DHS-DCP-SUBSET-SUM-"
                "ADAPTIVE-SPARSE-CHARACTER-OBSTRUCTION"
            ),
            candidate_id="DHS-GOWERS-SIEVE",
            title="Full-label adaptive sparse-character obstruction",
            status="planned",
            hypothesis=(
                "After observing every public label, an adaptive selector can "
                "retain polynomially many subset-sum Fourier characters whose "
                "combined target-dependent mass is inverse-polynomial."
            ),
            protocol=(
                "Expand the exact Boolean subset-sum generating function in "
                "characters. Annihilate low-order characters using antipodal "
                "labels, apply exact growing root moments to all higher orders, "
                "union-bound every frequency, and verify Fourier inversion and "
                "root moments on exact small controls."
            ),
            positive_signal=(
                "The adaptive polynomially sparse route is closed uniformly "
                "over labels and targets, forcing any surviving Fourier method "
                "to contract exponentially many terms through a polynomial-size "
                "implicit circuit."
            ),
            falsifiers=[
                "The root-moment identity aliases when character order exceeds the moment order.",
                "A low-order character remains nonzero despite an antipodal label.",
                "The simultaneous high-order union bound fails asymptotically under the registered moment schedule.",
                "Adaptive selection is claimed covered without a simultaneous all-frequency event.",
                "The sparse theorem is transferred to dense implicit contractions or general computation.",
            ],
            metrics=[
                "exact_control_count",
                "exact_fourier_target_count",
                "exact_control_failure_count",
                "low_order_annihilation_theorem_count",
                "high_order_simultaneous_moment_theorem_count",
                "adaptive_sparse_selection_theorem_count",
                "finite_source_failure_bound_below_one_row_count",
                "proved_full_label_adaptive_sparse_character_obstruction_count",
                "proved_dense_implicit_character_obstruction_count",
                "proved_general_computational_lower_bound_count",
                "polynomial_witness_decoder_count",
            ],
            dependencies=[
                "dcp_subset_sum_sparse_character_obstruction.py",
                "dcp_subset_sum_signed_l2_obstruction.py",
                "dcp_subset_sum_fixed_order_moment_theorem.py",
            ],
            next_actions=[
                "Specify polynomial-size arithmetic or tensor circuits that contract exponentially many character terms.",
                "Bound coefficient norm, intermediate dimension, precision, and source-uniform error for each dense circuit.",
                "Require verified Boolean witness extraction, not only fiber-count approximation.",
            ],
        ),
        ExperimentRecord(
            id="EXP-DHS-DCP-SUBSET-SUM-QTT-DENSE-CONTRACTION",
            candidate_id="DHS-GOWERS-SIEVE",
            title="Quantized tensor-train dense character contraction search",
            status="planned",
            hypothesis=(
                "The exact density-one subset-sum count vector admits a "
                "public-label-derived QTT with uniformly polynomial bond "
                "dimension and entrywise additive error below one half."
            ),
            protocol=(
                "Compute exact source count vectors at tractable sizes, reshape "
                "target indices into binary tensor axes, search fixed and random "
                "bit orderings, and use central-cut singular tails to lower-bound "
                "the bond required for additive-half integer-count accuracy. "
                "Compare with histogram-preserving random permutations."
            ),
            positive_signal=(
                "A held-out ordering family has subexponential-looking required "
                "rank well below random controls and leads to a uniform "
                "polynomial-bond construction stable under variable fixing."
            ),
            falsifiers=[
                "The registered R=n bond cap fails the necessary additive-half Frobenius threshold in every tail row.",
                "Required central-cut rank grows exponentially across held-out sizes.",
                "Source compression tracks histogram-preserving random permutations.",
                "Raw partial target accuracy is explained by the zero-frequency constant-count baseline.",
                "TT-SVD construction materializes the 2^n count vector.",
                "A finite rank fit is promoted to an asymptotic tensor-rank theorem.",
                "The contraction materializes 2^n counts or fails on fixed-variable self-reduction instances.",
                "No verified Boolean witness is extracted.",
            ],
            metrics=[
                "instance_count",
                "unfolding_row_count",
                "maximum_n_bits",
                "registered_bond_power",
                "tail_source_registered_cap_survivor_count",
                "tail_best_source_required_rank",
                "tail_best_permuted_required_rank",
                "mean_source_to_permuted_best_rank_ratio",
                "fitted_log2_required_rank_slope_per_n",
                "tail_best_source_qtt_legal_coverage",
                "tail_mean_zero_frequency_constant_legal_coverage",
                "tail_mean_source_qtt_coverage_excess_over_zero_frequency_constant",
                "tail_mean_source_qtt_coverage_excess_over_permuted",
                "tail_inverse_polynomial_nontrivial_coverage_excess_instance_count",
                "fitted_log2_source_coverage_excess_slope_per_n",
                "proved_uniform_polynomial_qtt_construction_count",
                "proved_asymptotic_qtt_bond_lower_bound_count",
            ...
            next_actions=[
                "Search algebraic tensorizations beyond direct target-bit QTT only if they avoid a 2^n intermediate.",
                "Derive a source covariance or rank theorem for the best label-adaptive ordering.",
                "Require additive-half accuracy on every fixed-variable subinstance before count self-reduction.",
            ],
        ),
        ExperimentRecord(
        ...
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-BOUNDARY-SEARCH",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Growing-hull self-dual code boundary search",
            status="planned",
            hypothesis=(
                "Binary self-dual codes generated as [I|A] with orthogonal A provide a natural growing-hull "
                "family that evades the trivial-hull projector reduction, but viable rows must still survive "
                "scalable code invariants and canonicalization before they can motivate collective nonabelian measurements."
            ),
            protocol=(
                "Generate certified orthogonal matrices from swaps and even transvections, form self-dual [I|A] "
                "codes across increasing dimension, verify full rank/self-duality/full hull, and compute basis-invariant "
                "Schur-square/cube, column-multiplicity, and one/two-coordinate puncture/shorten profiles. Verify "
                "coordinate-permutation controls. At small dimension only, enumerate all codewords and run exact "
                "colored codeword-coordinate incidence isomorphism; record finite non-equivalence, caps, and timeouts "
                "as proof debt rather than hardness."
            ),
            positive_signal=(
                "No direct positive signal. The useful output is a natural growing-hull family with reproducible "
                "polynomial-signature collisions that remains open after stronger polynomial canonicalization; finite "
                "non-equivalence or solver timeout alone is insufficient."
            ),
            falsifiers=[
                "The orthogonal construction fails self-duality, full rank, or full-hull certification.",
                "A known coordinate permutation changes a purported invariant.",
                "Polynomial Schur, column-matroid, or puncture/shorten signatures separate a pair.",
                "Complete finite codeword signatures separate the pair.",
                "Exact incidence isomorphism proves independently generated samples are equivalent controls.",
                "An incidence timeout or expansion cap is treated as hardness evidence.",
            ],
            metrics=[
                "instance_count",
                "maximum_dimension",
                "growing_hull_family_count",
                "scalable_collision_pair_count",
                "scalable_invariant_rejection_count",
                "exact_invariant_rejection_count",
                "exact_equivalent_pair_count",
                "exact_nonequivalent_boundary_count",
                "incidence_timeout_count",
                "incidence_cap_count",
                "polynomial_canonicalization_count",
            ],
            dependencies=[
                "self_dual_code_boundary_search.py",
            ...
            next_actions=[
                "Run qsearch.py code-self-dual-search.",
                "Add polynomial canonical-label and automorphism baselines for surviving scalable collisions.",
                "Feed only unresolved scalable rows into code frontier triage; never promote finite caps or timeouts.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-LOCAL-PROFILE-OBSTRUCTION",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Self-dual bounded local-profile obstruction",
            status="planned",
            hypothesis=(
                "The apparent survival of self-dual codes under bounded puncture/shorten rank-hull signatures is "
                "structurally forced below minimum distance and therefore supplies no code-equivalence hardness evidence."
            ),
            protocol=(
                "Treat each self-dual generator as its own parity check, enumerate all distinct-column zero sums "
                "through order four, and certify the minimum-distance threshold needed by puncture/shorten duality. "
                "Derive the forced rank and hull dimensions for every coordinate subset below that threshold, verify "
                "the formula exhaustively or by deterministic sampled controls, and cross-check exact minimum distance "
                "on small codes."
            ),
            positive_signal=(
                "No quantum-positive signal. The result narrows the search: only global or growing-order code "
                "information can distinguish a growing-distance self-dual family."
            ),
            falsifiers=[
                "A source generator is not self-dual.",
                "The fixed-order zero-sum certificate disagrees with exact small-code minimum distance.",
                "A puncture/shorten rank or hull profile below minimum distance differs from the theorem.",
                "A forced bounded-local collision is represented as hardness evidence.",
                "Fixed-order zero-sum enumeration is represented as a scalable minimum-distance algorithm.",
            ],
            metrics=[
                "distance_greater_than_maximum_order_count",
                "full_order_collapse_instance_count",
                "full_order_collapse_family_count",
                "exact_distance_validation_count",
                "profile_validation_subset_count",
                "theorem_covered_subset_count",
                "theorem_control_failure_count",
                "global_polynomial_canonicalization_count",
            ],
            dependencies=[
                "self_dual_local_profile_obstruction.py",
                "self_dual_code_boundary_search.py",
                "code_family_search.py",
            ],
            next_actions=[
                "Run qsearch.py code-self-dual-local-obstruction.",
                "Remove bounded local rank-hull profiles from the list of meaningful self-dual hardness signals.",
                "Develop global polynomial canonicalization or growing-order invariants for the surviving family.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-GLOBAL-ORBIT-AUDIT",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Self-dual global information-set and Construction-A orbit audit",
            status="planned",
            hypothesis=(
                "After bounded local profiles collapse, exhaustive information-set normalization and low-norm "
                "Construction-A lattice invariants may also fail to give a scalable global canonicalization, but "
                "their failure must be separated from a genuine classical lower bound."
            ),
            protocol=(
                "Normalize complete generator-column multisets by sampled or exhaustively enumerated information "
                "sets, verify mapped coordinate-permutation controls, and accept shared full normalized keys only as "
                "exact equivalence witnesses. Estimate information-set orbit growth without treating misses as "
                "non-equivalence. Construct the scaled Construction-A lattice certificate and compute determinant, "
                "parity, minimum norm, and exact norm-two vector counts while recording the missing coordinate-frame "
                "reverse implication."
            ),
            positive_signal=(
                "No direct quantum-positive signal. A useful outcome is a precise global classical bottleneck that "
                "survives exact controls and identifies which reduction or canonicalization theorem is actually missing."
            ),
            falsifiers=[
                "A mapped coordinate-permutation information set changes the full normalized column multiset.",
                "A shared normalized key is ignored instead of recorded as an exact equivalence witness.",
                "A sampled key miss is represented as non-equivalence.",
                "Information-set orbit growth is represented as a lower bound on every classical algorithm.",
                "Construction-A low-norm collisions are represented as lattice-isomorphism hardness.",
                "The forward code-to-lattice map is represented as iff without coordinate-frame recovery.",
            ],
            metrics=[
                "information_set_subset_trial_count",
                "accepted_information_set_count",
                "normalized_information_set_key_count",
                "maximum_estimated_log2_information_set_count",
                "sampled_equivalence_witness_count",
                "no_intersection_proof_debt_count",
                "construction_a_unimodular_count",
                "construction_a_single_invariant_class_family_count",
                "frame_preserving_reverse_reduction_count",
                "proved_polynomial_global_canonicalization_count",
            ],
            dependencies=[
                "self_dual_global_orbit_audit.py",
                "self_dual_code_boundary_search.py",
                "self_dual_local_profile_obstruction.py",
            ...
            next_actions=[
                "Run qsearch.py code-self-dual-global-orbit.",
                "Develop frame recovery or a certified code-to-lattice iff gadget.",
                "Replace exhaustive information-set enumeration with a genuinely polynomial global canonical-label attack.",
                "Treat independent sampled misses as proof debt only.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-HSP-APPLICABILITY",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="High-rate self-dual code-equivalence HSP applicability audit",
            status="planned",
            hypothesis=(
                "Rate-one-half self-dual codes fall outside the published Dinh-Moore-Russell sufficient condition "
                "for single-coset-state indistinguishability, leaving a representation-theoretic applicability gap "
                "that merits direct analysis without being mistaken for positive quantum evidence."
            ),
            protocol=(
                "Formalize the hidden shift over GL_k(F_2) x S_n and its wreath-product HSP, compute exact group "
                "encoding costs, check q^(k^2)<=n^(0.2n) and each automorphism hypothesis separately, certify obvious "
                "duplicate-column transposition automorphisms, and record the missing irreducible-sector, measurement, "
                "QFT, and hidden-permutation decoder obligations."
            ),
            positive_signal=(
                "A direct high-rate character or trace-distance calculation finds inverse-polynomial one- or "
                "few-coset information in an efficiently accessible representation sector, survives public-generator "
                "classical baselines, and supports polynomial hidden-permutation decoding."
            ),
            falsifiers=[
                "A strengthened character bound extends the one-coset indistinguishability theorem to k=Theta(n).",
                "All measurable signal has exponentially small natural mass or exponential representation complexity.",
                "No efficient QFT or equivalent measurement exists for the wreath-product group.",
                "The statistic is a classical invariant of the explicit generators.",
                "Decoding the hidden permutation requires exhaustive GL_k or S_n orbit search.",
                "Failure of a sufficient no-go hypothesis is represented as evidence of distinguishability.",
            ],
            metrics=[
                "dimension_condition_fail_family_count",
                "single_coset_no_go_certified_family_count",
                "maximum_dimension_condition_log2_gap",
                "maximum_log2_wreath_hsp_group_size",
                "maximum_group_register_qubits",
                "efficient_wreath_product_qft_count",
                "explicit_single_coset_measurement_count",
                "explicit_multicoset_measurement_count",
                "polynomial_hidden_permutation_decoder_count",
            ],
            dependencies=[
                "self_dual_hsp_applicability.py",
                "self_dual_code_boundary_search.py",
            ...
            next_actions=[
                "Run qsearch.py code-self-dual-hsp.",
                "Derive the high-rate one-coset conjugate-state trace distance or a matching no-go extension.",
                "Identify accessible irreducible sectors of (GL_k(2) x S_n) wr Z_2.",
                "Construct and cost a decoder before treating any information signal as algorithmic progress.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-ROWSPACE-HSP-REDUCTION",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Rowspace-canonical self-dual code HSP reduction",
            status="planned",
            hypothesis=(
                "For explicit full-rank code generators, the GL_k row scrambler is removable gauge: canonical "
                "rowspace evaluation reduces code equivalence to hidden shift over S_n, so failure of a no-go "
                "dimension condition caused by |GL_k| is not a quantum opening."
            ),
            protocol=(
                "Define f_C(P)=RREF(MP), verify invariance under invertible row operations, verify the hidden-shift "
                "identity under mapped coordinate permutations, and verify that the identity level set is exactly "
                "PAut(C). Compare raw and rowspace-canonical wreath-product group sizes. Keep rigidity, automorphism "
                "size/minimal degree, collective measurement, and decoder obligations explicit."
            ),
            positive_signal=(
                "No direct quantum-positive signal. The useful result is an exact smaller reduction that identifies "
                "the actual symmetric-group automorphism and collective-measurement bottleneck."
            ),
            falsifiers=[
                "RREF changes under an invertible row operation.",
                "The rowspace hiding functions fail the hidden-shift identity.",
                "A permutation lies in the identity level set without stabilizing the code.",
                "The removable GL_k factor is retained as evidence of a high-rate representation-theoretic opening.",
                "No duplicate columns is represented as proof of rigidity.",
                "A graph-isomorphism-style no-go is imported without proving the automorphism hypotheses.",
            ],
            metrics=[
                "hidden_shift_control_failure_count",
                "gl_factor_eliminated_family_count",
                "maximum_eliminated_group_log2_size",
                "maximum_rowspace_wreath_log2_size",
                "obvious_nonrigid_instance_count",
                "rigidity_certified_instance_count",
                "large_minimal_degree_certified_instance_count",
                "gi_type_order_two_no_go_certified_instance_count",
                "explicit_collective_measurement_count",
                "polynomial_hidden_permutation_decoder_count",
            ],
            dependencies=[
                "self_dual_rowspace_hsp_reduction.py",
                "self_dual_code_boundary_search.py",
                "self_dual_hsp_applicability.py",
                "Dinh-Moore-Russell-2015-Code-Equivalence-Coset-States",
                "Moore-Russell-Schulman-2008-Symmetric-Group-Fourier",
            ...
            next_actions=[
                "Run qsearch.py code-self-dual-rowspace-hsp.",
                "Certify PAut(C), automorphism size, and minimal degree on the scalable self-dual tail.",
                "Apply or extend the symmetric-group coset-state no-go only after those hypotheses are proved.",
                "Search for collective measurements only on rows that survive the no-go audit.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-AUTOMORPHISM-WORKBENCH",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Self-dual code automorphism and rigidity workbench",
            status="planned",
            hypothesis=(
                "Exact bounded-weight codeword-support hypergraphs can stratify scalable self-dual instances into "
                "rigid GI-type HSP cases, explicitly nonrigid stabilizer cases, and honest unresolved debt."
            ),
            protocol=(
                "Use self-duality to enumerate every support of weight at most eight by an equal-syndrome "
                "meet-in-the-middle construction. Run automorphism-invariant color refinement on the resulting "
                "coordinate/support incidence graph. Accept singleton coordinate colors as a rigidity certificate; "
                "verify every nonidentity graph symmetry against the full code rowspace; retain all other outcomes "
                "as proof debt."
            ),
            positive_signal=(
                "No direct quantum-positive signal. The result determines which symmetric-group coset-state "
                "obstruction is legally applicable and prevents measurement search on incorrectly classified rows."
            ),
            falsifiers=[
                "A generated bounded-weight support fails the public parity-check zero-syndrome condition.",
                "Bounded support enumeration misses a support of weight at most eight.",
                "A nonidentity graph symmetry fails full rowspace verification.",
                "A failed automorphism search is represented as rigidity.",
                "Finite sampled rigidity is represented as an infinite-family theorem.",
                "A single-register GI-type no-go is represented as closing collective measurements.",
            ],
            metrics=[
                "complete_bounded_support_enumeration_count",
                "maximum_enumerated_half_subset_count",
                "rigidity_certified_instance_count",
                "explicit_automorphism_instance_count",
                "unresolved_instance_count",
                "singleton_refinement_certificate_count",
                "gi_type_single_register_no_go_certified_instance_count",
                "infinite_family_rigidity_theorem_count",
                "collective_measurement_count",
                "polynomial_hidden_permutation_decoder_count",
            ],
            dependencies=[
                "self_dual_automorphism_workbench.py",
                "self_dual_code_boundary_search.py",
                "self_dual_rowspace_hsp_reduction.py",
                "networkx",
                "Moore-Russell-Schulman-2008-Symmetric-Group-Fourier",
            ...
            next_actions=[
                "Run qsearch.py code-self-dual-automorphisms.",
                "Raise the exact support order or add an algebraic stabilizer solver for unresolved k=32 rows.",
                "Prove a growing-family automorphism theorem rather than extrapolating finite samples.",
                "Route rigid and nonrigid strata to their correct coset-state analyses.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-HIGH-ORDER-AUTOMORPHISM-RESOLVER",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Higher fixed-order self-dual automorphism resolver",
            status="planned",
            hypothesis=(
                "The unresolved length-64 self-dual rows are sparse only at support weight eight; exact weight-ten "
                "supports may already certify rigidity and route them into the GI-type single-register obstruction."
            ),
            protocol=(
                "Retain the order-eight artifact, select only unresolved instances, pack every size-at-most-five "
                "coordinate subset as a syndrome/support record, sort by syndrome, enumerate all zero-sum supports "
                "through weight ten, and run automorphism-invariant coordinate refinement. Accept only singleton "
                "coordinate colors as rigidity certificates."
            ),
            positive_signal=(
                "No direct quantum-positive signal. Resolution identifies the correct rigid HSP stratum and moves "
                "research effort from single-register Fourier sampling to the genuine collective-measurement barrier."
            ),
            falsifiers=[
                "Packed enumeration misses a support of weight at most ten.",
                "An emitted support fails the public zero-syndrome check.",
                "Stable coordinate colors are not singleton.",
                "Fixed order-ten success is represented as an infinite-family theorem.",
                "A rigid single-register obstruction is represented as ruling out collective measurements.",
            ],
            metrics=[
                "target_instance_count",
                "resolved_rigidity_instance_count",
                "remaining_unresolved_instance_count",
                "complete_high_order_enumeration_count",
                "maximum_enumerated_half_subset_count",
                "maximum_packed_array_bytes",
                "singleton_refinement_certificate_count",
                "gi_type_single_register_no_go_certified_instance_count",
                "infinite_family_rigidity_theorem_count",
                "collective_measurement_count",
            ...
            dependencies=[
                "self_dual_high_order_automorphism_resolver.py",
                "self_dual_automorphism_workbench.py",
                "self_dual_rowspace_hsp_reduction.py",
                "numpy",
                "networkx",
            ...
            next_actions=[
                "Run qsearch.py code-self-dual-high-order.",
                "Generate and classify larger self-dual dimensions without assuming fixed weight ten remains sufficient.",
                "On rigidity-certified rows, stop searching single-register Fourier observables.",
                "Construct or obstruct genuinely collective measurements with full decoder accounting.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-FIXED-ORDER-SPARSITY-OBSTRUCTION",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Fixed-order self-dual support sparsity obstruction",
            status="planned",
            hypothesis=(
                "The successful finite weight-eight/ten automorphism certificates cannot scale on a uniform "
                "rate-one-half self-dual ensemble because every fixed-weight support count vanishes exponentially."
            ),
            protocol=(
                "Derive the exact membership probability of a nontrivial even vector in a uniform binary self-dual "
                "code via the Lagrangian quotient by the all-ones vector. Compute exact expected support counts, "
                "calibrate them against observed artifacts, solve the entropy-half threshold, and cost the explicit "
                "meet-in-the-middle order needed for linearly many supports."
            ),
            positive_signal=(
                "No direct quantum-positive signal. The obstruction rejects a classical certificate architecture "
                "and forces either an implicit growing-weight invariant, a uniform automorphism theorem, or a "
                "genuinely collective quantum mechanism."
            ),
            falsifiers=[
                "The self-dual ensemble membership probability is counted incorrectly.",
                "Observed support counts are represented as independent samples or a family proof.",
                "Finite weight-ten success is extrapolated to fixed-order asymptotic rigidity.",
                "Failure of this classical certificate is represented as quantum advantage.",
                "Implicit algebraic growing-weight invariants are represented as ruled out.",
            ],
            metrics=[
                "weight_10_expected_below_one_length_count",
                "linear_required_weight_length_count",
                "exponential_enumeration_length_count",
                "entropy_half_relative_weight_threshold",
                "asymptotic_fixed_order_rigidity_certificate_count",
                "infinite_family_automorphism_theorem_count",
                "collective_measurement_count",
            ],
            dependencies=[
                "self_dual_fixed_order_sparsity_obstruction.py",
                "self_dual_automorphism_workbench.py",
                "self_dual_high_order_automorphism_resolver.py",
                "symplectic Lagrangian counting",
                "binary entropy bounds",
            ],
            next_actions=[
                "Run qsearch.py code-self-dual-sparsity.",
                "Stop treating explicit fixed support order as the asymptotic automorphism route.",
                "Search for implicit growing-weight algebraic invariants or prove a random self-dual rigidity theorem.",
                "Keep collective quantum measurement and decoder obligations separate from this classical obstruction.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-SPECTRUM",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Rigid code-equivalence wreath-product spectrum",
            status="planned",
            hypothesis=(
                "The rigid rowspace HSP must be analyzed in the exact wreath product "
                "(S_n x S_n) semidirect Z_2; its bridge class has a tractable one-copy representation spectrum "
                "that identifies the precise growing-copy covariant-decoder obligation."
            ),
            protocol=(
                "Classify wreath irreps by unequal induced pairs and equal +/- extensions, derive bridge character "
                "ratios, verify the regular-representation dimension sum, compute the exact class-average frame, "
                "one-copy PGM success, Holevo information, and copy lower bounds, and keep growing-copy diagonal "
                "action, carrier-sensitive POVM, and hidden-permutation decoding as separate gates."
            ),
            positive_signal=(
                "A useful result is a polynomial-description growing-copy block decomposition and covariant decoder "
                "built on the correct wreath schema. One-copy spectral formulas alone are not positive evidence."
            ),
            falsifiers=[
                "The code-equivalence bridge ensemble is silently replaced by an involution class in S_n.",
                "Wreath irrep dimensions fail to square-sum to the group order.",
                "Weak Fourier labels are represented as identifying the hidden bridge.",
                "One-copy PGM success is represented as a hidden-permutation algorithm.",
                "Polynomial copy complexity is represented as a polynomial measurement or decoder.",
            ],
            metrics=[
                "exact_irrep_dimension_sum_verification_count",
                "weak_fourier_zero_information_record_count",
                "maximum_exact_pgm_advantage_over_guess",
                "minimum_exact_one_copy_holevo_bits",
                "maximum_coarse_zero_error_copy_lower_bound",
                "one_copy_wreath_fourier_schema_count",
                "growing_copy_diagonal_action_transform_count",
                "carrier_sensitive_covariant_povm_count",
                "polynomial_hidden_permutation_decoder_count",
            ],
            dependencies=[
                "self_dual_wreath_spectrum.py",
                "self_dual_rowspace_hsp_reduction.py",
                "self_dual_high_order_automorphism_resolver.py",
                "representation theory of H wr S_2",
                "coset_holevo_information.py",
            ],
            next_actions=[
                "Run qsearch.py code-self-dual-wreath.",
                "Construct the k=Theta(log n!) diagonal-action multiplicity decomposition for W_n.",
                "Specify a carrier-sensitive covariant POVM with compressed outcomes.",
                "Prove a polynomial hidden-permutation decoder and compare against classical code canonicalization.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-HECKE-AUDIT",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Rigid code-equivalence wreath Hecke/Gelfand audit",
            status="planned",
            hypothesis=(
                "The bridge-label stabilizer C_n yields a multiplicity-free scalar Hecke algebra, but it must be "
                "separated from the actual order-two hidden subgroup H. This separation should remove the factorial "
                "scalar kernel table while exposing the operator-valued multi-copy frame as the real obligation."
            ),
            protocol=(
                "Identify C_W(h_e), W_n/C_n, and C_n double cosets; decompose Ind_C^W(1); verify exact symmetric-group "
                "character orthogonality and class-sum moments; compute H-fixed multiplicities in every finite wreath "
                "irrep control; derive the normalized k-copy Hilbert-Schmidt kernel and its spectrum; and audit the "
                "Moore-Russell Gelfand-pair hypothesis against H rather than C."
            ),
            positive_signal=(
                "A useful result is a polynomial coherent reduction of the operator-valued k-copy frame, followed by "
                "a carrier-sensitive covariant POVM and decoder. Scalar commutativity or a finite character table "
                "alone is not positive evidence."
            ),
            falsifiers=[
                "The label stabilizer C_n is confused with the hidden subgroup H=<h_e>.",
                "All-register PGM optimality is imported although (W_n,H) has irrep multiplicity greater than one.",
                "A p(n)-dimensional scalar Hecke transform is represented as an operator-valued frame diagonalization.",
                "The equality-only Hilbert-Schmidt kernel is represented as carrying permutation cycle geometry.",
                "Grouping 2^k subset terms into k+1 size orbits is represented as a coherent transform or decoder.",
            ],
            metrics=[
                "centralizer_gelfand_pair_proof_count",
                "finite_character_orthogonality_verification_count",
                "actual_hidden_subgroup_non_gelfand_count",
                "maximum_exact_hidden_subgroup_irrep_multiplicity",
                "factorial_hidden_label_table_elimination_count",
                "pairwise_hs_kernel_collapse_count",
                "maximum_information_threshold_copy_count",
                "maximum_register_subset_orbit_count",
                "uniform_polynomial_coherent_scalar_transform_count",
                "operator_valued_kcopy_frame_reduction_count",
                "carrier_sensitive_covariant_povm_count",
                "polynomial_hidden_permutation_decoder_count",
            ],
            dependencies=[
                "self_dual_wreath_hecke_audit.py",
                "self_dual_wreath_spectrum.py",
                "symmetric_character.py",
                "Moore-Russell-2005-Conjugate-Hidden-Subgroup-PGM",
                "Moore-Russell-2005-Explicit-Multiregister-Measurement",
            ],
            next_actions=[
                "Run qsearch.py code-self-dual-wreath-hecke.",
                "Build the exact subset-indexed operator moment algebra for the k-copy average frame.",
                "Exploit register-permutation orbit compression without assuming the orbit sums commute.",
                "Find a polynomial carrier transform, frame inverse, covariant POVM, and hidden-permutation decoder.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-PGM-POLAR-AUDIT",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Rigid code-equivalence wreath PGM polar audit",
            status="planned",
            hypothesis=(
                "The mixed-state PGM admits an exact polar-isometry reduction and compact average-frame LCU, but "
                "generic inversion should encounter a factorial normalization barrier that only a structured "
                "representation-specific preconditioner can remove."
            ),
            protocol=(
                "Define the k-copy support projectors P_s, average frame B_k, and analysis operator A_k; derive "
                "A_k^*A_k=B_k and the PGM effects; expand B_k over permutation labels and subset masks; compute exact "
                "first/second frame moments, effective-rank fractions, candidate-verification soundness, and the "
                "polar singular scale at k=ceil(log2(n!)); charge generic QSVT and optimistic Grover baselines."
            ),
            positive_signal=(
                "A useful result is a uniform structured preconditioner or direct carrier polar transform whose "
                "complexity depends polynomially on n rather than the Theta((n!)^-1/2) generic singular scale, "
                "together with a success theorem under ordinary mixed coset-state access."
            ),
            falsifiers=[
                "A compact normalization-one LCU block encoding is represented as a polynomial frame inverse.",
                "Generic inverse-square-root resolution at singular scale Theta((n!)^-1/2) is represented as polynomial.",
                "An unknown mixed coset state is treated as a reusable coherent candidate-verification oracle.",
                "A direct permutation output register is represented as proving PGM success or efficient implementation.",
                "Candidate equality verification is represented as a structured search over S_n.",
            ],
            metrics=[
                "operator_frame_polar_reduction_count",
                "average_frame_lcu_contract_count",
                "pairwise_candidate_verifier_formula_count",
                "maximum_copy_count",
                "minimum_effective_rank_fraction",
                "maximum_generic_polar_resolution_log2_charge",
                "maximum_optimistic_grover_log2_candidate_queries",
                "coherent_candidate_verifier_reuse_count",
                "uniform_polynomial_structured_preconditioner_count",
                "polynomial_frame_inverse_count",
                "carrier_sensitive_povm_circuit_count",
                "polynomial_hidden_permutation_decoder_count",
            ],
            dependencies=[
                "self_dual_wreath_pgm_polar_audit.py",
                "self_dual_wreath_hecke_audit.py",
                "self_dual_wreath_spectrum.py",
                "Moore-Russell-2005-Explicit-Multiregister-Measurement",
                "Bacon-Childs-van-Dam-2005-Semidirect-PGM",
            ],
            next_actions=[
                "Run qsearch.py code-self-dual-wreath-pgm.",
                "Construct the subset-orbit carrier algebra and test whether its symmetrized generators commute.",
                "Search representation blocks for a polynomial spectral preconditioner of B_k.",
                "Prove PGM success or design a different legal covariant measurement under mixed coset-state access.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-SUBSET-CARRIER-ALGEBRA",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Wreath k-copy subset-orbit carrier algebra",
            status="planned",
            hypothesis=(
                "Register symmetry compresses the 2^k average-frame subset terms into k+1 orbit sums, but carrier "
                "multiplicities make the resulting algebra noncommutative; exact commutator and word-growth controls "
                "should determine whether scalar/Krawtchouk preconditioning is viable."
            ),
            protocol=(
                "Represent T_A and U_r as exact sparse elements of the tensor wreath group algebra; verify bridge "
                "multiplication controls; compute disjoint, overlapping, and fully register-symmetrized commutators; "
                "then generate bounded-depth words in the U_r and certify modular lower bounds on their span without "
                "constructing factorial Hilbert-space matrices."
            ),
            positive_signal=(
                "A useful result is a uniform noncommutative multiplicity-block transform whose blocks, spectra, and "
                "frame preconditioner have polynomial descriptions and gate complexity. A k+1 formal orbit count "
                "alone is not positive evidence."
            ),
            falsifiers=[
                "Disjoint-register operators fail to commute, invalidating the sparse multiplication control.",
                "Overlapping and symmetrized orbit operators are silently assumed to share one eigenbasis.",
                "A scalar Krawtchouk transform is used after an exact nonzero carrier commutator.",
                "Finite modular rank lower bounds are represented as exact all-n algebra dimensions.",
                "Bounded-depth finite closure is represented as a uniform polynomial block transform.",
            ],
            metrics=[
                "commutator_record_count",
                "commuting_control_count",
                "overlapping_subset_noncommutation_count",
                "symmetrized_orbit_noncommutation_count",
                "first_symmetrized_noncommuting_copy_count",
                "maximum_truncated_algebra_rank_lower_bound",
                "maximum_truncated_word_support_size",
                "scalar_krawtchouk_preconditioner_count",
                "uniform_noncommutative_carrier_block_transform_count",
                "polynomial_structured_frame_preconditioner_count",
                "polynomial_frame_inverse_count",
                "polynomial_hidden_permutation_decoder_count",
            ],
            dependencies=[
                "self_dual_wreath_subset_carrier_algebra.py",
                "self_dual_wreath_pgm_polar_audit.py",
                "self_dual_wreath_hecke_audit.py",
                "tensor wreath group algebra",
                "register-permutation commutant",
            ],
            next_actions=[
                "Run qsearch.py code-self-dual-wreath-carrier.",
                "Identify the representation-theoretic blocks generated by U_r under diagonal W_n x S_k symmetry.",
                "Search for a uniform sparse recurrence or block Jacobi form for the carrier algebra.",
                "Construct and condition a noncommutative frame preconditioner before attempting PGM synthesis.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-CARRIER-ORBIT-GROWTH",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Wreath carrier hidden-label orbit-growth theorem",
            status="planned",
            hypothesis=(
                "The noncommutative carrier algebra cannot scale through explicit orbit tables: after fixing one "
                "bridge label, depth-three words are indexed by simultaneous conjugacy orbits of permutation pairs, "
                "whose Burnside count is already factorial."
            ),
            protocol=(
                "Reduce d bridge labels under diagonal left-right wreath conjugation to d-1 relative permutations "
                "under simultaneous S_n conjugation; derive the Burnside count sum z_lambda^(d-2); bound the extra "
                "wreath swap by a factor of two; compare hidden-label orbit growth with register-subset intersection "
                "profiles at the information-threshold copy count; and verify exact finite character partitions."
            ),
            positive_signal=(
                "A useful result is a compressed harmonic/representation transform that accesses factorial carrier "
                "sectors without enumerating them, with polynomial block labels, matrix elements, conditioning, and "
                "coherent circuit complexity."
            ),
            falsifiers=[
                "Polynomial subset-intersection profile counts are represented as polynomial hidden-label orbit counts.",
                "The identity centralizer contribution n! is omitted from the depth-three Burnside sum.",
                "The wreath swap is represented as collapsing more than a factor two without proof.",
                "An explicit factorial orbit table is represented as a scalable block transform.",
                "Finite exact counts are represented as a harmonic circuit or frame preconditioner.",
            ],
            metrics=[
                "record_count",
                "exact_record_count",
                "factorial_hidden_label_orbit_lower_bound_count",
                "maximum_log2_full_wreath_orbit_lower_bound",
                "maximum_log2_subset_profile_upper_bound",
                "explicit_polynomial_carrier_orbit_table_count",
            ...
            dependencies=[
                "self_dual_wreath_carrier_orbit_growth.py",
                "self_dual_wreath_subset_carrier_algebra.py",
                "Burnside lemma",
                "symmetric-group centralizer orders",
                "simultaneous conjugacy harmonic analysis",
            ],
            next_actions=[
                "Run qsearch.py code-self-dual-wreath-orbits.",
                "Replace explicit carrier orbits with irreducible and multiplicity labels under simultaneous conjugation.",
                "Derive sparse recoupling rules for multiplying subset-orbit operators in that harmonic basis.",
                "Search for a blockwise conditioned frame preconditioner and direct polar transform.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-HARMONIC-CARRIER-SCHEMA",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Wreath harmonic carrier block schema",
            status="planned",
            hypothesis=(
                "Depth-three simultaneous-conjugacy carrier sectors admit an exact irreducible/multiplicity schema, "
                "but compact labels alone do not remove factorial complexity because Kronecker multiplicity blocks "
                "can have n!/p(n) matrix coordinates."
            ),
            protocol=(
                "Decompose C[S_n] under conjugation as a direct sum of V_nu with multiplicity "
                "m_nu=sum_lambda g(lambda,lambda,nu); verify sum_nu m_nu^2 against the Burnside pair-orbit count; "
                "enumerate exact finite blocks through n=12; prove tail lower bounds from the identity conjugacy "
                "class; and separately audit coordinate address size, internal Kronecker transforms, sparse product "
                "rules, conditioning, and coherent circuit complexity."
            ),
            positive_signal=(
                "A useful result is an all-n sparse internal Kronecker transform and carrier multiplication recurrence "
                "whose gate and condition-number bounds avoid materializing the superpolynomial multiplicity blocks."
            ),
            falsifiers=[
                "A compact partition and multiplicity-index address is represented as an efficient coherent transform.",
                "The p(n) outer sectors are represented as polynomially many sectors.",
                "Dense n!/p(n) multiplicity blocks are materialized or hidden behind finite tables.",
                "An S_n group QFT is represented as resolving internal Kronecker multiplicities.",
                "Finite exact Kronecker coefficients are represented as sparse all-n carrier-product rules.",
            ],
            metrics=[
                "harmonic_burnside_identity_verification_count",
                "exact_block_record_count",
                "non_multiplicity_free_block_count",
                "maximum_exact_conjugation_multiplicity",
                "maximum_log2_certified_block_coordinate_lower_bound",
                "maximum_log2_certified_multiplicity_lower_bound",
                "internal_kronecker_basis_transform_count",
                "sparse_carrier_product_rule_count",
                "uniform_coherent_harmonic_transform_count",
                "polynomial_structured_frame_preconditioner_count",
                "polynomial_hidden_permutation_decoder_count",
            ],
            dependencies=[
                "self_dual_wreath_harmonic_carrier_schema.py",
                "self_dual_wreath_carrier_orbit_growth.py",
                "symmetric_character.py",
                "Peter-Weyl decomposition for S_n",
                "Kronecker coefficients",
                "Burnside lemma",
            ],
            next_actions=[
                "Run qsearch.py code-self-dual-wreath-harmonics.",
                "Derive sparse carrier-generator matrix elements directly in the Kronecker multiplicity basis.",
                "Search stable-shape and bounded-width recurrences before attempting generic multiplicity blocks.",
                "Prove a coherent internal transform and conditioned frame action without dense block enumeration.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-COMMUTANT-TRANSFER-AUDIT",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Wreath restricted commutant capability transfer",
            status="planned",
            hypothesis=(
                "The existing all-n bounded-support commutant gap for "
                "(n-2,2) tensor (n-2,2) transfers to one genuine equal-source carrier sector, but its four "
                "resolved invariant coordinates have factorially negligible coverage and do not control frame mixing."
            ),
            protocol=(
                "Rebuild the symbolic commutant-gap certificate; verify multiplicity two for the equal-source "
                "lambda=(n-2,2), target=(n-3,2,1) sector; enumerate all equal-source Kronecker sectors through "
                "n=12; compare the four resolved matrix coordinates with exact simultaneous-conjugacy dimensions; "
                "extend the 4/n! coverage upper bound through n=64; and audit source routing, cross-source carrier "
                "matrix elements, frame invariance, and decoder obligations separately."
            ),
            positive_signal=(
                "A useful result is a general equal-source and cross-source bounded-support commutant family with "
                "inverse-polynomial gaps, explicit carrier-generator matrix elements, and invariant frame blocks "
                "covering nonnegligible natural coset-state mass."
            ),
            falsifiers=[
                "The restricted multiplicity-two theorem is represented as a general Kronecker transform.",
                "Four resolved coordinates are represented as nonnegligible coverage of an at-least-n! carrier space.",
                "Source and target routing is represented as invariance under the actual frame.",
                "Cross-source carrier matrix elements are omitted from a frame-transform claim.",
                "An inverse-polynomial label gap is represented as a hidden-permutation decoder.",
            ],
            metrics=[
                "restricted_all_n_inverse_polynomial_gap_theorem_count",
                "restricted_invariant_matrix_coordinate_count",
                "maximum_exact_equal_source_nonzero_sector_count",
                "maximum_exact_nontrivial_multiplicity_sector_count",
                ...
                "general_equal_source_gap_theorem_count",
                "cross_source_carrier_mixing_rule_count",
                "carrier_frame_invariant_subspace_count",
                "polynomial_hidden_permutation_decoder_count",
            ],
            dependencies=[
                "self_dual_wreath_commutant_transfer_audit.py",
                "self_dual_wreath_harmonic_carrier_schema.py",
                "coset_commutant_gap_certificate.py",
                "symmetric_character.py",
                "bounded-support commutant block encodings",
            ],
            next_actions=[
                "Run qsearch.py code-self-dual-wreath-commutant.",
                "Derive the actual frame generator matrix elements between source lambda sectors.",
                "Search uniform bounded-support separators across representative equal-source sectors.",
                "Prove invariant carrier subspaces and nonnegligible natural access before using restricted gap tools.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-PHYSICAL-FRAME-BLOCKS",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Physical wreath Fourier frame-block probe",
            status="planned",
            hypothesis=(
                "The actual k-copy frame is exactly block diagonal in tuples of physical W_n irreps, and selected "
                "equal-pair blocks can be constructed from correlated bridge involutions; finite spectra will expose "
                "conditioning and kernel structure without resolving hidden-label harmonic multiplicities."
            ),
            protocol=(
                "Build every S_n Young-seminormal permutation matrix for selected partitions; construct the +/- "
                "equal-pair wreath bridge matrices; verify involution, Hermiticity, and one-copy class scalars; form "
                "B_(pi_1,...,pi_k)=(1/n!) sum_s tensor_i(I+pi_i(h_s))/2 for every sign count; diagonalize all S_3 "
                "information-threshold blocks and representative S_4/S_5 controls; and separately audit unequal "
                "irreps, growing tuple coverage, harmonic-source conservation, recurrences, and preconditioning."
            ),
            positive_signal=(
                "A useful result is a uniform all-n recurrence or block encoding covering all physical irrep tuples "
                "with inverse-polynomial support conditioning and a coherent blockwise frame inverse."
            ),
            falsifiers=[
                "Physical wreath-irrep tuple conservation is represented as hidden-label harmonic-source conservation.",
                "Finite S_3 information-threshold conditioning is represented as all-n conditioning.",
                "Selected equal-pair blocks are represented as coverage of unequal-pair and mixed-partition tuples.",
                "Dense finite eigendecomposition is represented as a coherent recurrence or preconditioner.",
                "A block formula is represented as a hidden-permutation decoder.",
            ],
            metrics=[
                "record_count",
                "information_threshold_block_count",
                "maximum_block_dimension",
                "maximum_distinct_eigenvalue_count",
                "minimum_positive_eigenvalue",
                "maximum_support_condition_number",
                "unequal_pair_physical_block_count",
                "all_partition_tuple_block_count",
                "uniform_all_n_spectral_recurrence_count",
                "polynomial_structured_frame_preconditioner_count",
                "polynomial_hidden_permutation_decoder_count",
            ],
            dependencies=[
                "self_dual_wreath_physical_frame_blocks.py",
                "self_dual_wreath_pgm_polar_audit.py",
                "self_dual_wreath_spectrum.py",
            ...
            next_actions=[
                "Run qsearch.py code-self-dual-wreath-frame-blocks.",
                "Construct unequal-pair induced wreath irreps and mixed physical-irrep tuple blocks.",
                "Search character-moment recurrences for block spectra at growing n and k.",
                "Prove worst-sector support conditioning before synthesizing a blockwise inverse.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-UNEQUAL-FRAME-BLOCKS",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Unequal-pair wreath collective frame blocks",
            status="planned",
            hypothesis=(
                "Unequal-pair induced wreath irreps have exactly zero one-copy bridge character but nontrivial "
                "correlated multi-copy frame spectra, so discarding them from a collective decoder would erase "
                "genuine carrier structure."
            ),
            protocol=(
                "Construct the induced representation on (V_lambda tensor V_mu) direct sum "
                "(V_mu tensor V_lambda); verify every bridge is a Hermitian involution with zero class average; "
                "form repeated-irrep correlated frame blocks; diagonalize all unequal W_3 sectors through the "
                "information threshold, every W_4 unequal pair through two copies, and representative W_5 controls; "
                "record kernels, eigenvalues, conditioning, and missing mixed-tuple/all-n recurrence obligations."
            ),
            positive_signal=(
                "A useful result is a uniform recurrence covering mixed physical-irrep tuples, with worst-sector "
                "inverse-polynomial conditioning and a coherent blockwise frame inverse."
            ),
            falsifiers=[
                "Zero one-copy bridge character is represented as a scalar multi-copy frame.",
                "Repeated copies of one unequal irrep are represented as mixed-irrep tuple coverage.",
                "Finite W_3 threshold spectra are represented as all-n conditioning.",
                "Dense finite diagonalization is represented as a coherent recurrence.",
                "Collective spectral structure is represented as a hidden-permutation decoder.",
            ],
            metrics=[
                "record_count",
                "one_copy_zero_character_control_count",
                "collective_nontrivial_spectrum_count",
                "information_threshold_unequal_block_count",
                "maximum_block_dimension",
                "maximum_distinct_eigenvalue_count",
                "minimum_positive_eigenvalue",
                "maximum_support_condition_number",
            ...
            dependencies=[
                "self_dual_wreath_unequal_frame_blocks.py",
                "self_dual_wreath_physical_frame_blocks.py",
                "self_dual_wreath_spectrum.py",
                "induced representations of (S_n x S_n) semidirect C_2",
            ],
            next_actions=[
                "Run qsearch.py code-self-dual-wreath-unequal-blocks.",
                "Construct mixed tuples containing distinct equal and unequal physical irreps.",
                "Derive trace-moment recurrences from symmetric-group characters.",
                "Search worst-sector conditioning at the information-threshold copy count.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-COMPLETE-W3-TUPLES",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Complete W_3 threshold physical-tuple audit",
            status="planned",
            hypothesis=(
                "Enumerating every naturally occupied physical-irrep tuple of W_3 at the exact three-copy "
                "information threshold will expose the true finite worst-sector support conditioning and determine "
                "whether representative repeated-irrep probes missed a harder mixed tuple."
            ),
            protocol=(
                "Construct all nine W_3 irreps and their bridge matrices; verify dimensions and exact coset-state "
                "label probabilities; enumerate all 165 unordered three-irrep tuples; diagonalize each correlated "
                "frame block; aggregate multinomial natural mass; separate zero-mass anti-trivial labels; and record "
                "support ranks, kernels, minimum positive eigenvalues, condition numbers, and missing all-n "
                "character-recurrence/coherent-inverse obligations."
            ),
            positive_signal=(
                "A useful result is a character-moment recurrence that reproduces the complete W_3 table and extends "
                "to all naturally occupied W_n tuple sectors at k=Theta(log n!), with inverse-polynomial support "
                "conditioning and coherent pseudoinversion."
            ),
            falsifiers=[
                "Zero-natural-mass tuples are included in worst naturally occupied conditioning.",
                "A kernel is treated as failure rather than restricting to the frame support.",
                "Complete W_3 dense diagonalization is represented as an all-n recurrence.",
                "Mild support conditioning is represented as a coherent pseudoinverse.",
                "Physical frame inversion is represented as a hidden-label transform or decoder.",
            ],
            metrics=[
                "physical_irrep_count",
                "unordered_threshold_tuple_count",
                "naturally_occupied_threshold_tuple_count",
                "natural_tuple_mass_sum",
                "natural_mass_in_kernel_blocks",
                "minimum_naturally_occupied_positive_eigenvalue",
                "maximum_naturally_occupied_support_condition_number",
                "uniform_all_n_character_moment_recurrence_count",
                "coherent_blockwise_frame_inverse_count",
                "polynomial_hidden_permutation_decoder_count",
            ],
            dependencies=[
                "self_dual_wreath_complete_w3_tuple_audit.py",
                "self_dual_wreath_physical_frame_blocks.py",
                "self_dual_wreath_unequal_frame_blocks.py",
                "exact W_3 representation catalog",
                "coset-state irrep label law",
            ],
            next_actions=[
                "Run qsearch.py code-self-dual-wreath-w3-tuples.",
                "Derive low-order trace moments directly from wreath characters and bridge-product conjugacy classes.",
                "Validate the recurrence against all 165 W_3 blocks and selected W_4 blocks.",
                "Prove growing-n support conditioning before attempting coherent pseudoinversion.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-CHARACTER-MOMENTS",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Physical wreath-frame character-moment recurrence",
            status="planned",
            hypothesis=(
                "Mixed physical-irrep frame blocks admit exact character contractions that replace explicit "
                "factorial hidden-permutation tables and can eventually expose a uniform support-gap recurrence."
            ),
            protocol=(
                "Derive the exact m-th trace-character expansion, prove the all-n second-moment partition-class "
                "formula, reproduce every complete W_3 threshold tuple moment through fourth order, and measure "
                "the simultaneous-conjugacy orbit barrier that remains for naive third moments."
            ),
            positive_signal=(
                "A polynomial symbolic third/higher-moment contraction or minimal-polynomial recurrence that "
                "extends to k=Theta(log n!), bounds every naturally occupied support gap, and supports coherent "
                "pseudoinversion."
            ),
            falsifiers=[
                "A second-moment effective-rank estimate is presented as a minimum-eigenvalue theorem.",
                "Finite W_3 moment agreement is presented as an all-n spectral recurrence.",
                "Third moments require explicit factorial simultaneous-conjugacy orbit tables.",
                "A classical character contraction is presented as a coherent blockwise transform.",
                "A support-gap theorem is claimed without controlling kernel-bearing natural sectors.",
            ],
            metrics=[
                "w3_validated_tuple_moment_count",
                "w3_failed_tuple_moment_count",
                "exact_second_moment_class_recurrence_count",
                "maximum_partition_class_term_count",
                "third_moment_factorial_orbit_barrier_count",
                "polynomial_third_moment_contraction_count",
                "all_moment_spectral_recurrence_count",
                "coherent_support_projector_count",
                "coherent_blockwise_frame_pseudoinverse_count",
                "polynomial_hidden_permutation_decoder_count",
            ],
            dependencies=[
                "self_dual_wreath_character_moments.py",
                "self_dual_wreath_complete_w3_tuple_audit.py",
                "symmetric_character.py",
                "wreath-product physical irrep character formulas",
            ],
            next_actions=[
                "Run qsearch.py code-self-dual-wreath-moments.",
                "Replace raw third-moment pair-orbit enumeration by a character or representation-ring contraction.",
                "Derive support-gap consequences only after enough moments or a minimal polynomial are controlled.",
                "Compile any classical contraction into an explicit coherent access model before claiming a decoder.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-THIRD-MOMENT-CONTRACTION",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Special-sector symbolic third-moment contraction",
            status="planned",
            hypothesis=(
                "The repeated trivial-standard unequal physical sector admits an exact polynomial coefficient "
                "recurrence for its third frame moment, eliminating the factorial permutation-pair table."
            ),
            protocol=(
                "Reduce the character formula to pairwise fixed-point agreements among three permutations; express "
                "the weighted q-sum as a permanent of identity/r matching perturbations; factor its rook polynomial "
                "over cycles; aggregate cycle types by a bivariate cycle-index recurrence; and validate against direct "
                "pair enumeration and the general wreath-character formula."
            ),
            positive_signal=(
                "An exact recurrence with polynomially many coefficient states and no pair enumeration, followed by "
                "a generalization to equal-pair commutator terms and arbitrary mixed naturally occupied sectors."
            ),
            falsifiers=[
                "The recurrence disagrees with direct pair distributions or character moments.",
                "Coefficient-state support grows factorially.",
                "One repeated unequal sector is represented as all-sector coverage.",
                "A third moment is represented as a support-gap theorem.",
                "A classical exact recurrence is represented as a coherent pseudoinverse or decoder.",
            ],
            metrics=[
                "failed_distribution_validation_count",
                "failed_moment_validation_count",
                "maximum_scaling_n",
                "maximum_bivariate_state_count",
                "explicit_factorial_pair_enumeration_count",
                "exact_polynomial_third_moment_contraction_count",
                "covered_physical_irrep_family_count",
                "all_physical_irrep_sector_contraction_count",
                "support_gap_theorem_count",
                "polynomial_hidden_permutation_decoder_count",
            ],
            dependencies=[
                "self_dual_wreath_third_moment_contraction.py",
                "self_dual_wreath_character_moments.py",
                "weighted rook polynomials",
                "symmetric-group cycle index",
            ],
            next_actions=[
                "Run qsearch.py code-self-dual-wreath-third-moment.",
                "Extend the class contraction to unequal mixed tuples.",
                "Contract equal-pair commutator-character terms through representation-ring or spin-network data.",
                "Seek support-gap consequences only after all naturally occupied sectors are covered.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-ALL-UNEQUAL-THIRD-MOMENT",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="All-unequal physical-sector third-moment class contraction",
            status="planned",
            hypothesis=(
                "Every arbitrary mixed tuple of unequal-pair physical irreps has an exact third-moment contraction "
                "over symmetric-group class connection coefficients, while equal-pair sectors require richer data."
            ),
            protocol=(
                "Derive the unequal projector-word character from the three relative permutation classes; compute "
                "their exact class-algebra connection kernel; validate against direct pair counts and all unequal "
                "W_3 threshold tuples; measure p(n)^3/p(n)^4 scaling; and search for a counterexample to determining "
                "the equal-pair commutator class from those three labels."
            ),
            positive_signal=(
                "An exact subfactorial contraction for all unequal tuples, together with a polynomial recoupling "
                "contraction that also handles equal-pair commutator insertions."
            ),
            falsifiers=[
                "Class connection counts disagree with direct permutation-pair counts.",
                "Unequal tuple moments disagree with the general character expansion.",
                "Subexponential p(n)^4 scaling is described as polynomial.",
                "Class triples are assumed to determine equal-pair commutator characters.",
                "All-unequal coverage is described as all physical-sector coverage.",
            ],
            metrics=[
                "failed_class_kernel_validation_count",
                "failed_unequal_moment_validation_count",
                "maximum_partition_count",
                "maximum_class_triple_count",
                "maximum_character_kernel_term_bound",
                "exact_subfactorial_class_contraction_count",
                "polynomial_in_n_all_unequal_contraction_count",
                "arbitrary_mixed_unequal_tuple_contraction_count",
                "equal_pair_commutator_contraction_count",
                "commutator_class_counterexample_count",
            ],
            dependencies=[
                "self_dual_wreath_all_unequal_third_moment.py",
                "symmetric_character.py",
                "symmetric-group class algebra",
                "wreath-product unequal irrep character formula",
            ],
            next_actions=[
                "Run qsearch.py code-self-dual-wreath-all-unequal.",
                "Build a commutator-character contraction using representation-ring and recoupling tensors.",
                "Determine whether stable/low-depth partition families admit polynomial contraction.",
                "Retain subexponential p(n)^4 cost as a blocker until removed.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-NATURAL-UNEQUAL-DOMINANCE",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Natural-source all-unequal wreath-sector reduction",
            status="planned",
            hypothesis=(
                "A natural physical wreath label is exactly two independent "
                "Plancherel draws, so equal-pair sectors are collisions and "
                "vanish across the information-threshold tuple."
            ),
            protocol=(
                "Derive equal and unequal physical weak-Fourier label masses, "
                "identify the equal mass with Plancherel collision probability, "
                "compute exact finite controls, and combine the union bound "
                "with the literature theorem for the maximal S_n irrep "
                "dimension."
            ),
            positive_signal=(
                "The probability of any equal-pair label among "
                "ceil(log2(n!)) natural samples tends to zero, reducing the "
                "asymptotic moment problem to arbitrary all-unequal tuples."
            ),
            falsifiers=[
                "The physical label masses do not equal the two-draw Plancherel law.",
                "Equal and unequal label masses do not sum to one.",
                "The maximal Plancherel atom theorem does not imply collision decay.",
                "The information-threshold copy count overwhelms the collision decay.",
                "A source-average reduction is described as worst-sector coverage.",
                "Fixed third moments are described as growing-order contraction.",
            ],
            metrics=[
                "failed_source_law_identity_count",
                "maximum_exact_control_n",
                "tail_equal_label_probability",
                ...
                "fixed_third_moment_all_unequal_contraction_count",
                "growing_order_all_unequal_contraction_count",
                "natural_average_inverse_polynomial_conclusive_theorem_count",
            ],
            dependencies=[
                "self_dual_wreath_natural_unequal_dominance.py",
                "self_dual_wreath_all_unequal_third_moment.py",
                "aggarwal-elboim-maximal-dimension-2026",
                "hook-length formula",
            ],
            next_actions=[
                "Run qsearch.py code-wreath-natural-unequal.",
                "Construct growing-order moment contractions for arbitrary all-unequal tuples.",
                "Prove a natural averaged conclusive bound after discarding the vanishing equal event.",
                "Keep maximal-effect dilation and hidden-permutation decoding as separate obligations.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-NATURAL-MOMENT-WORD-MAP",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="All-order natural wreath moment word-map reduction",
            status="planned",
            hypothesis=(
                "Natural source averaging and character column orthogonality "
                "reduce every normalized physical wreath-frame moment to "
                "identity and bridge-class subset word counts."
            ),
            protocol=(
                "Expand projector products at arbitrary moment order, average "
                "normalized characters under the exact natural irrep law, "
                "derive the identity/bridge-class indicator formula, validate "
                "every finite bridge sequence, and compare with complete W_3 "
                "spectral moments."
            ),
            positive_signal=(
                "A compressed recurrence computes the identity/bridge subset-"
                "word distribution at the required growing moment order and "
                "supports a concentration theorem for natural tuple success."
            ),
            falsifiers=[
                "Character column orthogonality disagrees with direct natural-irrep sums.",
                "The formula fails complete W_3 spectral moments.",
                "Physical irrep sums remain in the contracted expression.",
                "Explicit (n!)^m bridge-sequence enumeration is described as polynomial.",
                "A source-averaged identity is described as per-tuple concentration.",
                "The reduction is described as a coherent measurement circuit.",
            ],
            metrics=[
                "failed_character_sequence_count",
                "failed_w3_spectrum_validation_count",
                "natural_character_column_identity_theorem_count",
                "all_order_source_averaged_word_map_reduction_count",
                "physical_irrep_sum_eliminated_count",
                "tail_required_moment_order",
            ...
            dependencies=[
                "self_dual_wreath_natural_moment_word_map.py",
                "self_dual_wreath_natural_unequal_dominance.py",
                "self_dual_wreath_subpovm_moment_certificate.py",
                "finite-group character column orthogonality",
            ],
            next_actions=[
                "Run qsearch.py code-wreath-moment-word-map.",
                "Classify subset bridge words by surface-word topology and free reductions.",
                "Derive a cycle-index or character recurrence for identity and bridge-class word counts.",
                "Prove source-tuple concentration before inferring sub-POVM success.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-WORD-MAP-MIXING",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Natural subset-word lazy-walk mixing and concentration gap",
            status="planned",
            hypothesis=(
                "The one-label identity/bridge subset-word statistic mixes "
                "rapidly under the lazy bridge walk, while the actual kth "
                "moment requires a separate coupled-walk concentration theorem."
            ),
            protocol=(
                "Diagonalize the central lazy bridge walk in every physical "
                "irrep, identify the exact stationary sectors, prove a uniform "
                "nonstationary spectral bound, validate direct word counts, "
                "and quantify the Jensen-to-bounded kth-moment interval."
            ),
            positive_signal=(
                "A polynomial contraction of the shared-generator k-walk or a "
                "quenched concentration inequality closes the kth-moment "
                "interval at the required growing order."
            ),
            falsifiers=[
                "Direct subset-word means disagree with the physical-irrep spectrum.",
                "Additional stationary sectors invalidate the 2/(n!)^2 floor.",
                "A nonstationary eigenvalue exceeds 3/4.",
                "Mean mixing is represented as kth-moment concentration.",
                "Jensen and boundedness already give an inverse-polynomial success bound.",
                "A classical mixing theorem is represented as a measurement circuit.",
            ],
            metrics=[
                "failed_lazy_walk_validation_count",
                "exact_stationary_mean_theorem_count",
                "uniform_three_quarter_nonstationary_spectral_bound_count",
            ...
            dependencies=[
                "self_dual_wreath_word_map_mixing.py",
                "self_dual_wreath_natural_moment_word_map.py",
                "central random walks on finite groups",
                "physical wreath character ratios",
            ],
            next_actions=[
                "Run qsearch.py code-wreath-word-map-mixing.",
                "Construct the shared-generator k-walk transfer operator.",
                "Search diagonal-conjugacy or partition-algebra compression of its target matrix element.",
                "Prove a quenched upper tail for the identity/bridge subset-word statistic.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-COUPLED-WORD-WALK-GAP",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Shared-generator coupled wreath word-walk gap",
            status="planned",
            hypothesis=(
                "The shared-generator k-copy subset-word process retains a "
                "constant spectral gap, but its unconditional high moment is "
                "contaminated by rare equal one-dimensional source sectors."
            ),
            protocol=(
                "Represent the kth word statistic as an endpoint observable "
                "of M_k=E_c tensor_i(I+R_c)/2 on the bridge-generated subgroup, "
                "use projection domination to prove a uniform 1/4 gap, match "
                "finite exact word moments, and compare the resulting root "
                "bound with the second-moment eigenvalue scale."
            ),
            positive_signal=(
                "Conditioning the character kernel on all-unequal Plancherel "
                "labels removes the rare stationary sectors and yields a "
                "moment-root bound on the 2^-k frame-eigenvalue scale."
            ),
            falsifiers=[
                "The coupled Markov endpoint disagrees with exact subset-word moments.",
                "The bridge class fails to generate the expected index-two subgroup.",
                "The tensor projection inequality does not imply a constant gap.",
                "Constant-gap contraction reaches the typical frame scale without conditioning.",
                "Rare equal one-dimensional labels are ignored in the high-moment asymptotics.",
                "A classical spectral bound is described as a measurement circuit.",
            ],
            metrics=[
                "bridge_subgroup_validation_failure_count",
            ...
            dependencies=[
                "self_dual_wreath_coupled_word_walk_gap.py",
                "self_dual_wreath_natural_moment_word_map.py",
                "self_dual_wreath_natural_unequal_dominance.py",
                "spectral tensorization for averaged projections",
            ],
            next_actions=[
                "Run qsearch.py code-wreath-coupled-word-gap.",
                "Derive the normalized-character kernel conditioned on unequal partition pairs.",
                "Decompose the conditioned endpoint into irreducible bridge-walk sectors.",
                "Seek level-sensitive contraction at the 2^-k frame scale.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-ALL-UNEQUAL-CONDITIONED-KERNEL",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="All-unequal conditioned wreath character kernel",
            status="planned",
            hypothesis=(
                "Conditioning natural physical labels on unequal Plancherel "
                "partition pairs removes rare stationary sectors and exposes "
                "the exact tensor contraction needed at growing copy count."
            ),
            protocol=(
                "Subtract diagonal partition pairs from the two-draw "
                "Plancherel character average, derive the signed base-coset "
                "kernel and swap-coset annihilation formula, validate complete "
                "small wreath groups and bridge words, then prove the strongest "
                "single-coordinate projection bound without overclaiming it."
            ),
            positive_signal=(
                "A level-sensitive tensor theorem improves B<=I/2 to an "
                "operator bound on the 2^{1-k} scale for natural all-unequal "
                "tuples at the information-threshold copy count."
            ),
            falsifiers=[
                "The conditioned unequal source probabilities do not sum to one.",
                "The signed kernel disagrees with direct unequal-irrep character averages.",
                "Swap-coset words retain conditioned character mass.",
                "The annealed conditioned one-label moment differs from 2^-m.",
                "The B<=I/2 bound is described as a 2^{1-k} norm bound.",
                "A signed classical kernel is described as an implemented coherent measurement.",
            ],
            metrics=[
                "conditioned_kernel_validation_failure_count",
                "conditioned_word_validation_failure_count",
                "conditioned_annealed_half_power_failure_count",
                "typical_all_unequal_conditioned_kernel_count",
                "all_order_conditioned_word_map_reduction_count",
            ...
            dependencies=[
                "self_dual_wreath_all_unequal_conditioned_kernel.py",
                "self_dual_wreath_coupled_word_walk_gap.py",
                "self_dual_wreath_natural_unequal_dominance.py",
                "symmetric-group character orthogonality",
            ],
            next_actions=[
                "Run qsearch.py code-wreath-conditioned-kernel.",
                "Decompose the signed kernel by nontrivial-coordinate Fourier level.",
                "Search tensor-product projection inequalities that decay exponentially in active coordinates.",
                "Prove a high-probability all-unequal norm bound before designing a Naimark dilation.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-GLOBAL-PARTITION-COLLISION",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Global Plancherel collision-free source reduction",
            status="planned",
            hypothesis=(
                "At the information-threshold copy count, all 2k source "
                "partitions underlying natural physical labels are globally "
                "distinct with probability 1-o(1)."
            ),
            protocol=(
                "Unpack k physical labels into 2k iid Plancherel draws, apply "
                "the pair-collision union bound, combine it with maximal-atom "
                "decay, compute exact finite weighted occupancy controls, and "
                "classify known half-norm blocks by cross-label partition reuse."
            ),
            positive_signal=(
                "A uniform tensor contraction for arbitrary collision-free "
                "source-partition tuples reaches the 2^{1-k} frame scale and "
                "converts the vanishing collision event into a natural-source "
                "success theorem."
            ),
            falsifiers=[
                "Physical labels cannot be unpacked as 2k iid Plancherel draws.",
                "The global repeat probability exceeds binom(2k,2) times the collision probability.",
                "The maximal-atom theorem does not make k^2 C_n vanish.",
                "Known half-norm counterexamples are globally collision-free.",
                "Preasymptotic finite controls are described as asymptotic evidence.",
                "Global distinctness is described as a frame-norm theorem.",
            ],
            metrics=[
                "global_collision_control_count",
                "global_source_draw_as_iid_plancherel_theorem_count",
                "global_collision_union_bound_theorem_count",
                "asymptotic_global_all_distinct_dominance_theorem_count",
                "finite_ninety_percent_global_distinct_row_count",
                "repeated_source_half_norm_counterexample_count",
                "repeated_source_counterexample_exclusion_count",
                "collision_free_tuple_norm_theorem_count",
            ...
            next_actions=[
                "Run qsearch.py code-wreath-global-collision.",
                "Search collision-free mixed tuples in the smallest feasible wreath groups.",
                "Relate partition distinctness to tensor projector overlap or character-ratio decay.",
                "Prove a collision-free growing-moment contraction without catalog enumeration.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-COLLISION-FREE-FRAME-PROBE",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Collision-free mixed wreath frame spectral probe",
            status="planned",
            hypothesis=(
                "Globally distinct unequal source tuples have frame norm "
                "within a polynomial factor of 2^-k even though exact "
                "projector independence fails."
            ),
            protocol=(
                "Construct mixed unequal physical frame operators without "
                "forming dense Kronecker matrices, run deterministic Hermitian "
                "Lanczos on complete W4 collision-free pairings and an "
                "adversarial W5 three-copy portfolio, and verify every top "
                "eigenpair residual."
            ),
            positive_signal=(
                "All finite collision-free tuples remain within a small "
                "constant factor of 2^{1-k}, motivating an all-n "
                "poly(n)2^-k norm theorem based on character ratios or "
                "overlap-graph expansion."
            ),
            falsifiers=[
                "A collision-free tuple has top norm exponentially larger than 2^{1-k}.",
                "Lanczos eigenpair residuals are too large to support the comparison.",
                "The exact 2^{1-k} conjecture survives every adversarial W5 pairing.",
                "Only repeated physical labels are tested.",
                "A finite constant-factor trend is described as an all-n theorem.",
                "A frame spectrum is described as a coherent measurement or decoder.",
            ],
            metrics=[
                "collision_free_probe_count",
                "complete_w4_collision_free_pairing_count",
                "w5_collision_free_three_copy_probe_count",
                "maximum_probe_block_dimension",
                "maximum_top_eigenpair_residual",
                "w5_exact_two_to_one_minus_k_violation_count",
                "w5_factor_two_target_bound_success_count",
                "maximum_w5_top_to_target_ratio",
                "collision_free_polynomial_factor_norm_theorem_count",
                "collision_free_growing_moment_contraction_count",
            ],
            dependencies=[
                "self_dual_wreath_collision_free_frame_probe.py",
                "self_dual_wreath_global_partition_collision.py",
                "self_dual_wreath_unequal_frame_blocks.py",
                "scipy sparse Hermitian eigensolver",
            ],
            next_actions=[
                "Run qsearch.py code-wreath-collision-free-frame.",
                "Express the mixed frame norm through a bridge-overlap graph or Fourier-level transfer operator.",
                "Import sharp Plancherel-typical character-ratio bounds.",
                "Prove a poly(n)2^-k norm bound for arbitrary globally distinct source tuples.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-CHARACTER-RATIO-CONTRACT",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Collision-free character-ratio contraction contract",
            status="planned",
            hypothesis=(
                "Symmetric-group character-ratio decay, combined with joint "
                "anti-concentration of short bridge subset words, yields the "
                "sufficient collision-free norm bound poly(n)2^-k."
            ),
            protocol=(
                "Factor unequal physical normalized characters into ordinary "
                "S_n ratios, validate every small wreath element, stress-test "
                "finite Féray-Sniady constants, import sharp primary bounds, "
                "and formalize the typical-shape and short-word obligations "
                "needed by the trace-moment certificate."
            ),
            positive_signal=(
                "A proved short-word anti-concentration inequality, uniform "
                "over globally distinct typical source partitions, closes a "
                "poly(n)2^-k Schatten-moment bound."
            ),
            falsifiers=[
                "The unequal physical character does not factor into S_n ratios.",
                "Typical-shape tails do not hold simultaneously over 2k draws.",
                "Low-length subset words have superpolynomial aggregate weight.",
                "Character signs align on a collision-free family and defeat product mixing.",
                "A pointwise character bound is described as a moment theorem.",
                "A norm theorem is described as a coherent measurement or decoder.",
            ],
            metrics=[
                "character_factorization_failure_count",
                "character_ratio_stress_record_count",
                "maximum_finite_feray_sniady_constant_proxy",
                "literature_linked_character_bound_count",
                "exact_unequal_character_factorization_theorem_count",
                "simultaneous_typical_shape_tail_theorem_count",
                "joint_short_word_anticoncentration_theorem_count",
                "collision_free_polynomial_factor_norm_theorem_count",
                "collision_free_growing_moment_contraction_count",
            ],
            dependencies=[
                "self_dual_wreath_character_ratio_contract.py",
                "self_dual_wreath_collision_free_frame_probe.py",
            ...
            next_actions=[
                "Run qsearch.py code-wreath-character-ratios.",
                "Measure joint short-word transposition-length profiles under shared bridge sequences.",
                "Separate low-support exceptional words from typical product-mixing terms.",
                "Prove the resulting collision-free Schatten-moment inequality.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-SHORT-WORD-PROFILE",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Bridge subset short-word anti-concentration profile",
            status="planned",
            hypothesis=(
                "Fixed nonempty even subset bridge words have uniform "
                "symmetric-group marginals, and mask diagonals are negligible; "
                "the only remaining character-product obstruction is "
                "non-diagonal shared-generator correlation."
            ),
            protocol=(
                "Prove fixed-mask uniformity by Haar elimination, validate "
                "complete small bridge sequences, compute exact unsigned-"
                "Stirling transposition-length tails through n=64, and bound "
                "pairwise subset-mask collisions at the certificate order."
            ),
            positive_signal=(
                "A mask-incidence-rank or overlap-hypergraph theorem extends "
                "the exact marginals to joint product mixing for distinct "
                "masks and closes the character-ratio moment contract."
            ),
            falsifiers=[
                "A fixed nonempty even subset word has a nonuniform base marginal.",
                "Unsigned Stirling tails disagree with direct permutation counts.",
                "Subset-mask collisions remain nonnegligible at the target order.",
                "Distinct masks are assumed independent despite shared bridge generators.",
                "Marginal anti-concentration is described as a joint moment theorem.",
                "A classical word profile is described as a coherent measurement.",
            ],
            metrics=[
                "fixed_mask_uniformity_failure_count",
                "fixed_even_mask_uniform_marginal_theorem_count",
                "unsigned_stirling_short_length_formula_count",
                "tail_half_n_log2_short_probability",
                "pairwise_mask_distinct_scaling_row_count",
            ...
            dependencies=[
                "self_dual_wreath_short_word_profile.py",
                "self_dual_wreath_character_ratio_contract.py",
                "unsigned Stirling numbers of the first kind",
                "finite-group Haar invariance",
            ],
            next_actions=[
                "Run qsearch.py code-wreath-short-words.",
                "Classify joint word maps by binary mask incidence rank and overlap hypergraph.",
                "Prove product mixing for non-diagonal mask classes.",
                "Combine the joint tail with symmetric-group character bounds.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-MASK-HYPERGRAPH-REDUCTION",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Mask-incidence two-core character reduction",
            status="planned",
            hypothesis=(
                "Joint unequal-character products vanish whenever a bridge "
                "generator appears in only one subset word, reducing every "
                "nonzero contribution to a dense incidence-hypergraph two-core."
            ),
            protocol=(
                "Represent subset masks as a binary incidence matrix, prove "
                "private-column vanishing from the zero unequal bridge class "
                "sum, validate exact small cases, gauge-fix triangle word maps, "
                "and enumerate collision-free W5 perfect matchings."
            ),
            positive_signal=(
                "A cluster expansion or product-mixing theorem bounds the "
                "aggregate character weight of dense two-core incidence "
                "hypergraphs by poly(n) times the fully mixed frame scale."
            ),
            falsifiers=[
                "A mask family with a private column has nonzero joint unequal-character average.",
                "The unequal bridge class sum is not the zero operator.",
                "Global source distinctness forces every two-core correlation to vanish.",
                "Triangle gauge fixing changes the exact average.",
                "A two-core reduction is described as an anti-concentration theorem.",
                "A character average is described as a coherent measurement.",
            ],
            metrics=[
                "private_column_validation_failure_count",
                "private_column_joint_vanishing_theorem_count",
                "mask_hypergraph_two_core_reduction_count",
                "w5_collision_free_triangle_record_count",
                "w5_nonzero_collision_free_triangle_count",
            ...
            dependencies=[
                "self_dual_wreath_mask_hypergraph_reduction.py",
                "self_dual_wreath_short_word_profile.py",
                "self_dual_wreath_character_ratio_contract.py",
                "finite-group class-sum orthogonality",
            ],
            next_actions=[
                "Run qsearch.py code-wreath-mask-hypergraph.",
                "Enumerate incidence two-core isomorphism classes at fixed character order.",
                "Derive a cluster expansion in cycle rank or excess.",
                "Combine two-core weights with typical character-ratio decay.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-SUBGROUP-TWIRL-REDUCTION",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Collision-free subgroup-twirl isotypic reduction",
            status="planned",
            hypothesis=(
                "The complete dense mask expansion is the trace expansion of "
                "a subgroup twirl of one tensor projector, reducing the "
                "collision-free frame norm theorem exactly to normalized "
                "partial traces on diagonal-S_n multiplicity spaces."
            ),
            protocol=(
                "Prove h_s=x_s h_e x_s^-1, construct the tensor subgroup "
                "action, validate direct-frame/twirl equality on every "
                "collision-free W4 pairing, build central isotypic projectors, "
                "localize each top eigenvalue, and class-compress restriction "
                "multiplicities through the information threshold."
            ),
            positive_signal=(
                "A uniform recoupling or character estimate bounds every "
                "multiplicity-space partial trace by poly(n) d_nu 2^-k, or "
                "an explicit natural asymptotic sector violates that target."
            ),
            falsifiers=[
                "The bridge conjugacy identity fails in the induced physical representation.",
                "The direct frame differs from the subgroup twirl.",
                "Central isotypic projectors fail to resolve the restriction character multiplicities.",
                "The direct top eigenvalue is not localized by an isotypic block.",
                "Schur averaging alone is described as a partial-trace delocalization theorem.",
                "Finite W4 spectra are extrapolated to an all-n norm bound.",
                "A representation reduction is described as a coherent measurement or decoder.",
            ],
            metrics=[
                "complete_w4_collision_free_twirl_validation_count",
                "finite_twirl_validation_failure_count",
                "maximum_finite_twirl_validation_residual",
                "bridge_subgroup_conjugacy_identity_theorem_count",
            ...
            dependencies=[
                ...
                "self_dual_wreath_mask_hypergraph_reduction.py",
                "self_dual_wreath_collision_free_frame_probe.py",
                "self_dual_wreath_typical_recoupling_transfer.py",
                "symmetric-group character orthogonality",
                "Schur averaging over finite groups",
            ],
            next_actions=[
                "Run qsearch.py code-wreath-subgroup-twirl.",
                "Express the base projector partial trace in a coupling-tree multiplicity basis.",
                "Search finite isotypic sectors for stable high-n worst-case shape patterns.",
                "Apply typical character bounds to normalized multiplicity partial traces.",
                "Construct an asymptotic counterexample if uniform delocalization fails.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-ORIENTATION-FOURIER-REDUCTION",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Orientation-projector Fourier reduction",
            status="planned",
            hypothesis=(
                "The collision-free orbit Gram Fourier blocks are uniform "
                "averages of explicit orientation invariant-subspace "
                "projectors; their overlap geometry may yield the missing "
                "poly(n) 2^-k frame-norm bound."
            ),
            protocol=(
                "Prove the compressed unequal-label overlap formula, Fourier "
                "transform the orbit Gram into orientation projectors, "
                "validate every collision-free W4 pairing against the direct "
                "physical frame, and run an exact Boolean Kronecker-support "
                "dynamic program through information-threshold portfolios."
            ),
            positive_signal=(
                "A canonical-angle, fusion-frame, or recoupling estimate "
                "bounds the fully supported projector sum by poly(n), or an "
                "explicit natural asymptotic sector disproves that target."
            ),
            falsifiers=[
                "The physical compressed-overlap formula fails.",
                "The orbit Gram and direct physical frame have different nonzero spectra.",
                "The orientation terms are not orthogonal projectors.",
                "Sparse orientation support is assumed after exact support saturation.",
                "Finite W4 agreement is promoted to an all-n norm theorem.",
                "A representation reduction is described as a coherent measurement or decoder.",
            ],
            metrics=[
                "complete_w4_orientation_fourier_validation_count",
                "finite_orientation_fourier_validation_failure_count",
                "maximum_orbit_gram_spectral_residual",
                "compressed_overlap_formula_theorem_count",
            ...
            dependencies=[
                ...
                "self_dual_wreath_collision_free_frame_probe.py",
                "symmetric_character.py",
                "finite-group operator-valued Fourier analysis",
            ],
            next_actions=[
                "Run qsearch.py code-wreath-orientation-fourier.",
                "Derive exact pairwise canonical-angle or projector-product formulas.",
                "Identify stable asymptotic worst sectors for the projector sum.",
                "Prove a fusion-frame/recoupling norm bound or construct a natural counterexample.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-ORIENTATION-FUSION-MOMENT",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Orientation fusion-frame moment reduction",
            status="planned",
            hypothesis=(
                "Exact pairwise orientation-projector overlaps and the "
                "complete averaged second spectral moment can distinguish a "
                "fatal fusion-frame collision obstruction from a genuinely "
                "higher-moment operator-norm problem."
            ),
            protocol=(
                "Derive the character-convolution formula for Tr(E_a E_b), "
                "validate ranks, overlaps, and canonical angles on all "
                "collision-free W4 tuples, reconstruct symmetric-group class "
                "product counts, and evaluate Tr(F_nu) and Tr(F_nu^2) through "
                "the n=12 information threshold without orientation enumeration."
            ),
            positive_signal=(
                "The threshold collision lower bound remains polynomially "
                "close to 2^-k with large effective rank, and the formula "
                "extends to controlled growing moments or an operator-valued "
                "Gram contraction that upper-bounds the norm."
            ),
            falsifiers=[
                "The character-convolution overlap disagrees with exact projector matrices.",
                "Class-product reconstruction fails to reproduce exact W4 block moments.",
                "A superpolynomial collision-to-target ratio appears on natural threshold portfolios.",
                "A bounded second-moment collision scale is described as an operator-norm upper bound.",
                "Pairwise canonical angles are described as a coherent measurement or decoder.",
            ],
            metrics=[
                "complete_w4_fusion_tuple_validation_count",
                "pair_overlap_formula_validation_count",
                "finite_pair_overlap_validation_failure_count",
                "maximum_pair_overlap_formula_residual",
                "maximum_w4_nontrivial_canonical_correlation",
                "w4_common_range_pair_count",
                "pairwise_character_convolution_theorem_count",
                "orientation_average_class_algebra_second_moment_theorem_count",
                "maximum_scaling_collision_lower_bound_to_target_ratio",
                "tail_collision_lower_bound_to_target_ratio",
                "tail_maximum_log2_effective_rank",
                "second_moment_superquartic_obstruction_count",
            ...
            dependencies=[
                ...
                "symmetric_character.py",
                "symmetric-group class algebra",
                "finite fusion-frame spectral moments",
            ],
            next_actions=[
                "Run qsearch.py code-wreath-orientation-moments.",
                "Generalize the class-algebra contraction to third and fourth orientation moments.",
                "Search stable common-range sectors and high canonical-correlation motifs.",
                "Convert growing moment control into a uniform projector-sum norm theorem.",
            ],
        ),
        ExperimentRecord(
        ...
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-AFFINE-RELATION-WEIGHTED-BULK",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Exp Code Self Dual Wreath Affine Relation Weighted Bulk",
            status="planned",
            hypothesis="Proved that exact carrier-correlation weights overcome the divergent affine support pressure: the complete collision-free pair-relation Gram has only a vanishing coefficient-rank outlier sector, whose relation image is exactly signal-free.",
            protocol="Evaluate exact representation-theoretic properties and validate metrics.",
            positive_signal="Proved that exact carrier-correlation weights overcome the divergent affine support pressure: the complete collision-free pair-relation Gram has only a vanishing coefficient-rank outlier sector, whose relation image is exactly signal-free.",
            falsifiers=["Raw support overlap is not the correct spectral burden statistic.", "Non-rich orientation triples do not restore a macroscopic weighted burden.", "A global first-moment argument suffices for bulk rank but not for an untrimmed edge.", "Signal-free relation trimming does not by itself compile the full synthesis cokernel projector."],
            metrics=["exact_signature_support_star_burden_theorem_count", "signature_support_control_count", "signature_support_control_failure_count", "orientation_signature_count_control_count", "orientation_signature_count_failure_count", "finite_scaling_row_count", "finite_collision_free_bulk_certified_row_count", "finite_collision_free_bulk_onset_n", "tail_n", "tail_log2_conditioned_burden_to_capacity_ratio", "tail_conditional_markov_failure_upper_bound", "tail_spectral_outlier_rank_fraction_upper_bound", "global_coefficient_bulk_edge_theorem_count", "relation_trim_zero_pgm_state_loss_theorem_count", "residual_cokernel_output_error_theorem_count", "untrimmed_relation_edge_theorem_count", "new_quantum_algorithm_count"],
            dependencies=[
                "self_dual_wreath_affine_relation_weighted_bulk.py",
            ],
            next_actions=[
                "Run qsearch.py code-wreath-affine-relation-weighted-bulk.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-AFFINE-STAR-CHANNEL-GAP",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Exp Code Self Dual Wreath Affine Star Channel Gap",
            status="planned",
            hypothesis="Proved a width-independent graded gap above one quarter for affine-star carrier channels, exactly explaining every extracted W6 residual channel.",
            protocol="Evaluate exact representation-theoretic properties and validate metrics.",
            positive_signal="Proved a width-independent graded gap above one quarter for affine-star carrier channels, exactly explaining every extracted W6 residual channel.",
            falsifiers=["Complete affine closure is stronger than necessary; affine stars are already uniformly safe.", "Vertex affinity without edge-graph classification is insufficient.", "The remaining asymptotic question is whether natural channels stay as separate affine stars or merge into a dangerous two-sided graph."],
            metrics=["affine_star_exact_defect_theorem_count", "affine_star_uniform_quarter_gap_theorem_count", "finite_affine_star_control_count", "finite_affine_star_control_failure_count", "finite_uniform_bound_failure_count", "natural_width_scaling_row_count", "natural_width_quarter_gap_failure_count", "w6_carrier_nine_defect", "w6_carrier_five_defect", "tail_n", "tail_copy_count", "tail_grading_defect_norm", "tail_endpoint_gap", "natural_residual_channel_affine_star_classification_count", "new_quantum_algorithm_count"],
            dependencies=[
                "self_dual_wreath_affine_star_channel_gap.py",
            ],
            next_actions=[
                "Run qsearch.py code-wreath-affine-star-channel-gap.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-AUGMENTED-H0-DIMENSION-OBSTRUCTION",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Exp Code Self Dual Wreath Augmented H0 Dimension Obstruction",
            status="planned",
            hypothesis="Proved that pair-common relations cannot exhaust the natural leaf synthesis kernel: augmented H0 survives at minimal copies and is macroscopic with one extra copy, requiring recursive span relations.",
            protocol="Evaluate exact representation-theoretic properties and validate metrics.",
            positive_signal="Proved that pair-common relations cannot exhaust the natural leaf synthesis kernel: augmented H0 survives at minimal copies and is macroscopic with one extra copy, requiring recursive span relations.",
            falsifiers=["Dense pair-core graph support does not imply pair-generation of the synthesis kernel.", "The direct common-core Cech complex is asymptotically incomplete at H0.", "The graded pair-relation endpoint gap cannot by itself implement the full orientation polar."],
            metrics=["exact_pair_budget_expectation_theorem_count", "expectation_control_count", "expectation_control_failure_count", "two_adic_minimal_copy_gap_theorem_count", "asymptotic_minimal_copy_augmented_h0_obstruction_count", "asymptotic_extra_copy_macroscopic_h0_obstruction_count", "scaling_row_count", "finite_conditioned_certified_row_count", "tail_minimal_h0_relative_lower_bound", "tail_minimal_log2_conditioned_failure_upper_bound", "tail_extra_h0_relative_lower_bound", "tail_extra_log2_conditioned_failure_upper_bound", "hierarchical_span_dependency_resolver_count", "new_quantum_algorithm_count"],
            dependencies=[
                "self_dual_wreath_augmented_h0_dimension_obstruction.py",
            ],
            next_actions=[
                "Run qsearch.py code-wreath-augmented-h0-dimension-obstruction.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-CANONICAL-COEFFICIENT-AFFINE",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Exp Code Self Dual Wreath Canonical Coefficient Affine",
            status="planned",
            hypothesis="Moved the affine mechanism into minimum-norm coefficient space, where W3/W5 have scalar certificates but a globally distinct S6 affine node forces matrix-valued partial supports.",
            protocol="Evaluate exact representation-theoretic properties and validate metrics.",
            positive_signal="Moved the affine mechanism into minimum-norm coefficient space, where W3/W5 have scalar certificates but a globally distinct S6 affine node forces matrix-valued partial supports.",
            falsifiers=["Affine carrier-core exhaustion is the wrong necessary condition; canonical coefficient affinity survives emergent intersections.", "Balanced shorted metrics do not force affine coefficient support.", "External label distinctness does not by itself prove internal multiplicity-free recoupling; S6 already supplies a counterexample."],
            metrics=["canonical_minimum_norm_identity_theorem_count", "finite_control_count", "finite_validation_failure_count", "label_simple_control_count", "label_simple_fractional_merge_count", "label_simple_coefficient_affine_certificate_failure_count", "repeated_label_coefficient_affine_failure_count", "repeated_label_nonscalar_or_unequal_metric_count", "all_n_coefficient_affine_theorem_count", "natural_collision_free_scalar_affine_falsifier_count", "explicit_recoupling_synthesis_map_count", "coherent_coefficient_transfer_count", "hierarchical_orientation_polar_sampler_count", "new_quantum_algorithm_count"],
            dependencies=[
                "self_dual_wreath_canonical_coefficient_affine.py",
            ],
            next_actions=[
                "Run qsearch.py code-wreath-canonical-coefficient-affine.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-CAYLEY-FIBER-REDUCTION",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Exp Code Self Dual Wreath Cayley Fiber Reduction",
            status="planned",
            hypothesis="Proved that scalar XOR-translation-invariant coefficient fibers reduce every affine merge to Walsh character modes; all label-simple W3/W5 overlaps pass and use only carrier dimensions one or two, while repeated and scalar non-Cayley controls fail.",
            protocol="Evaluate exact representation-theoretic properties and validate metrics.",
            positive_signal="Proved that scalar XOR-translation-invariant coefficient fibers reduce every affine merge to Walsh character modes; all label-simple W3/W5 overlaps pass and use only carrier dimensions one or two, while repeated and scalar non-Cayley controls fail.",
            falsifiers=["Scalar coefficient effects without XOR translation invariance do not force half balance.", "The finite half-integral spectrum is explained by an abelian mask Gram, not by reducing carrier cores.", "An abelian mask normal form does not synthesize the physical representation fibers.", "Low-dimensional carriers in fixed W3/W5 controls do not establish low-dimensional carrier support asymptotically."],
            metrics=["scalar_cayley_walsh_theorem_count", "generic_cayley_control_count", "generic_noncayley_falsifier_count", "wreath_control_count", "label_simple_fractional_merge_count", "label_simple_scalar_cayley_failure_count", "label_simple_walsh_prediction_failure_count", "repeated_label_scalar_cayley_failure_count", "maximum_finite_candidate_pair_carrier_dimension", "all_n_scalar_cayley_fiber_theorem_count", "efficient_cayley_kernel_evaluator_count", "polynomial_active_carrier_bound_count", "coherent_physical_fiber_synthesis_count", "hierarchical_orientation_polar_sampler_count", "new_quantum_algorithm_count"],
            dependencies=[
                "self_dual_wreath_cayley_fiber_reduction.py",
            ],
            next_actions=[
                "Run qsearch.py code-wreath-cayley-fiber-reduction.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-CENTRAL-SUPPORT-RANK-BRIDGE",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Exp Code Self Dual Wreath Central Support Rank Bridge",
            status="planned",
            hypothesis="Quantified the decisive polynomial relative-rank bypass and proved that global diagonal symmetry cannot establish its premise.",
            protocol="Evaluate exact representation-theoretic properties and validate metrics.",
            positive_signal="Quantified the decisive polynomial relative-rank bypass and proved that global diagonal symmetry cannot establish its premise.",
            falsifiers=["Global diagonal covariance does not remove the central-support problem.", "Finite collision-free relative-rank data is not an asymptotic rank theorem.", "Without polynomial bad-projection rank, scalar spectral mass still need not control bad source blocks."],
            metrics=["relative_rank_central_support_bridge_theorem_count", "polynomial_rank_degree_reduction_theorem_count", "global_covariance_symmetry_no_go_theorem_count", "rank_bridge_control_count", "covariance_control_count", "finite_control_failure_count", "conditional_scaling_row_count", "tail_n", "tail_group_rank_exponent", "tail_conditional_upper_edge_degree", "tail_conditional_lower_edge_degree", "natural_polynomial_relative_rank_theorem_count", "center_valued_local_law_theorem_count", "natural_all_depth_frame_edge_theorem_count", "new_quantum_algorithm_count"],
            dependencies=[
                "self_dual_wreath_central_support_rank_bridge.py",
            ],
            next_actions=[
                "Run qsearch.py code-wreath-central-support-rank-bridge.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-COHERENT-FOURIER-DECODER",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Exp Code Self Dual Wreath Coherent Fourier Decoder",
            status="planned",
            hypothesis="Derived and validated the exact cross-sector Fourier decoding criterion, reducing hidden-permutation output to coherent sector reweighting and row-multiplicity entanglement normalization before an efficient inverse S_n QFT.",
            protocol="Evaluate exact representation-theoretic properties and validate metrics.",
            positive_signal="Derived and validated the exact cross-sector Fourier decoding criterion, reducing hidden-permutation output to coherent sector reweighting and row-multiplicity entanglement normalization before an efficient inverse S_n QFT.",
            falsifiers=["Independent sector decoding is unnecessary in the ideal case and impossible at constant success after dephasing; coherent matrix-coefficient synthesis is the correct target.", "An n!-outcome table is not intrinsically required: inverse S_n Fourier synthesis outputs the permutation in O(n log n) bits.", "Plancherel sector weights alone are insufficient; row-column Schmidt flatness enters multiplicatively in the exact amplitude.", "No speedup follows until physical carrier extraction, flattening, reweighting, and classical separation are proved."],
            metrics=["matrix_coefficient_fourier_basis_theorem_count", "coherent_fourier_success_formula_theorem_count", "perfect_plancherel_maximal_entanglement_decoder_theorem_count", "finite_control_count", "finite_control_failure_count", "ideal_control_success_probability", "minimum_mismatch_control_success_probability", "scaling_record_count", "efficient_inverse_symmetric_group_qft_count", "physical_sector_weight_match_theorem_count", "coherent_dual_row_extraction_count", "sector_schmidt_flattening_count", "polynomial_sector_reweighting_count", "end_to_end_fourier_decoder_count", "new_quantum_algorithm_count"],
            dependencies=[
                "self_dual_wreath_coherent_fourier_decoder.py",
            ],
            next_actions=[
                "Run qsearch.py code-wreath-coherent-fourier-decoder.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-COLLISION-FREE-EVENT-TRANSFER",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Exp Code Self Dual Wreath Collision Free Event Transfer",
            status="planned",
            hypothesis="Replaced mandatory injective moment transfer by an exact event-level conditioning route and quantified the full hierarchy union cost.",
            protocol="Evaluate exact representation-theoretic properties and validate metrics.",
            positive_signal="Replaced mandatory injective moment transfer by an exact event-level conditioning route and quantified the full hierarchy union cost.",
            falsifiers=["Injective observable contraction is not mandatory for conditioning a pre-proved good event.", "Per-node o(1) tails are insufficient across an exponential orientation hierarchy.", "Finite collision-free mass cannot be replaced by its asymptotic limit in numerical rows."],
            metrics=["event_level_conditioning_transfer_theorem_count", "hierarchy_union_bound_theorem_count", "generic_control_count", "generic_control_failure_count", "hierarchy_scaling_row_count", "tail_n", "tail_log2_global_distinct_probability", "tail_log2_required_per_node_failure", "injective_moment_transfer_required_for_event_conditioning", "independent_uniform_spectral_event_theorem_count", "new_quantum_algorithm_count"],
            dependencies=[
                "self_dual_wreath_collision_free_event_transfer.py",
            ],
            next_actions=[
                "Run qsearch.py code-wreath-collision-free-event-transfer.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-COMMON-CORE-POLAR-BYPASS",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Exp Code Self Dual Wreath Common Core Polar Bypass",
            status="planned",
            hypothesis="Proved that every exponential block-common-core witness is half-balanced at each aligned polar-tree level. This defeats the recurring-core argument as a no-go for hierarchical sampling; quotient-coset intersections are now the next obstruction target.",
            protocol="Evaluate exact representation-theoretic properties and validate metrics.",
            positive_signal="Proved that every exponential block-common-core witness is half-balanced at each aligned polar-tree level. This defeats the recurring-core argument as a no-go for hierarchical sampling; quotient-coset intersections are now the next obstruction target.",
            falsifiers=["Do not use the exponential block-common-core eigenvalue as a lower bound on aligned hierarchical-polar cost.", "Do not carry local rejection-filter incidence no-gos over to a sampler that retains common cores and normalizes relative counts.", "Do not claim a full bypass until quotient-coset intersections and all non-block overlap channels are classified."],
            metrics=["balanced_common_core_polar_bypass_theorem_count", "finite_noncommuting_common_core_control_count", "finite_validation_failure_count", "maximum_finite_tree_depth", "maximum_common_core_half_balance_residual", "block_core_scaling_record_count", "block_core_witness_bypassed_count", "tail_n", "tail_block_count", "tail_common_orientation_family_size", "tail_projector_sum_spike_lower_bound", "tail_balanced_polar_tree_depth", "quotient_coset_classification_theorem_count", "global_nonorthogonal_sampler_count", "polynomial_hidden_permutation_decoder_count", "new_quantum_algorithm_count"],
            dependencies=[
                "self_dual_wreath_common_core_polar_bypass.py",
            ],
            next_actions=[
                "Run qsearch.py code-wreath-common-core-polar-bypass.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-COMPLETE-S6-VERTEX-CHANNEL-AUDIT",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Exp Code Self Dual Wreath Complete S6 Vertex Channel Audit",
            status="planned",
            hypothesis="Exhausted 1,155 globally distinct S6 portfolios: all sixteen nonorthogonal high-degree vertex channels are positive flat affine triangles.",
            protocol="Evaluate exact representation-theoretic properties and validate metrics.",
            positive_signal="Exhausted 1,155 globally distinct S6 portfolios: all sixteen nonorthogonal high-degree vertex channels are positive flat affine triangles.",
            falsifiers=["The three previously selected W6 controls were insufficient evidence; the complete finite boundary is now explicit.", "No larger, overlapping, noncommuting, or negative-holonomy channel occurs in the stated low-dimensional S6 portfolio.", "The result does not exclude such channels once higher multiplicity source irreps or larger n are admitted."],
            metrics=["source_partition_count", "source_matching_count", "target_count", "physical_portfolio_count", "degree_at_least_three_vertex_count", "nonorthogonal_vertex_count", "nontrivial_component_count", "three_edge_clique_component_count", "affine_plane_component_count", "flat_groupoid_failure_count", "maximum_partial_isometry_residual", "maximum_support_commutator_residual", "maximum_path_residual", "maximum_holonomy_residual", "all_n_affine_triangle_channel_theorem_count", "new_quantum_algorithm_count"],
            dependencies=[
                "self_dual_wreath_complete_s6_vertex_channel_audit.py",
            ],
            next_actions=[
                "Run qsearch.py code-wreath-complete-s6-vertex-channel-audit.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-COMPONENT-DEFECT-GAP-BRIDGE",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Exp Code Self Dual Wreath Component Defect Gap Bridge",
            status="planned",
            hypothesis="Proved that a uniform positive component edge and coordinate-block rank accounting convert the entire many-outcome POVM into a full-rank aggregate nonscalarity defect without assuming trace balance, linking the Jacobi edge target directly to the regular-master central-support problem.",
            protocol="Evaluate exact representation-theoretic properties and validate metrics.",
            positive_signal="Proved that a uniform positive component edge and coordinate-block rank accounting convert the entire many-outcome POVM into a full-rank aggregate nonscalarity defect without assuming trace balance, linking the Jacobi edge target directly to the regular-master central-support problem.",
            falsifiers=["Many tiny-trace outcomes do not dilute the aggregate defect when their positive edges stay bounded.", "Coordinate block rank accounting removes the need to concentrate every component trace separately.", "An average edge or small trace is not a substitute for a uniform retained positive edge.", "The Haar/Jacobi prediction is not a natural Plancherel/Racah edge theorem.", "A defect gap is a mass-analysis bridge, not a component-POVM circuit."],
            metrics=["positive_edge_to_full_rank_defect_bridge_theorem_count", "balanced_trace_defect_gap_corollary_count", "coordinate_block_trace_free_defect_gap_corollary_count", "finite_control_count", "finite_control_failure_count", "positive_gap_control_count", "failed_premise_control_count", "jacobi_scaling_record_count", "tail_minimum_predicted_defect_gap", "tail_minimum_coordinate_cauchy_defect_gap", "natural_positive_component_edge_theorem_count", "natural_block_to_fiber_aspect_theorem_count", "natural_component_defect_central_support_bound_count", "recursive_orientation_polar_sampler_count", "new_quantum_algorithm_count"],
            dependencies=[
                "self_dual_wreath_component_defect_gap_bridge.py",
            ],
            next_actions=[
                "Run qsearch.py code-wreath-component-defect-gap-bridge.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-COMPONENT-POVM-REGULAR-MASTER-REDUCTION",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Exp Code Self Dual Wreath Component Povm Regular Master Reduction",
            status="planned",
            hypothesis="Lifted canonical matrix component effects to the regular master and separated source probability, physical common-span mass, and ordinary defect trace, exposing a center-valued local law as the next natural theorem.",
            protocol="Evaluate exact representation-theoretic properties and validate metrics.",
            positive_signal="Lifted canonical matrix component effects to the regular master and separated source probability, physical common-span mass, and ordinary defect trace, exposing a center-valued local law as the next natural theorem.",
            falsifiers=["Low-dimensional source exclusion does not exclude low-dimensional internal carriers.", "Scalar defect moments do not control the probability of a bad source block without relative-rank information.", "The regular-master reduction is an analysis tool, not a component-POVM circuit.", "The finite repeated-source control is not collision-free asymptotic evidence."],
            metrics=["canonical_full_domain_component_effect_theorem_count", "regular_master_component_defect_reduction_theorem_count", "central_support_mass_observable_theorem_count", "finite_source_block_count", "finite_matrix_effect_source_block_count", "finite_globally_distinct_matrix_effect_source_block_count", "finite_matrix_source_to_common_mass_ratio", "finite_matrix_common_to_scalar_defect_mass_ratio", "scalar_moment_dilution_control_count", "high_dimension_central_support_bound_count", "natural_component_support_select_circuit_count", "recursive_orientation_polar_sampler_count", "new_quantum_algorithm_count"],
            dependencies=[
                "self_dual_wreath_component_povm_regular_master_reduction.py",
            ],
            next_actions=[
                "Run qsearch.py code-wreath-component-povm-regular-master-reduction.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-COMPONENT-POVM-SPARSE-SUPPORT-BOUNDARY",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Exp Code Self Dual Wreath Component Povm Sparse Support Boundary",
            status="planned",
            hypothesis="Proved that rare component outcomes need not have small positive effect eigenvalues and identified sparse support-projector SELECT, not generic square-root approximation, as the relevant Haar-model compiler bottleneck.",
            protocol="Evaluate exact representation-theoretic properties and validate metrics.",
            positive_signal="Proved that rare component outcomes need not have small positive effect eigenvalues and identified sparse support-projector SELECT, not generic square-root approximation, as the relevant Haar-model compiler bottleneck.",
            falsifiers=["Small component trace is not evidence for an exponentially small positive effect edge.", "Many outcomes do not by themselves activate the generic square-root polynomial lower bound.", "The Haar sparse-support law is a benchmark, not a natural wreath-frame theorem.", "Asymptotic support-scalarity would not remove the coherent support-SELECT and holonomy problems."],
            metrics=["exact_component_projection_geometry_theorem_count", "finite_control_count", "finite_control_failure_count", "sparse_outcome_jacobi_limit_theorem_count", "sparse_scaling_record_count", "tail_minimum_positive_edge", "tail_maximum_support_rank_fraction", "tail_maximum_edge_deviation_from_fiber_aspect", "natural_component_jacobi_universality_theorem_count", "natural_component_support_select_circuit_count", "recursive_orientation_polar_sampler_count", "new_quantum_algorithm_count"],
            dependencies=[
                "self_dual_wreath_component_povm_sparse_support_boundary.py",
            ],
            next_actions=[
                "Run qsearch.py code-wreath-component-povm-sparse-support-boundary.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-COVARIANT-PGM-FACTORIZATION",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Exp Code Self Dual Wreath Covariant Pgm Factorization",
            status="planned",
            hypothesis="Derived and validated the exact Fourier-domain PGM Stinespring factorization, cancelling explicit factorial environment normalization.",
            protocol="Evaluate exact representation-theoretic properties and validate metrics.",
            positive_signal="Derived and validated the exact Fourier-domain PGM Stinespring factorization, cancelling explicit factorial environment normalization.",
            falsifiers=["The sqrt(n!) dimension charge in the generic Petz implementation is not intrinsic to the group-covariant PGM.", "The physical mixed decoder is a dual-row-indexed tight-frame isometry, not a single pure Fourier state.", "After exact covariance compression, multiplicity inversion is the sole nontrivial state-dependent linear-algebra operation."],
            metrics=["covariance_compressed_pgm_factorization_theorem_count", "group_size_normalization_cancellation_theorem_count", "projector_rank_normalization_cancellation_theorem_count", "finite_control_count", "finite_control_failure_count", "scaling_record_count", "maximum_removed_generic_sqrt_group_log2_charge", "controlled_multiplicity_inverse_block_encoding_count", "polynomial_covariant_pgm_circuit_count", "new_quantum_algorithm_count"],
            dependencies=[
                "self_dual_wreath_covariant_pgm_factorization.py",
            ],
            next_actions=[
                "Run qsearch.py code-wreath-covariant-pgm-factorization.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-COVERAGE-WELCH-PRESSURE",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Exp Code Self Dual Wreath Coverage Welch Pressure",
            status="planned",
            hypothesis="Proved that divergent support pressure forces divergent aggregate inter-plane overlap and isolated the normalized excess fusion potential as the exact natural coverage statistic.",
            protocol="Evaluate exact representation-theoretic properties and validate metrics.",
            positive_signal="Proved that divergent support pressure forces divergent aggregate inter-plane overlap and isolated the normalized excess fusion potential as the exact natural coverage statistic.",
            falsifiers=["Distinct-plane overlaps cannot remain factorially weak once their total rank demand exceeds pair-core capacity.", "Neither large support demand nor large raw overlap certifies a coverage edge.", "The next natural calculation must be a four-orientation projector second moment and report excess above the Welch floor."],
            metrics=["finite_control_count", "finite_control_failure_count", "tight_control_excess", "aligned_control_excess", "tail_n", "tail_forced_overlap_over_trace_log2_lower_bound", "fusion_welch_pressure_theorem_count", "coverage_tail_criterion_theorem_count", "natural_tight_coverage_theorem_count", "new_quantum_algorithm_count"],
            dependencies=[
                "self_dual_wreath_coverage_welch_pressure.py",
            ],
            next_actions=[
                "Run qsearch.py code-wreath-coverage-welch-pressure.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-CROSS-DEPENDENCY-NEUTRALITY",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Exp Code Self Dual Wreath Cross Dependency Neutrality",
            status="planned",
            hypothesis="Replaced compressed frame inversion by an exact cross-synthesis dependency quotient; all label-simple finite dependencies are neutral, while repeated labels and a generic equal-dimension control exhibit nonneutral defects.",
            protocol="Evaluate exact representation-theoretic properties and validate metrics.",
            positive_signal="Replaced compressed frame inversion by an exact cross-synthesis dependency quotient; all label-simple finite dependencies are neutral, while repeated labels and a generic equal-dimension control exhibit nonneutral defects.",
            falsifiers=["Equal child dimensions do not imply equal shorted metrics.", "A child-range intersection is not characterized by the raw cross-kernel until internal synthesis kernels are quotiented out.", "The kernel formulation is not a circuit without a sparse representation-theoretic dependency decomposition."],
            metrics=["exact_cross_dependency_theorem_count", "generic_control_count", "wreath_control_count", "finite_cross_dependency_record_count", "finite_theorem_validation_failure_count", "label_simple_nonneutral_dependency_count", "repeated_label_nonneutral_dependency_count", "symbolic_representation_dependency_decomposition_count", "all_n_neutrality_theorem_count", "coherent_dependency_transform_count", "new_quantum_algorithm_count"],
            dependencies=[
                "self_dual_wreath_cross_dependency_neutrality.py",
            ],
            next_actions=[
                "Run qsearch.py code-wreath-cross-dependency-neutrality.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-DEPENDENCY-HOMOLOGY",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Exp Code Self Dual Wreath Dependency Homology",
            status="planned",
            hypothesis="Constructed the pair-common quotient of cross dependencies: W3 has genuine neutral emergent homology, while a globally distinct S6 affine plane has an entirely pair-generated nonneutral 34-dimensional intersection.",
            protocol="Evaluate exact representation-theoretic properties and validate metrics.",
            positive_signal="Constructed the pair-common quotient of cross dependencies: W3 has genuine neutral emergent homology, while a globally distinct S6 affine plane has an entirely pair-generated nonneutral 34-dimensional intersection.",
            falsifiers=["Exact pair-common ranges do not exhaust every finite child intersection.", "A raw pair-edge count ignores linear dependencies and overlap among pair classes.", "Pair generation must be separated from grading neutrality.", "Collision-free source labels do not restore universal pair-class half-balance.", "A finite dependency quotient is not a coherent all-n recoupling transform."],
            metrics=["dependency_homology_quotient_count", "finite_control_count", "finite_homology_audit_failure_count", "finite_fractional_merge_count", "finite_pair_generated_merge_count", "finite_emergent_merge_count", "w3_distinct_maximum_emergent_homology_dimension", "w5_pair_generated_merge_count", "w6_pair_generated_merge_count", "repeated_label_nonneutral_merge_count", "collision_free_nonneutral_pair_generated_merge_count", "collision_free_nonneutral_cross_dependency_dimension", "collision_free_nonneutral_grading_residual", "all_n_pair_common_chain_complex_count", "all_n_emergent_homology_vanishing_or_neutrality_count", "coherent_pair_common_quotient_count", "new_quantum_algorithm_count"],
            dependencies=[
                "self_dual_wreath_dependency_homology.py",
            ],
            next_actions=[
                "Run qsearch.py code-wreath-dependency-homology.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-EARLY-LEVEL-OVERLAP-LOCALIZATION",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Exp Code Self Dual Wreath Early Level Overlap Localization",
            status="planned",
            hypothesis="Proved that common-free orientation families cannot develop child-span intersections while each child has fewer than n/2 leaves. This localizes early fractional channels to exact common ranges; linear-width overlap and a coherent common-range resolver remain open.",
            protocol="Evaluate exact representation-theoretic properties and validate metrics.",
            positive_signal="Proved that common-free orientation families cannot develop child-span intersections while each child has fewer than n/2 leaves. This localizes early fractional channels to exact common ranges; linear-width overlap and a coherent common-range resolver remain open.",
            falsifiers=["Do not search for emergent noncommon intersections at constant or sublinear child width before resolving exact common ranges.", "Do not extrapolate the coherence bound beyond m<n/2.", "Do not call early-level localization a full sampler without a coherent common-range decomposition and large-width theorem."],
            metrics=["common_free_early_overlap_exclusion_theorem_count", "block_gram_span_correlation_bound_theorem_count", "synthetic_control_count", "synthetic_validation_failure_count", "w5_control_count", "w5_applicable_common_free_control_count", "w5_applicable_validation_failure_count", "w5_exact_common_range_control_count", "tail_n", "tail_maximum_certified_child_leaf_count", "tail_certified_common_free_merge_level_count", "tail_total_orientation_tree_depth", "exact_common_range_resolver_count", "linear_width_overlap_theorem_count", "global_nonorthogonal_sampler_count", "polynomial_hidden_permutation_decoder_count", "new_quantum_algorithm_count"],
            dependencies=[
                "self_dual_wreath_early_level_overlap_localization.py",
            ],
            next_actions=[
                "Run qsearch.py code-wreath-early-level-overlap-localization.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-EXTENDED-KRONECKER-THRESHOLD",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Exp Code Self Dual Wreath Extended Kronecker Threshold",
            status="planned",
            hypothesis="Proved normalized fixed-target multiplicity concentration for three or more independent Plancherel irreps.",
            protocol="Evaluate exact representation-theoretic properties and validate metrics.",
            positive_signal="Proved normalized fixed-target multiplicity concentration for three or more independent Plancherel irreps.",
            falsifiers=["The independent pointwise support transition occurs at three factors, not an unknown larger covering radius.", "Pointwise target-uniform bounds do not imply simultaneous covering of all irreps.", "Pointwise multiplicity concentration does not provide simultaneous incidence or frame-edge control."],
            metrics=["exact_fixed_target_variance_theorem_count", "three_factor_fixed_target_positivity_theorem_count", "hamming_three_support_transition_theorem_count", "finite_control_count", "finite_control_failure_count", "n50_q4_worst_target_variance_upper_bound", "simultaneous_all_target_covering_theorem_count", "fixed_target_normalized_multiplicity_concentration_theorem_count", "fixed_hamming_pair_common_rank_concentration_theorem_count", "natural_node_frame_edge_theorem_count", "new_quantum_algorithm_count"],
            dependencies=[
                "self_dual_wreath_extended_kronecker_threshold.py",
            ],
            next_actions=[
                "Run qsearch.py code-wreath-extended-kronecker-threshold.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-FINAL-ROOT-LEVERAGE-EDGE",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Exp Code Self Dual Wreath Final Root Leverage Edge",
            status="planned",
            hypothesis="Showed that the existing K+2 schedule prevents a vanishing scalar-surrogate final-root component aspect and proved an all-coordinate constant edge plus aggregate defect gap in the Haar model.",
            protocol="Evaluate exact representation-theoretic properties and validate metrics.",
            positive_signal="Showed that the existing K+2 schedule prevents a vanishing scalar-surrogate final-root component aspect and proved an all-coordinate constant edge plus aggregate defect gap in the Haar model.",
            falsifiers=["The existing K+2 final root does not suffer a vanishing common-fiber aspect in the surrogate.", "Marginal mean leverage alone is insufficient; the uniform result needs the beta large-deviation exponent.", "Haar rank-one leverage concentration is not a natural wreath-frame theorem.", "A final-root surrogate edge does not resolve earlier relation nodes or compile the PGM."],
            metrics=["k_plus_two_surrogate_final_root_component_aspect_theorem_count", "haar_uniform_rank_one_leverage_edge_theorem_count", "haar_edge_to_defect_gap_bridge_count", "finite_control_count", "finite_control_failure_count", "finite_half_mean_edge_miss_count", "scaling_record_count", "positive_finite_scaling_defect_gap_row_count", "tail_n", "tail_common_fiber_aspect", "tail_log2_union_edge_failure_upper_bound", "tail_coordinate_defect_gap_lower_bound", "natural_final_root_leverage_edge_theorem_count", "all_relation_node_component_edge_theorem_count", "natural_component_support_select_circuit_count", "new_quantum_algorithm_count"],
            dependencies=[
                "self_dual_wreath_final_root_leverage_edge.py",
            ],
            next_actions=[
                "Run qsearch.py code-wreath-final-root-leverage-edge.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-COMPONENT-DEFECT-RANK-MASS",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Exp Code Self Dual Wreath Component Defect Rank Mass",
            status="planned",
            hypothesis="Proved that final-root component nonscalarity occupies asymptotically full common-fiber rank on constant natural source mass, without a positive component-edge premise.",
            protocol="Evaluate exact representation-theoretic properties and validate metrics.",
            positive_signal="Proved that final-root component nonscalarity occupies asymptotically full common-fiber rank on constant natural source mass, without a positive component-edge premise.",
            falsifiers=["Ordinary scalar defect trace is not needed to control center-valued defect support.", "A natural positive component edge is not required to prove positive nonscalarity source or physical mass.", "Asymptotically full defect rank does not imply a usable spectral gap.", "Positive-mass component nonscalarity is not itself an MRS lower-bound separation."],
            metrics=["component_defect_relative_rank_bridge_theorem_count", "finite_control_count", "finite_control_failure_count", "finite_nontrivial_defect_kernel_control_count", "natural_component_defect_positive_source_mass_theorem_count", "natural_component_defect_asymptotically_full_common_rank_theorem_count", "asymptotic_conditioned_source_event_mass_lower_bound", "asymptotic_conditioned_physical_defect_support_mass_lower_bound", "tail_component_count", "tail_defect_relative_common_rank_lower_bound", "natural_positive_component_edge_theorem_count", "physical_mrs_separation_theorem_count", "new_quantum_algorithm_count"],
            dependencies=[
                "self_dual_wreath_component_defect_rank_mass.py",
            ],
            next_actions=[
                "Run qsearch.py code-wreath-component-defect-rank-mass.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-COMPONENT-EFFECT-ALGEBRA-BOUNDARY",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Exp Code Self Dual Wreath Component Effect Algebra Boundary",
            status="planned",
            hypothesis="Separated full-rank component nonscalarity from genuinely noncommutative effect-algebra structure and identified the exact regular-master commutator-support target.",
            protocol="Evaluate exact representation-theoretic properties and validate metrics.",
            positive_signal="Separated full-rank component nonscalarity from genuinely noncommutative effect-algebra structure and identified the exact regular-master commutator-support target.",
            falsifiers=["Full-rank component nonscalarity does not imply a noncommutative effect algebra.", "Positive natural support mass of D_ns is not positive natural support mass of commutators.", "Finite low-dimensional noncommutativity is not an asymptotic natural-mass theorem.", "A commuting effect algebra may be algorithmically favorable if its joint eigenbasis is coherently accessible."],
            metrics=["exact_component_commutator_criterion_theorem_count", "finite_control_count", "finite_control_failure_count", "full_rank_nonscalarity_commuting_counterexample_count", "noncommutative_full_matrix_algebra_control_count", "natural_nonscalarity_positive_mass_theorem_input_count", "natural_noncommutative_effect_algebra_source_mass_theorem_count", "natural_physical_noncommutative_support_mass_theorem_count", "new_quantum_algorithm_count"],
            dependencies=[
                "self_dual_wreath_component_effect_algebra_boundary.py",
            ],
            next_actions=[
                "Run qsearch.py code-wreath-component-effect-algebra-boundary.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-COMPONENT-POVM-SPECTRAL-TRIM",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Exp Code Self Dual Wreath Component Povm Spectral Trim",
            status="planned",
            hypothesis="Proved a state-weighted spectral sub-POVM trim whose cutoff is inverse polynomial under polynomial natural input flatness, and exhibited a concentrated-state countercontrol.",
            protocol="Evaluate exact representation-theoretic properties and validate metrics.",
            positive_signal="Proved a state-weighted spectral sub-POVM trim whose cutoff is inverse polynomial under polynomial natural input flatness, and exhibited a concentrated-state countercontrol.",
            falsifiers=["A natural uniform positive component edge is not necessary for average-state component dilation.", "Uniform-input trace retention does not imply worst-case state retention.", "Natural constant component aspect relocates the gate to incoming-state flatness and coherent effect access.", "Inverse-polynomial spectral threshold does not itself compile component support SELECT."],
            metrics=["state_weighted_component_spectral_trim_theorem_count", "finite_control_count", "finite_control_failure_count", "concentrated_state_unit_failure_countercontrol_count", "natural_final_fiber_aspect_input_count", "conditional_inverse_polynomial_trim_scaling_row_count", "tail_n", "tail_retained_component_threshold", "tail_inverse_threshold_cost", "natural_incoming_state_flatness_theorem_count", "coherent_component_threshold_filter_count", "new_quantum_algorithm_count"],
            dependencies=[
                "self_dual_wreath_component_povm_spectral_trim.py",
            ],
            next_actions=[
                "Run qsearch.py code-wreath-component-povm-spectral-trim.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-FINAL-ROOT-NATURAL-COMMON-SPAN",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Exp Code Self Dual Wreath Final Root Natural Common Span",
            status="planned",
            hypothesis="Proved a constant relative final sibling common span on positive globally distinct natural mass and propagated it to constant component fiber aspect with O(1/q) block ratio.",
            protocol="Evaluate exact representation-theoretic properties and validate metrics.",
            positive_signal="Proved a constant relative final sibling common span on positive globally distinct natural mass and propagated it to constant component fiber aspect with O(1/q) block ratio.",
            falsifiers=["The target carrier dimension does not spoil the normalized child aspect; its factor cancels exactly.", "Full final-child support is stronger than necessary for component geometry.", "Fixed second moments cannot prove a spectral edge, but they do prove positive-mass support rank through Cauchy and Markov.", "Natural block/fiber aspect control does not imply a natural positive component eigenvalue edge."],
            metrics=["deterministic_common_span_rank_bridge_theorem_count", "deterministic_control_count", "deterministic_control_failure_count", "natural_positive_mass_final_common_span_theorem_count", "natural_final_component_aspect_theorem_count", "asymptotic_conditioned_event_mass_lower_bound", "asymptotic_common_span_relative_rank_lower_bound", "asymptotic_common_fiber_aspect_lower_bound", "asymptotic_component_block_ratio_coefficient", "conditional_half_relative_edge_defect_gap_lower_bound", "conditioned_uniform_rank_certified_finite_row_count", "finite_positive_common_span_bound_row_count", "natural_final_component_positive_edge_theorem_count", "natural_component_support_select_circuit_count", "new_quantum_algorithm_count"],
            dependencies=[
                "self_dual_wreath_final_root_natural_common_span.py",
            ],
            next_actions=[
                "Run qsearch.py code-wreath-final-root-natural-common-span.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-FIXED-FAMILY-COMMON-RANK-DILUTION",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Exp Code Self Dual Wreath Fixed Family Common Rank Dilution",
            status="planned",
            hypothesis="Proved an exact support-versus-rank dilution law for every fixed balanced orientation family.",
            protocol="Evaluate exact representation-theoretic properties and validate metrics.",
            positive_signal="Proved an exact support-versus-rank dilution law for every fixed balanced orientation family.",
            falsifiers=["Common-support probability and scalar relative rank can differ by factorial powers.", "A dense fixed-order intersection nerve is not a spectral-edge theorem.", "Fixed-family intersections are not automatically eigenspaces of the full node frame."],
            metrics=["fixed_family_exact_expectation_theorem_count", "fixed_family_support_covering_theorem_count", "fixed_family_relative_rank_concentration_theorem_count", "finite_control_count", "finite_control_failure_count", "maximum_family_size_scaling_record", "n48_m4_expected_relative_rank_log2", "full_node_bad_projection_dilution_theorem_count", "natural_node_frame_edge_theorem_count", "new_quantum_algorithm_count"],
            dependencies=[
                "self_dual_wreath_fixed_family_common_rank_dilution.py",
            ],
            next_actions=[
                "Run qsearch.py code-wreath-fixed-family-common-rank-dilution.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-GLOBAL-CARRIER-CHANNEL-EXTRACTOR",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Exp Code Self Dual Wreath Global Carrier Channel Extractor",
            status="planned",
            hypothesis="Extracted physical W6 carrier channels as affine incomplete stars, reproducing the 1/17 and 1/9 graded defects and rejecting affine-to-complete closure.",
            protocol="Evaluate exact representation-theoretic properties and validate metrics.",
            positive_signal="Extracted physical W6 carrier channels as affine incomplete stars, reproducing the 1/17 and 1/9 graded defects and rejecting affine-to-complete closure.",
            falsifiers=["Affine orientation support does not force a complete residual channel graph.", "Vertex-local carrier groupoids must be glued through both endpoint projection algebras before assigning a global channel.", "Natural progress requires scaling the extracted channel graph family, not extrapolating from selected affine stars."],
            metrics=["physical_global_channel_extraction_algorithm_count", "physical_extraction_control_count", "physical_extraction_failure_count", "extracted_nontrivial_channel_count", "extracted_affine_incomplete_channel_count", "extracted_complete_affine_channel_count", "maximum_extracted_channel_edge_count", "maximum_extracted_channel_multiplicity", "extracted_three_edge_star_channel_count", "extracted_nonstar_channel_count", "maximum_extracted_channel_grading_defect", "minimum_extracted_channel_endpoint_gap", "natural_threshold_channel_scaling_theorem_count", "new_quantum_algorithm_count"],
            dependencies=[
                "self_dual_wreath_global_carrier_channel_extractor.py",
            ],
            next_actions=[
                "Run qsearch.py code-wreath-global-carrier-channel-extractor.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-GLOBAL-COLLISION-FREE-MASS",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Exp Code Self Dual Wreath Global Collision Free Mass",
            status="planned",
            hypothesis="Derived the exact Plancherel mass of globally distinct source partitions; at n=48 and k=203 the finite mass is about 2^-26.510, while the maximal-dimension theorem proves that it tends to one asymptotically.",
            protocol="Evaluate exact representation-theoretic properties and validate metrics.",
            positive_signal="Derived the exact Plancherel mass of globally distinct source partitions; at n=48 and k=203 the finite mass is about 2^-26.510, while the maximal-dimension theorem proves that it tends to one asymptotically.",
            falsifiers=["Finite n<=48 collision-free mass is strongly pre-asymptotic and cannot falsify asymptotic typicality.", "Within-pair unequal conditioning is not the same as collision freedom across copies.", "Finite low-n spectral gaps are not a uniform theorem over the asymptotically typical collision-free event."],
            metrics=["global_collision_free_mass_formula_count", "exact_validation_count", "exact_validation_failure_count", "finite_scaling_row_count", "combinatorially_impossible_threshold_row_count", "inverse_polynomial_mass_row_count", "minimum_finite_log2_collision_free_mass", "tail_n", "tail_partition_count", "tail_copy_count", "tail_log2_collision_free_mass", "tail_plancherel_collision_probability", "asymptotic_global_collision_free_mass_tends_to_one_theorem_count", "asymptotic_inverse_polynomial_mass_theorem_count", "new_quantum_algorithm_count"],
            dependencies=[
                "self_dual_wreath_global_collision_free_mass.py",
            ],
            next_actions=[
                "Run qsearch.py code-wreath-global-collision-free-mass.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-GLOBAL-DISTINCT-JOINT-KERNEL",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Exp Code Self Dual Wreath Global Distinct Joint Kernel",
            status="planned",
            hypothesis="Derived and exactly validated the nonfactorizing injective Plancherel kernel for globally distinct sources. Generic moment transfer is too coarse, but high-probability spectral events can instead be conditioned directly once proved independently.",
            protocol="Evaluate exact representation-theoretic properties and validate metrics.",
            positive_signal="Derived and exactly validated the nonfactorizing injective Plancherel kernel for globally distinct sources. Generic moment transfer is too coarse, but high-probability spectral events can instead be conditioned directly once proved independently.",
            falsifiers=["Global source distinctness does not preserve independence across physical labels.", "Pairwise-unequal product kernels have exact nonzero finite factorization defects after global conditioning.", "An o(1) source-conditioning error is not enough for an exp(-Theta(n log n)) PGM moment target.", "Direct moment transfer needs signed injective control, but event-level conditioning can bypass moment transfer."],
            metrics=["injective_transform_validation_count", "injective_transform_validation_failure_count", "joint_word_kernel_control_count", "joint_word_kernel_structure_failure_count", "nonfactorizing_joint_word_control_count", "conditioning_tv_finite_bound_failure_count", "injective_plancherel_transform_theorem_count", "global_distinct_joint_character_kernel_theorem_count", "conditioning_total_variation_identity_theorem_count", "conditioning_distance_asymptotic_scale_theorem_count", "generic_tv_transfer_no_go_theorem_count", "tail_n", "tail_copy_count", "tail_log2_conditioning_total_variation", "tail_log2_required_second_moment_scale", "tail_log2_tv_to_required_scale_gap", "uniform_collision_free_signed_contraction_theorem_count", "event_level_conditioning_bypass_theorem_count", "new_quantum_algorithm_count"],
            dependencies=[
                "self_dual_wreath_global_distinct_joint_kernel.py",
            ],
            next_actions=[
                "Run qsearch.py code-wreath-global-distinct-joint-kernel.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-GLOBAL-PARTITION-COLLISION",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Exp Code Self Dual Wreath Global Partition Collision",
            status="planned",
            hypothesis="Proved that natural threshold tuples have globally distinct source partitions with probability 1-o(1), excluding the known repeated-source 1/2-norm block; finite controls through n=32 remain preasymptotic.",
            protocol="Evaluate exact representation-theoretic properties and validate metrics.",
            positive_signal="Proved that natural threshold tuples have globally distinct source partitions with probability 1-o(1), excluding the known repeated-source 1/2-norm block; finite controls through n=32 remain preasymptotic.",
            falsifiers=["Within-label inequality is weaker than global distinctness across all 2k source partitions.", "The information-threshold copy count is polynomial and cannot overcome exponential collision decay.", "Known W3 half-norm unequal counterexamples reuse source partitions and are asymptotically negligible.", "Finite n controls remain preasymptotic and are not evidence of early global-distinct dominance.", "Global distinctness alone does not prove frame-norm decay.", "No collision-free contraction, measurement circuit, decoder, or classical separation is claimed."],
            metrics=["global_collision_control_count", "maximum_exact_occupancy_n", "global_source_draw_as_iid_plancherel_theorem_count", "global_collision_union_bound_theorem_count", "asymptotic_global_all_distinct_dominance_theorem_count", "finite_ninety_percent_global_distinct_row_count", "repeated_source_half_norm_counterexample_count", "repeated_source_counterexample_exclusion_count", "collision_free_tuple_norm_theorem_count", "collision_free_growing_moment_contraction_count", "natural_average_inverse_polynomial_conclusive_theorem_count", "structured_maximal_effect_dilation_count", "polynomial_hidden_permutation_decoder_count"],
            dependencies=[
                "self_dual_wreath_global_partition_collision.py",
            ],
            next_actions=[
                "Run qsearch.py code-wreath-global-partition-collision.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-GPE-HOLONOMY-RESOLVER-REDUCTION",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Exp Code Self Dual Wreath Gpe Holonomy Resolver Reduction",
            status="planned",
            hypothesis="Reduced flat higher-order carrier consistency to simultaneous fixed spaces of executable GPE holonomies and isolated the natural holonomy-gap theorem as the remaining circuit bottleneck.",
            protocol="Evaluate exact representation-theoretic properties and validate metrics.",
            positive_signal="Reduced flat higher-order carrier consistency to simultaneous fixed spaces of executable GPE holonomies and isolated the natural holonomy-gap theorem as the remaining circuit bottleneck.",
            falsifiers=["Canonical pair transport does not trivialize cycles because each edge uses a different tensor-block decomposition.", "Dense Racah matrices need not be computed classically to run or phase-estimate their cycle products.", "Efficient edge circuits alone do not imply efficient common-section projection; the holonomy frustration gap can be tiny."],
            metrics=["connection_kernel_holonomy_reduction_theorem_count", "finite_holonomy_control_count", "finite_holonomy_control_failure_count", "implicit_dense_racah_circuit_compiler_count", "near_flat_gap_counterexample_count", "near_flat_inverse_gap_growth_verified_count", "natural_inverse_polynomial_holonomy_gap_theorem_count", "recursive_child_span_holonomy_reduction_theorem_count", "complete_relative_frame_resolver_count", "new_quantum_algorithm_count"],
            dependencies=[
                "self_dual_wreath_gpe_holonomy_resolver_reduction.py",
            ],
            next_actions=[
                "Run qsearch.py code-wreath-gpe-holonomy-resolver-reduction.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-GPE-PAIR-POLAR-TRANSPORT",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Exp Code Self Dual Wreath Gpe Pair Polar Transport",
            status="planned",
            hypothesis="Derived a polynomial generalized-phase-estimation circuit for exact orientation-pair polar transport that preserves unknown Kronecker multiplicity labels and avoids inverse-dimension amplification.",
            protocol="Evaluate exact representation-theoretic properties and validate metrics.",
            positive_signal="Derived a polynomial generalized-phase-estimation circuit for exact orientation-pair polar transport that preserves unknown Kronecker multiplicity labels and avoids inverse-dimension amplification.",
            falsifiers=["High-dimensional natural carrier mass blocks normalized cross-overlap QSVT but does not block direct pair polar transport.", "A complete symmetric-group Kronecker transform is stronger than necessary for pair transport; coherent GPE row access suffices.", "Pair transport is no longer a defensible main bottleneck. Future work must attack higher-order relative-frame composition."],
            metrics=["gpe_pair_polar_reassociation_theorem_count", "finite_carrier_control_count", "finite_carrier_control_failure_count", "normalized_cross_overlap_dimension_barrier_bypass_count", "full_kronecker_transform_dependency_count", "polynomial_pair_transport_circuit_count", "polynomial_higher_level_relative_polar_circuit_count", "polynomial_complete_orientation_polar_circuit_count", "new_quantum_algorithm_count"],
            dependencies=[
                "self_dual_wreath_gpe_pair_polar_transport.py",
            ],
            next_actions=[
                "Run qsearch.py code-wreath-gpe-pair-polar-transport.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-GPE-RECURSIVE-NODE-COMPILER",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Exp Code Self Dual Wreath Gpe Recursive Node Compiler",
            status="planned",
            hypothesis="Separated recursive relation preparation into normalized child embeddings and a short-metric endpoint mixer, then proved that flat affine child embeddings compile from GPE transports in logarithmic depth without a width penalty.",
            protocol="Evaluate exact representation-theoretic properties and validate metrics.",
            positive_signal="Separated recursive relation preparation into normalized child embeddings and a short-metric endpoint mixer, then proved that flat affine child embeddings compile from GPE transports in logarithmic depth without a width penalty.",
            falsifiers=["Pair-level GPE does not by itself remove recursive child-frame normalization.", "Constant metric spectra do not imply a scalar endpoint mixer when shorted metrics fail to commute.", "Exact recursive completeness remains compatible with exponentially small endpoint gaps.", "Finite affine GPE paths are not an all-n uniform path-selection theorem.", "A globally source-distinct S6 affine node falsifies universal scalar child fibers."],
            metrics=["recursive_parent_relation_normal_form_theorem_count", "node_control_count", "node_control_failure_count", "equal_metric_hadamard_control_count", "noncommuting_matrix_mixer_countercontrol_count", "flat_affine_log_depth_preparation_theorem_count", "finite_affine_gpe_compiler_control_count", "finite_affine_gpe_compiler_failure_count", "conditioning_counterfamily_row_count", "tail_metric_ratio", "tail_endpoint_gap", "tail_inverse_gap_cost", "all_n_structured_child_embedding_theorem_count", "companion_natural_scalar_affine_falsifier_count", "uniform_coherent_generator_select_count", "natural_recursive_endpoint_gap_theorem_count", "recursive_orientation_polar_sampler_count", "new_quantum_algorithm_count"],
            dependencies=[
                "self_dual_wreath_gpe_recursive_node_compiler.py",
            ],
            next_actions=[
                "Run qsearch.py code-wreath-gpe-recursive-node-compiler.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-GRADED-CHANNEL-GRAPH-REDUCTION",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Exp Code Self Dual Wreath Graded Channel Graph Reduction",
            status="planned",
            hypothesis="Reduced every flat carrier support graph to two child Laplacian resolvents, reproducing the W6 1/17 and 1/9 defects and unifying the crossing obstruction with the complete-closure rescue.",
            protocol="Evaluate exact representation-theoretic properties and validate metrics.",
            positive_signal="Reduced every flat carrier support graph to two child Laplacian resolvents, reproducing the W6 1/17 and 1/9 defects and unifying the crossing obstruction with the complete-closure rescue.",
            falsifiers=["Affine orientation vertices do not force complete residual channel-edge support.", "The graded gap depends on child Laplacian resolvents, not the ungraded metric floor or raw degree.", "The natural all-depth target is two-sided endpoint-effect comparability for global carrier channels."],
            metrics=["arbitrary_flat_channel_resolvent_reduction_theorem_count", "endpoint_effect_comparability_criterion_theorem_count", "finite_graph_control_count", "finite_graph_control_failure_count", "maximum_direct_to_resolvent_metric_residual", "maximum_direct_to_resolvent_grading_residual", "w6_affine_star_defect_reproduction_count", "crossing_no_go_reproduction_count", "complete_internal_rescue_reproduction_count", "natural_channel_resolvent_comparability_theorem_count", "new_quantum_algorithm_count"],
            dependencies=[
                "self_dual_wreath_graded_channel_graph_reduction.py",
            ],
            next_actions=[
                "Run qsearch.py code-wreath-graded-channel-graph-reduction.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-GRADED-FLAT-TRANSPORT-NO-GO",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Exp Code Self Dual Wreath Graded Flat Transport No Go",
            status="planned",
            hypothesis="Proved an exact flat-transport counterfamily with constant metric floor but endpoint gap Theta(1/p); at natural width the tail model has log2 gap -1675.011.",
            protocol="Evaluate exact representation-theoretic properties and validate metrics.",
            positive_signal="Proved an exact flat-transport counterfamily with constant metric floor but endpoint gap Theta(1/p); at natural width the tail model has log2 gap -1675.011.",
            falsifiers=["A width-independent relation-metric floor is not a width-independent relative-polar gap.", "A flat partial-isometry carrier groupoid still supports dangerous row and column modes.", "The next proof must quotient internal pair/Cech relations before bounding the grading defect."],
            metrics=["flat_transport_exact_spectrum_theorem_count", "metric_floor_not_graded_gap_no_go_theorem_count", "finite_flat_transport_control_count", "finite_flat_transport_control_failure_count", "finite_constant_metric_width_closing_gap_count", "natural_width_scaling_row_count", "natural_width_inverse_polynomial_endpoint_gap_row_count", "tail_n", "tail_copy_count", "tail_metric_floor", "tail_log2_endpoint_gap", "full_internal_cech_mode_elimination_theorem_count", "new_quantum_algorithm_count"],
            dependencies=[
                "self_dual_wreath_graded_flat_transport_no_go.py",
            ],
            next_actions=[
                "Run qsearch.py code-wreath-graded-flat-transport-no-go.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-GRADED-FROBENIUS-TRIM",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Exp Code Self Dual Wreath Graded Frobenius Trim",
            status="planned",
            hypothesis="Constructed a grading-compatible pinched Frobenius trim and proved an explicit constant endpoint gap after deleting vanishing natural relation coefficient fraction; the deleted relation image has zero ideal PGM state mass.",
            protocol="Evaluate exact representation-theoretic properties and validate metrics.",
            positive_signal="Constructed a grading-compatible pinched Frobenius trim and proved an explicit constant endpoint gap after deleting vanishing natural relation coefficient fraction; the deleted relation image has zero ideal PGM state mass.",
            falsifiers=["Generic spectral intersection is unnecessary; sectorwise pinching preserves the sibling grading exactly.", "Vanishing Frobenius density can yield a constant endpoint gap after a quantified trim, but not on the full untrimmed space.", "Information-theoretic coefficient trimming is exactly signal-preserving on the ideal polar range, but is not an efficient or complete quantum operation by itself."],
            metrics=["grading_compatible_pinched_trim_theorem_count", "schur_endpoint_bound_theorem_count", "finite_control_count", "finite_control_failure_count", "scaling_row_count", "constant_gap_certified_row_count", "constant_gap_certification_onset_n", "tail_n", "tail_removed_fraction_upper_bound", "tail_endpoint_gap_lower_bound", "tail_structural_failure_probability_upper_bound", "coherent_pinched_energy_projector_count", "relation_trim_zero_pgm_state_loss_theorem_count", "all_n_pair_cokernel_completeness_theorem_count", "new_quantum_algorithm_count"],
            dependencies=[
                "self_dual_wreath_graded_frobenius_trim.py",
            ],
            next_actions=[
                "Run qsearch.py code-wreath-graded-frobenius-trim.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-HAMMING-STRATUM-RANK-TRANSITION",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Exp Code Self Dual Wreath Hamming Stratum Rank Transition",
            status="planned",
            hypothesis="Proved a sharp transition from transverse Hamming radii one/two to rank-concentrated live strata at every fixed radius at least three.",
            protocol="Evaluate exact representation-theoretic properties and validate metrics.",
            positive_signal="Proved a sharp transition from transverse Hamming radii one/two to rank-concentrated live strata at every fixed radius at least three.",
            falsifiers=["Exponential stratum size blocks all-pair union bounds but not density statements.", "Pair-common rank density does not determine coherent incidence or a frame edge.", "The natural independent support threshold is exactly three, not an unspecified covering radius."],
            metrics=["fixed_hamming_stratum_rank_density_theorem_count", "sharp_hamming_pair_support_transition_theorem_count", "finite_control_count", "finite_control_failure_count", "maximum_scaling_n", "n50_h3_expected_bad_fraction_upper_bound", "simultaneous_all_fixed_hamming_pair_theorem_count", "coherent_incidence_theorem_count", "natural_node_frame_edge_theorem_count", "new_quantum_algorithm_count"],
            dependencies=[
                "self_dual_wreath_hamming_stratum_rank_transition.py",
            ],
            next_actions=[
                "Run qsearch.py code-wreath-hamming-stratum-rank-transition.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-HIERARCHICAL-COKERNEL-RESOLUTION",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Exp Code Self Dual Wreath Hierarchical Cokernel Resolution",
            status="planned",
            hypothesis="Resolved augmented H0 exactly by recursive child-span relations and isolated child pseudoinverse-frame comparability as the true endpoint-gap gate.",
            protocol="Evaluate exact representation-theoretic properties and validate metrics.",
            positive_signal="Resolved augmented H0 exactly by recursive child-span relations and isolated child pseudoinverse-frame comparability as the true endpoint-gap gate.",
            falsifiers=["Direct pair-common incompleteness is not a no-go for hierarchical span relations.", "Exact hierarchical relation completeness does not imply efficient conditioning.", "Support topology alone cannot determine parent endpoint weights."],
            metrics=["exact_hierarchical_cokernel_decomposition_theorem_count", "finite_control_count", "finite_control_failure_count", "emergent_h0_control_count", "emergent_h0_resolved_control_count", "physical_control_count", "conditioning_counterfamily_row_count", "tail_counterfamily_child_width", "tail_counterfamily_endpoint_gap", "tail_counterfamily_inverse_gap_cost", "natural_child_frame_comparability_theorem_count", "coherent_hierarchical_kernel_transform_count", "new_quantum_algorithm_count"],
            dependencies=[
                "self_dual_wreath_hierarchical_cokernel_resolution.py",
            ],
            next_actions=[
                "Run qsearch.py code-wreath-hierarchical-cokernel-resolution.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-HIERARCHICAL-POLAR-TREE",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Exp Code Self Dual Wreath Hierarchical Polar Tree",
            status="planned",
            hypothesis="Factored the global orientation polar into k coherently indexed binary relative-frame isometries. The first level is polynomial; all complete W4 level-two effects are projective or exactly balanced. The decisive open theorem is the typical large-n relative-effect spectrum and its direct circuit.",
            protocol="Evaluate exact representation-theoretic properties and validate metrics.",
            positive_signal="Factored the global orientation polar into k coherently indexed binary relative-frame isometries. The first level is polynomial; all complete W4 level-two effects are projective or exactly balanced. The decisive open theorem is the typical large-n relative-effect spectrum and its direct circuit.",
            falsifiers=["Do not require explicit compilation of every orientation-tree node; a nested flag addresses nodes coherently by level.", "Do not claim the chain rule alone removes whitening; it moves whitening into matrix-valued conditional frame ratios.", "Do not cite the Moore--Russell--Sniady theorem directly against this global coherent primitive, or claim noncoverage as success."],
            metrics=["hierarchical_polar_chain_rule_theorem_count", "matrix_valued_conditional_probability_reduction_count", "coherent_k_level_compilation_theorem_count", "finite_w4_control_count", "finite_hierarchical_validation_failure_count", "finite_half_integral_relative_effect_failure_count", "finite_noncommuting_child_frame_count", "finite_fractional_relative_effect_eigenvalue_count", "minimum_finite_fractional_relative_effect_gap", "maximum_recursive_to_direct_polar_residual", "tail_n", "tail_tree_depth", "higher_level_relative_sampler_count", "typical_relative_effect_gap_theorem_count", "global_nonorthogonal_sampler_count", "polynomial_hidden_permutation_decoder_count", "new_quantum_algorithm_count"],
            dependencies=[
                "self_dual_wreath_hierarchical_polar_tree.py",
            ],
            next_actions=[
                "Run qsearch.py code-wreath-hierarchical-polar-tree.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-HIERARCHY-LOW-CARRIER-TRIM",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Exp Code Self Dual Wreath Hierarchy Low Carrier Trim",
            status="planned",
            hypothesis="Proved a quarter-factorial rank--correlation tradeoff that trims all low-dimensional pair carriers throughout the hierarchy.",
            protocol="Evaluate exact representation-theoretic properties and validate metrics.",
            positive_signal="Proved a quarter-factorial rank--correlation tradeoff that trims all low-dimensional pair carriers throughout the hierarchy.",
            falsifiers=["Exact-common carriers are not the only sectors that admit a vanishing-rank hierarchy trim.", "Worst-case inverse-(n-1) pair correlations can be removed on all but vanishing natural rank; they are not the remaining edge bottleneck.", "Vanishing pair correlations still do not imply an exponentially wide frame edge without signed high-order incidence control."],
            metrics=["exact_low_carrier_mass_control_count", "finite_control_failure_count", "scaling_record_count", "finite_conditioned_trim_record_count", "first_finite_conditioned_trim_n", "tail_n", "tail_cutoff_dimension_log2", "tail_low_carrier_count", "tail_conditioned_trim_rank_log2", "tail_residual_pair_correlation_log2", "all_hierarchy_low_carrier_tradeoff_theorem_count", "high_carrier_incidence_theorem_count", "natural_node_frame_edge_theorem_count", "new_quantum_algorithm_count"],
            dependencies=[
                "self_dual_wreath_hierarchy_low_carrier_trim.py",
            ],
            next_actions=[
                "Run qsearch.py code-wreath-hierarchy-low-carrier-trim.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-HIERARCHY-PAIR-COMMON-RANK-BUDGET",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Exp Code Self Dual Wreath Hierarchy Pair Common Rank Budget",
            status="planned",
            hypothesis="Proved an all-depth, all-target factorial rank budget for exact pair-common directions and isolated the noncommon return operator as the remaining frame-edge obstruction.",
            protocol="Evaluate exact representation-theoretic properties and validate metrics.",
            positive_signal="Proved an all-depth, all-target factorial rank budget for exact pair-common directions and isolated the noncommon return operator as the remaining frame-edge obstruction.",
            falsifiers=["Typical recurrence of pair-common support does not imply macroscopic common rank at information-threshold width.", "Triangle and cycle alignment cannot defeat the scalar rank budget for exact common directions, although it remains decisive on the noncommon complement.", "Unconditioned regular stationary outliers must be trimmed; their existence is not representative of collision-free physical rank."],
            metrics=["exact_hierarchy_pair_count_control_count", "exact_pair_common_mass_control_count", "finite_control_failure_count", "scaling_record_count", "finite_conditioned_rank_trim_record_count", "conditioned_tail_bound_strictly_decreasing_count", "tail_n", "tail_conditioned_all_target_union_bound_log2", "tail_simultaneous_relative_rank_threshold_log2", "exact_common_hierarchy_rank_budget_theorem_count", "noncommon_collision_free_return_theorem_count", "natural_node_frame_edge_theorem_count", "new_quantum_algorithm_count"],
            dependencies=[
                "self_dual_wreath_hierarchy_pair_common_rank_budget.py",
            ],
            next_actions=[
                "Run qsearch.py code-wreath-hierarchy-pair-common-rank-budget.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-INTERNAL-CLOSURE-GRADED-RESCUE",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Exp Code Self Dual Wreath Internal Closure Graded Rescue",
            status="planned",
            hypothesis="Proved that complete internal pair closure converts the flat crossing obstruction into a uniform endpoint gap of at least one quarter, independent of natural merge width.",
            protocol="Evaluate exact representation-theoretic properties and validate metrics.",
            positive_signal="Proved that complete internal pair closure converts the flat crossing obstruction into a uniform endpoint gap of at least one quarter, independent of natural merge width.",
            falsifiers=["The crossing-only flat counterfamily is not stable under the hierarchy's internal Schur quotient.", "Metric control becomes useful for grading only after complete internal channel closure is established.", "The decisive remaining theorem is global affine gluing of natural carrier channels, not another local star bound."],
            metrics=["complete_internal_channel_schur_spectrum_theorem_count", "complete_internal_channel_uniform_graded_gap_theorem_count", "finite_internal_closure_control_count", "finite_internal_closure_control_failure_count", "finite_quarter_gap_failure_count", "natural_width_scaling_row_count", "natural_width_quarter_gap_failure_count", "tail_n", "tail_copy_count", "tail_metric_floor", "tail_grading_defect_norm", "tail_endpoint_gap", "natural_global_affine_channel_closure_theorem_count", "new_quantum_algorithm_count"],
            dependencies=[
                "self_dual_wreath_internal_closure_graded_rescue.py",
            ],
            next_actions=[
                "Run qsearch.py code-wreath-internal-closure-graded-rescue.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-INTERPLANE-GAUGE-HOMOLOGY",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Exp Code Self Dual Wreath Interplane Gauge Homology",
            status="planned",
            hypothesis="Classified the scalar inter-plane gauge obstruction and proved that the pattern-rich hierarchy retains Theta(4^K) independent cycle bits despite every local plane being positive.",
            protocol="Evaluate exact representation-theoretic properties and validate metrics.",
            positive_signal="Classified the scalar inter-plane gauge obstruction and proved that the pattern-rich hierarchy retains Theta(4^K) independent cycle bits despite every local plane being positive.",
            falsifiers=["Single-plane positive-holonomy audits cannot establish a global gauge or frame edge, even on the rich support family.", "The all-positive Steiner surrogate is one gauge class among 2^Theta(4^K) combinatorially admissible classes.", "Future finite audits must include multi-plane incidence cycles and report gauge-invariant cycle products."],
            metrics=["full_homology_control_count", "rich_homology_control_count", "rich_point_degree_control_count", "finite_control_failure_count", "tail_rich_copy_count", "tail_rich_cycle_rank", "scalar_interplane_gauge_classification_theorem_count", "rich_cycle_rank_theorem_count", "natural_cycle_holonomy_theorem_count", "operator_valued_connection_theorem_count", "new_quantum_algorithm_count"],
            dependencies=[
                "self_dual_wreath_interplane_gauge_homology.py",
            ],
            next_actions=[
                "Run qsearch.py code-wreath-interplane-gauge-homology.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-INVARIANT-PROJECTOR-CIRCUIT",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Exp Code Self Dual Wreath Invariant Projector Circuit",
            status="planned",
            hypothesis="Converted the orientation invariant projector from an assumed Kronecker primitive into an explicit polynomial projected circuit using the symmetric-group QFT and coherent group averaging.",
            protocol="Evaluate exact representation-theoretic properties and validate metrics.",
            positive_signal="Converted the orientation invariant projector from an assumed Kronecker primitive into an explicit polynomial projected circuit using the symmetric-group QFT and coherent group averaging.",
            falsifiers=["A general efficient Kronecker decomposition is not required to apply the invariant projector as a Kraus block.", "The selected orientation-family filter avoids factorial amplification and factorial spectral precision.", "The decisive remaining issue is now postfilter information and decoding, not implementation of E_(nu,h)."],
            metrics=["symmetric_group_qft_intertwining_theorem_count", "irrep_action_from_qft_theorem_count", "projected_group_average_block_encoding_theorem_count", "controlled_orientation_invariant_projector_circuit_schema_count", "qft_finite_control_count", "qft_finite_validation_failure_count", "projected_block_finite_control_count", "projected_block_validation_failure_count", "scaling_record_count", "factorial_amplitude_amplification_row_count", "all_n_postfilter_information_theorem_count", "polynomial_hidden_permutation_decoder_count", "new_quantum_algorithm_count"],
            dependencies=[
                "self_dual_wreath_invariant_projector_circuit.py",
            ],
            next_actions=[
                "Run qsearch.py code-wreath-invariant-projector-circuit.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-ISOTYPIC-DEPHASING-NO-GO",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Exp Code Self Dual Wreath Isotypic Dephasing No Go",
            status="planned",
            hypothesis="Proved that dephasing diagonal S_n irrep labels caps exact hidden-permutation success by the largest Plancherel atom, hence by exp(-Omega(sqrt(n))); cross-sector coherence is mandatory.",
            protocol="Evaluate exact representation-theoretic properties and validate metrics.",
            positive_signal="Proved that dephasing diagonal S_n irrep labels caps exact hidden-permutation success by the largest Plancherel atom, hence by exp(-Omega(sqrt(n))); cross-sector coherence is mandatory.",
            falsifiers=["Measuring nu classically and decoding each sector independently cannot yield constant exact hidden-permutation success.", "Multiplicity-space dimension does not evade the no-go.", "The coherent isotypic label in the orientation filter is an essential information resource, not optional bookkeeping.", "The theorem does not rule out coherent cross-sector decoding or coarser decision problems."],
            metrics=["covariant_povm_symmetrization_theorem_count", "positive_partial_trace_domination_theorem_count", "isotypic_dephasing_identification_no_go_theorem_count", "data_processing_extension_theorem_count", "operator_control_count", "operator_control_failure_count", "exact_symmetric_group_scaling_record_count", "dimension_square_sum_failure_count", "finite_constant_success_ruled_out_count", "tail_n", "tail_exact_success_upper_bound", "tail_log2_success_upper_bound", "coherent_cross_sector_decoder_count", "new_quantum_algorithm_count"],
            dependencies=[
                "self_dual_wreath_isotypic_dephasing_no_go.py",
            ],
            next_actions=[
                "Run qsearch.py code-wreath-isotypic-dephasing-no-go.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-LEAF-WHITENING-COMMUTATOR-NO-GO",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Exp Code Self Dual Wreath Leaf Whitening Commutator No Go",
            status="planned",
            hypothesis="Characterized commuting canonical effects for projection frames and built a bounded-condition, duplicate-free counterfamily that kills every coarse leaf-to-component commutator transfer route.",
            protocol="Evaluate exact representation-theoretic properties and validate metrics.",
            positive_signal="Characterized commuting canonical effects for projection frames and built a bounded-condition, duplicate-free counterfamily that kills every coarse leaf-to-component commutator transfer route.",
            falsifiers=["Density-one inverse-polynomial leaf commutators do not imply noncommuting canonical effects.", "A bounded frame condition number does not rescue leaf-to-component commutator transfer.", "Distinct high-rank leaves, constant total-rank aspect, vanishing relative leaf rank, and full-rank nonscalarity still do not rescue the transfer.", "The next natural gate is spectral arithmetic or direct commutator support, not another coarse frame-conditioning estimate."],
            metrics=["commuting_whitening_integer_cover_criterion_theorem_count", "reciprocal_integer_spectral_witness_theorem_count", "duplicate_free_bounded_condition_counterfamily_count", "finite_control_count", "finite_control_failure_count", "tail_fiber_dimension", "tail_frame_condition_number", "tail_cross_part_pair_fraction", "tail_cross_leaf_commutator_norm", "tail_nonscalarity_defect_edge", "natural_component_nonreciprocal_spectrum_mass_theorem_count", "natural_canonical_component_commutator_mass_theorem_count", "new_quantum_algorithm_count"],
            dependencies=[
                "self_dual_wreath_leaf_whitening_commutator_no_go.py",
            ],
            next_actions=[
                "Run qsearch.py code-wreath-leaf-whitening-commutator-no-go.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-LEVEL-THREE-FLAG-AUDIT",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Exp Code Self Dual Wreath Level Three Flag Audit",
            status="planned",
            hypothesis="Audited every three-bit linear flag, found only endpoint/half relative channels on selected label-simple sectors, and proved by countercontrol that repeated labels generate genuinely nonhalf conditional weights.",
            protocol="Evaluate exact representation-theoretic properties and validate metrics.",
            positive_signal="Audited every three-bit linear flag, found only endpoint/half relative channels on selected label-simple sectors, and proved by countercontrol that repeated labels generate genuinely nonhalf conditional weights.",
            falsifiers=["Balanced binary trees do not generically force relative effects into {0,1/2,1}.", "The W4 half-integral pattern survives a genuine third level and every linear flag in the selected W5 controls.", "Spectral half-integrality alone is not an implementation of the relative merge isometry.", "The formal MRS theorem does not cover this coherent global projector family, but that noncoverage is not a speedup proof."],
            metrics=["ordered_f2_three_basis_count", "finite_control_count", "finite_validation_failure_count", "pairwise_distinct_label_control_count", "pairwise_distinct_label_half_integral_failure_count", "collision_free_control_count", "collision_free_half_integral_failure_count", "repeated_label_counterexample_count", "merge_occurrence_count", "fractional_eigenvalue_occurrence_count", "nonhalf_fractional_eigenvalue_occurrence_count", "collision_free_half_integrality_all_n_theorem_count", "polynomial_relative_eigenspace_projector_count", "hierarchical_orientation_polar_sampler_count", "polynomial_physical_pgm_circuit_count", "new_quantum_algorithm_count"],
            dependencies=[
                "self_dual_wreath_level_three_flag_audit.py",
            ],
            next_actions=[
                "Run qsearch.py code-wreath-level-three-flag-audit.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-LOCAL-PAIR-TRANSVERSALITY",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Exp Code Self Dual Wreath Local Pair Transversality",
            status="planned",
            hypothesis="Proved that globally distinct natural source blocks are simultaneously transverse through Hamming radius two with high probability.",
            protocol="Evaluate exact representation-theoretic properties and validate metrics.",
            positive_signal="Proved that globally distinct natural source blocks are simultaneously transverse through Hamming radius two with high probability.",
            falsifiers=["Unioning over orientation pairs ignores that common-range conditions are shared source-label relations.", "The full regular pair-common obstruction is not typical at Hamming radii one and two.", "Local transversality alone does not control the full orientation-frame edge."],
            metrics=["hamming_one_common_range_necessity_theorem_count", "hamming_two_common_range_necessity_theorem_count", "asymptotic_radius_two_transversality_theorem_count", "finite_control_count", "finite_control_failure_count", "maximum_scaling_n", "higher_order_incidence_theorem_count", "natural_node_frame_edge_theorem_count", "new_quantum_algorithm_count"],
            dependencies=[
                "self_dual_wreath_local_pair_transversality.py",
            ],
            next_actions=[
                "Run qsearch.py code-wreath-local-pair-transversality.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-MATRIX-CAYLEY-BOUNDARY",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Exp Code Self Dual Wreath Matrix Cayley Boundary",
            status="planned",
            hypothesis="Proved the exact matrix-valued Cayley boundary: opposite saturation-space overlap produces nonhalf channels. An explicit positive-definite XOR-covariant counterfamily kills covariance-only proofs, while commuting sectors remain balanced.",
            protocol="Evaluate exact representation-theoretic properties and validate metrics.",
            positive_signal="Proved the exact matrix-valued Cayley boundary: opposite saturation-space overlap produces nonhalf channels. An explicit positive-definite XOR-covariant counterfamily kills covariance-only proofs, while commuting sectors remain balanced.",
            falsifiers=["Matrix-valued XOR covariance does not by itself force half-integral affine merges.", "Equal child Gram blocks do not make the cross-dependency quotient neutral when opposite saturation kernels overlap.", "Matrix-valued multiplicity sectors are not automatically fatal; commuting or saturation-orthogonal sectors remain balanced.", "A spectral balance theorem does not implement the multiplicity-space kernel projectors."],
            metrics=["matrix_cayley_boundary_theorem_count", "exact_control_count", "theorem_validation_failure_count", "commuting_neutral_control_count", "noncommuting_nonneutral_counterexample_count", "counterexample_maximum_opposite_saturation_overlap", "counterexample_maximum_half_residual", "finite_wreath_matrix_valued_cayley_counterexample_count", "all_n_saturation_orthogonality_theorem_count", "coherent_internal_saturation_projector_count", "new_quantum_algorithm_count"],
            dependencies=[
                "self_dual_wreath_matrix_cayley_boundary.py",
            ],
            next_actions=[
                "Run qsearch.py code-wreath-matrix-cayley-boundary.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-MATRIX-POVM-RECURSIVE-COMPILER",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Exp Code Self Dual Wreath Matrix Povm Recursive Compiler",
            status="planned",
            hypothesis="Replaced the dead scalar-affine extension with an exact nested matrix-POVM compiler normal form and isolated square-root POVM dilation as the new access bottleneck.",
            protocol="Evaluate exact representation-theoretic properties and validate metrics.",
            positive_signal="Replaced the dead scalar-affine extension with an exact nested matrix-POVM compiler normal form and isolated square-root POVM dilation as the new access bottleneck.",
            falsifiers=["Noncommuting component effects do not obstruct an exact algebraic compiler normal form.", "Outcome count alone does not impose square-root amplitude amplification on a normalized child embedding.", "Pair GPE transport does not prepare matrix POVM square-root amplitudes.", "Generic block-encoding access can still be superpolynomial at an exponentially small effect edge.", "Small component trace or outcome probability does not prove that the positive effect edge is small.", "The finite S6 effect edge is not asymptotic evidence because its source mechanism is negligible."],
            metrics=["matrix_component_povm_normal_form_theorem_count", "povm_control_count", "povm_control_failure_count", "noncommuting_povm_control_count", "nested_recursive_povm_normal_form_theorem_count", "companion_natural_s6_partial_support_control_count", "companion_direct_s6_source_mass_no_go_count", "generic_square_root_degree_boundary_theorem_count", "tail_input_size", "tail_effect_edge_log2", "tail_markov_degree_lower_bound_log2", "tail_polynomial_benchmark_log2", "natural_component_povm_dilation_circuit_count", "all_depth_gpe_support_transport_count", "recursive_orientation_polar_sampler_count", "new_quantum_algorithm_count"],
            dependencies=[
                "self_dual_wreath_matrix_povm_recursive_compiler.py",
            ],
            next_actions=[
                "Run qsearch.py code-wreath-matrix-povm-recursive-compiler.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-MIXED-COVARIANT-DECODER",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Exp Code Self Dual Wreath Mixed Covariant Decoder",
            status="planned",
            hypothesis="Replaced the pure dual-row heuristic by the exact mixed-state covariant seed, tight-frame, rank, and PGM whitening criteria.",
            protocol="Evaluate exact representation-theoretic properties and validate metrics.",
            positive_signal="Replaced the pure dual-row heuristic by the exact mixed-state covariant seed, tight-frame, rank, and PGM whitening criteria.",
            falsifiers=["The pure coherent Fourier decoder is only a benchmark: the physical mixed projector orbit generally requires multiple coherent junk rows.", "Sector-weight concentration alone cannot certify a decoder because it does not control multiplicity support or C_nu conditioning.", "W_3 PGM gains above the exact-label ceiling disappear substantially under isotypic dephasing, confirming the need for cross-sector coherence.", "Large tight-frame rank does not by itself imply computational hardness."],
            metrics=["mixed_covariant_seed_theorem_count", "multiplicity_tight_frame_theorem_count", "seed_rank_lower_bound_theorem_count", "canonical_pgm_seed_theorem_count", "finite_control_count", "finite_control_failure_count", "pure_rank_one_seed_impossible_control_count", "pgm_beats_dephasing_ceiling_control_count", "maximum_finite_pgm_success_probability", "maximum_cross_isotypic_success_contribution", "maximum_finite_seed_junk_rank_lower_bound", "typical_multiplicity_spectrum_theorem_count", "polynomial_multiplicity_whitening_count", "global_information_theoretic_pgm_success_theorem_count", "conditional_branch_uniform_success_theorem_count", "new_quantum_algorithm_count"],
            dependencies=[
                "self_dual_wreath_mixed_covariant_decoder.py",
            ],
            next_actions=[
                "Run qsearch.py code-wreath-mixed-covariant-decoder.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-COMMON-SPAN-COMPONENT-UNIVERSALITY-NO-GO",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Exp Code Self Dual Wreath Common Span Component Universality No Go",
            status="planned",
            hypothesis="Proved POVM universality of common-span compression and built a structured commuting counterfamily matching every coarse natural leaf/frame/component property currently established.",
            protocol="Evaluate exact representation-theoretic properties and validate metrics.",
            positive_signal="Proved POVM universality of common-span compression and built a structured commuting counterfamily matching every coarse natural leaf/frame/component property currently established.",
            falsifiers=["Non-reciprocal spectra are not commutator witnesses after common-span compression.", "A constant component edge and positive common-span rank do not transfer uncompressed leaf noncommutativity.", "The exact full-support integer-cover theorem cannot be applied directly to final-root compressed effects.", "Only a direct natural compressed-effect theorem or compiler can advance the component-algebra gate."],
            metrics=["common_span_povm_universality_theorem_count", "full_support_reciprocal_spectrum_transfer_falsifier_count", "all_coarse_data_commuting_component_counterfamily_count", "naimark_control_count", "naimark_control_failure_count", "structured_control_count", "structured_control_failure_count", "tail_common_span_dimension", "tail_common_span_relative_rank", "tail_frame_condition_number", "tail_cross_part_leaf_pair_fraction", "tail_cross_leaf_commutator_norm", "tail_component_positive_edge", "tail_nonscalarity_defect_edge", "natural_compressed_component_commutator_mass_theorem_count", "new_quantum_algorithm_count"],
            dependencies=[
                "self_dual_wreath_common_span_component_universality_no_go.py",
            ],
            next_actions=[
                "Run qsearch.py code-wreath-common-span-component-universality-no-go.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-MRS-COHERENCE-ESCAPE-CRITERION",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Exp Code Self Dual Wreath Mrs Coherence Escape Criterion",
            status="planned",
            hypothesis="Corrected the MRS audit: dephasing invariance characterizes only whether early projective transcript measurement changes statistics; classical transcript-only simulation requires block-scalar effects, and full adaptive MRS separation remains open.",
            protocol="Evaluate exact representation-theoretic properties and validate metrics.",
            positive_signal="Corrected the MRS audit: dephasing invariance characterizes only whether early projective transcript measurement changes statistics; classical transcript-only simulation requires block-scalar effects, and full adaptive MRS separation remains open.",
            falsifiers=["Leaving intermediate irrep labels unmeasured is not by itself outside the measured-sieve model.", "Label-controlled coherent computation with a label-diagonal final effect is transcript-dephasing invariant.", "Dephasing invariance does not imply transcript-only simulation when an isotypic transcript block has dimension greater than one.", "Pair GPE is carrier-block diagonal and cannot alone witness cross-transcript interference.", "A finite noncommuting effect is not a useful escape until it changes the physical decision probability on positive mass.", "Avoiding the MRS model would not prove an efficient algorithm or quantum speedup."],
            metrics=["transcript_dephasing_identity_theorem_count", "universal_measured_transcript_invariance_criterion_count", "projective_transcript_only_criterion_count", "finite_control_count", "finite_control_failure_count", "offdiagonal_coherence_witness_control_count", "within_block_quantum_witness_control_count", "deferred_measurement_non_escape_control_count", "hadamard_witness_probability_difference", "recoupled_witness_probability_difference", "within_block_transcript_only_probability_difference", "current_physical_pgm_transcript_povm_separation_count", "mrs_model_escape_theorem_count", "new_quantum_algorithm_count"],
            dependencies=[
                "self_dual_wreath_mrs_coherence_escape_criterion.py",
            ],
            next_actions=[
                "Run qsearch.py code-wreath-mrs-coherence-escape-criterion.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-MRS-TRANSCRIPT-POVM-SEPARATION",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Exp Code Self Dual Wreath Mrs Transcript Povm Separation",
            status="planned",
            hypothesis="Converted the MRS model boundary into an executable convex effect-space separation problem with exact dual witnesses and adaptive transcript-POVM compilation, while keeping every physical escape claim closed.",
            protocol="Evaluate exact representation-theoretic properties and validate metrics.",
            positive_signal="Converted the MRS model boundary into an executable convex effect-space separation problem with exact dual witnesses and adaptive transcript-POVM compilation, while keeping every physical escape claim closed.",
            falsifiers=["Off-diagonal coherence is sufficient but not necessary for transcript-only separation.", "A fixed projective dephasing test is not the full adaptive MRS criterion.", "Separation from one fixed transcript policy is not separation from the whole MRS algorithm class.", "Effect-space separation would not itself prove an efficient algorithm or speedup."],
            metrics=["fixed_transcript_zonotope_criterion_theorem_count", "dual_separation_witness_theorem_count", "adaptive_transcript_povm_compiler_theorem_count", "finite_control_count", "finite_control_failure_count", "classical_simulable_control_count", "within_block_separation_control_count", "offdiagonal_separation_control_count", "adaptive_nonprojective_separation_control_count", "physical_pgm_fixed_policy_separation_count", "physical_pgm_full_mrs_model_separation_count", "new_quantum_algorithm_count"],
            dependencies=[
                "self_dual_wreath_mrs_transcript_povm_separation.py",
            ],
            next_actions=[
                "Run qsearch.py code-wreath-mrs-transcript-povm-separation.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-MULTISCALE-POLAR-SCHEDULE",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Exp Code Self Dual Wreath Multiscale Polar Schedule",
            status="planned",
            hypothesis="Generalized the polar chain to constant arity, exposed the binary threshold hard edge, and proved an eight-way Gaussian schedule with all-depth constant frame conditioning.",
            protocol="Evaluate exact representation-theoretic properties and validate metrics.",
            positive_signal="Generalized the polar chain to constant arity, exposed the binary threshold hard edge, and proved an eight-way Gaussian schedule with all-depth constant frame conditioning.",
            falsifiers=["Top-level K+2 conditioning does not condition every binary internal node.", "Pairwise shorted-metric comparability is not intrinsic to the polar chain identity.", "Constant physical spectra do not remove block-encoding normalization costs."],
            metrics=["exact_multiway_polar_chain_theorem_count", "multiway_control_count", "multiway_control_failure_count", "binary_interval_uniform_edge_no_go_theorem_count", "mixed_arity_gaussian_all_depth_edge_theorem_count", "mixed_schedule_scaling_count", "mixed_schedule_failure_count", "maximum_merge_arity", "gaussian_all_depth_edge_floor", "gaussian_all_depth_condition_upper", "natural_all_depth_edge_theorem_count", "tight_node_frame_block_encoding_theorem_count", "new_quantum_algorithm_count"],
            dependencies=[
                "self_dual_wreath_multiscale_polar_schedule.py",
            ],
            next_actions=[
                "Run qsearch.py code-wreath-multiscale-polar-schedule.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-NATIVE-FRAME-ACCESS-BOUNDARY",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Exp Code Self Dual Wreath Native Frame Access Boundary",
            status="planned",
            hypothesis="Computed the exact resampling amplitude pair and proved that even a perfectly conditioned frame requires square-root orientation queries under generic normalized access.",
            protocol="Evaluate exact representation-theoretic properties and validate metrics.",
            positive_signal="Computed the exact resampling amplitude pair and proved that even a perfectly conditioned frame requires square-root orientation queries under generic normalized access.",
            falsifiers=["A hard lower edge plus trace truncation is insufficient for generic polynomial QSVT.", "Variable-time amplification does not remove orientation width on native frame inputs.", "The companion GPE pair polar confirms that a representation-specific direct transform can bypass this generic width barrier."],
            metrics=["flat_frame_black_box_lower_bound_theorem_count", "explicit_qrs_amplitude_pair_theorem_count", "variable_time_native_scale_identity_count", "finite_control_count", "flat_control_count", "finite_control_failure_count", "superpolynomial_scaling_row_count", "tail_n", "tail_copy_count", "tail_flat_qrs_lower_bound_log2", "tail_polynomial_benchmark_log2", "representation_specific_polar_transform_count", "companion_gpe_pair_polar_transform_count", "new_quantum_algorithm_count"],
            dependencies=[
                "self_dual_wreath_native_frame_access_boundary.py",
            ],
            next_actions=[
                "Run qsearch.py code-wreath-native-frame-access-boundary.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-NATURAL-LEAF-COMMUTATOR-MASS",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Exp Code Self Dual Wreath Natural Leaf Commutator Mass",
            status="planned",
            hypothesis="Proved inverse-polynomial noncommutativity for a density-one fraction of natural balanced orientation-leaf pairs and isolated the missing canonical-component transfer.",
            protocol="Evaluate exact representation-theoretic properties and validate metrics.",
            positive_signal="Proved inverse-polynomial noncommutativity for a density-one fraction of natural balanced orientation-leaf pairs and isolated the missing canonical-component transfer.",
            falsifiers=["Natural high-dimensional leaf projectors do not become asymptotically commuting on balanced pairs.", "A convergence rate for Sellke covering is unnecessary for density-one pair noncommutativity.", "Leaf noncommutativity does not survive canonical frame whitening under any generic conditioning/aspect/rank implication.", "An inverse-polynomial commutator witness is structural evidence, not a measurement circuit or MRS separation."],
            metrics=["projection_principal_plane_commutator_theorem_count", "finite_control_count", "finite_control_failure_count", "natural_fixed_balanced_leaf_pair_noncommutativity_theorem_count", "natural_density_one_balanced_leaf_pair_noncommutativity_theorem_count", "global_distinct_leaf_noncommutativity_transfer_theorem_count", "tail_n", "tail_commutator_operator_norm_lower_bound", "tail_inverse_commutator_scale", "natural_canonical_component_commutator_mass_theorem_count", "new_quantum_algorithm_count"],
            dependencies=[
                "self_dual_wreath_natural_leaf_commutator_mass.py",
            ],
            next_actions=[
                "Run qsearch.py code-wreath-natural-leaf-commutator-mass.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-NATURAL-PAIR-CARRIER-LAW",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Exp Code Self Dual Wreath Natural Pair Carrier Law",
            status="planned",
            hypothesis="Proved the exact fourth-power natural pair-carrier law, all annealed overlap moments, quenched concentration, and factorial suppression of low-dimensional carrier mass.",
            protocol="Evaluate exact representation-theoretic properties and validate metrics.",
            positive_signal="Proved the exact fourth-power natural pair-carrier law, all annealed overlap moments, quenched concentration, and factorial suppression of low-dimensional carrier mass.",
            falsifiers=["The worst-case 1/(n-1) pair correlation is not representative of multiplicity-weighted natural pair mass.", "Presence of trivial, sign, or hook carriers does not give them nonnegligible natural pair mass.", "Pair carrier concentration alone cannot be promoted to a full-frame edge without controlling coherent higher-order incidence."],
            metrics=["exact_control_count", "exact_control_failure_count", "scaling_record_count", "carrier_law_concentration_trend_count", "tail_n", "tail_expected_weighted_l1_error_upper_bound", "tail_quadratic_dimension_mass_log2", "tail_fourth_power_rms_correlation_log2", "coherent_triangle_incidence_theorem_count", "natural_node_frame_edge_theorem_count", "conditional_active_pair_native_mass_transfer_theorem_count", "new_quantum_algorithm_count"],
            dependencies=[
                "self_dual_wreath_natural_pair_carrier_law.py",
            ],
            next_actions=[
                "Run qsearch.py code-wreath-natural-pair-carrier-law.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-OPERATOR-STEINER-BULK-REDUCTION",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Exp Code Self Dual Wreath Operator Steiner Bulk Reduction",
            status="planned",
            hypothesis="Proved an exact operator-valued Hilbert-Schmidt identity and reduced matrix Steiner bulk conditioning to a lower-edge theorem for the natural diagonal coverage operator.",
            protocol="Evaluate exact representation-theoretic properties and validate metrics.",
            positive_signal="Proved an exact operator-valued Hilbert-Schmidt identity and reduced matrix Steiner bulk conditioning to a lower-edge theorem for the natural diagonal coverage operator.",
            falsifiers=["Arbitrary scalar or nonabelian cycle holonomy cannot by itself create macroscopic bulk outlier rank under good coverage.", "Matrix multiplicity is not intrinsically fatal; uneven or low-rank diagonal coverage is the concrete failure mode.", "Future experiments should measure coverage spectra and the line-fiber-burden/coverage-squared ratio, not cycle signs alone."],
            metrics=["finite_control_count", "finite_control_failure_count", "uniform_scaling_record_count", "tail_multiplicity_one_trim_fraction", "tail_multiplicity_four_trim_fraction", "tail_multiplicity_sixty_four_trim_fraction", "operator_hilbert_schmidt_burden_theorem_count", "coverage_to_bulk_edge_reduction_count", "natural_diagonal_coverage_theorem_count", "new_quantum_algorithm_count"],
            dependencies=[
                "self_dual_wreath_operator_steiner_bulk_reduction.py",
            ],
            next_actions=[
                "Run qsearch.py code-wreath-operator-steiner-bulk-reduction.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-ORIENTATION-FILTER-PHYSICAL-ACCESS",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Exp Code Self Dual Wreath Orientation Filter Physical Access",
            status="planned",
            hypothesis="Proved that every dual contraction composed after the direct synthesis adjoint has at most Theta(1/n!) average conclusive probability, while preserving the separately constructed direct physical branch-interference bypass.",
            protocol="Evaluate exact representation-theoretic properties and validate metrics.",
            positive_signal="Proved that every dual contraction composed after the direct synthesis adjoint has at most Theta(1/n!) average conclusive probability, while preserving the separately constructed direct physical branch-interference bypass.",
            falsifiers=["Polynomial manipulation of orientation Fourier blocks does not by itself yield a polynomial physical measurement.", "Postprocessing the direct dual-analysis map cannot improve its factorially weak average success.", "The direct physical multiplicity-space interference filter must now be tested for all-n retained information and norm."],
            metrics=["direct_physical_to_dual_success_bound_theorem_count", "arbitrary_dual_contraction_bound_theorem_count", "finite_control_count", "finite_validation_failure_count", "scaling_record_count", "inverse_polynomial_direct_access_row_count", "tail_n", "tail_direct_access_success_log2", "tail_amplification_lower_bound_log2", "structured_polar_transfer_count", "direct_physical_orientation_filter_count", "polynomial_hidden_permutation_decoder_count", "new_quantum_algorithm_count"],
            dependencies=[
                "self_dual_wreath_orientation_filter_physical_access.py",
            ],
            next_actions=[
                "Run qsearch.py code-wreath-orientation-filter-physical-access.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-ORIENTATION-RANK-BUDGET",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Exp Code Self Dual Wreath Orientation Rank Budget",
            status="planned",
            hypothesis="Calibrated orientation dimension to the spectral task: Theta(log n) simultaneously matches the universal polynomial-tail rank bound and retains natural-source information with high probability.",
            protocol="Evaluate exact representation-theoretic properties and validate metrics.",
            positive_signal="Calibrated orientation dimension to the spectral task: Theta(log n) simultaneously matches the universal polynomial-tail rank bound and retains natural-source information with high probability.",
            falsifiers=["Near-perfect retention from dim(H)=Omega(k) is not sufficient reason to prefer that regime; its rejection rank is exponentially smaller than trace alone can justify.", "A logarithmic-dimensional H is enough for vanishing natural information loss while preserving polynomial spectral capacity.", "Rank matching is not a norm theorem; structured alignment remains the decisive unresolved obligation."],
            metrics=["trace_high_spectrum_count_theorem_count", "orientation_rejection_rank_theorem_count", "logarithmic_rank_capacity_match_theorem_count", "logarithmic_natural_retention_theorem_count", "finite_control_count", "finite_control_failure_count", "scaling_record_count", "separated_logarithmic_window_count", "legacy_linear_rank_insufficient_from_trace_only_count", "tail_n", "tail_logarithmic_subspace_dimension", "tail_copy_count", "tail_markov_failure_probability_log2_upper_bound", "source_adapted_alignment_theorem_count", "conditioned_polynomial_frame_norm_theorem_count", "polynomial_hidden_permutation_decoder_count", "new_quantum_algorithm_count"],
            dependencies=[
                "self_dual_wreath_orientation_rank_budget.py",
            ],
            next_actions=[
                "Run qsearch.py code-wreath-orientation-rank-budget.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-ORIENTATION-RETENTION-THEOREM",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Exp Code Self Dual Wreath Orientation Retention Theorem",
            status="planned",
            hypothesis="Proved an exact character-ratio rejection formula and an exponentially small Plancherel expectation, yielding gentle constant-information retention for the direct physical filter.",
            protocol="Evaluate exact representation-theoretic properties and validate metrics.",
            positive_signal="Proved an exact character-ratio rejection formula and an exponentially small Plancherel expectation, yielding gentle constant-information retention for the direct physical filter.",
            falsifiers=["The direct physical filter does not pay for common-core rejection by discarding substantial natural source mass.", "Annealed trace retention upgrades to typical natural tuples without an unproved concentration theorem.", "The research bottleneck is now the residual filtered frame and efficient decoding, not filter access or information loss."],
            metrics=["exact_character_rejection_theorem_count", "plancherel_expected_rejection_theorem_count", "quenched_high_probability_retention_theorem_count", "gentle_identification_retention_theorem_count", "exact_physical_control_count", "exact_physical_validation_failure_count", "plancherel_character_control_count", "plancherel_character_validation_failure_count", "scaling_record_count", "high_probability_retention_scaling_row_count", "tail_n", "tail_expected_rejection_log2", "tail_markov_failure_log2_upper_bound", "postfilter_polynomial_frame_norm_theorem_count", "efficient_filtered_povm_count", "polynomial_hidden_permutation_decoder_count", "new_quantum_algorithm_count"],
            dependencies=[
                "self_dual_wreath_orientation_retention_theorem.py",
            ],
            next_actions=[
                "Run qsearch.py code-wreath-orientation-retention-theorem.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-ORIENTATION-SUBSPACE-FILTER",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Exp Code Self Dual Wreath Orientation Subspace Filter",
            status="planned",
            hypothesis="Constructed a coherent orientation-character filter with effect F_H-F_H^2 that exactly removes selected common cores without factorial-scale spectral estimation.",
            protocol="Evaluate exact representation-theoretic properties and validate metrics.",
            positive_signal="Constructed a coherent orientation-character filter with effect F_H-F_H^2 that exactly removes selected common cores without factorial-scale spectral estimation.",
            falsifiers=["The common-core spike can be addressed at unit scale by coherent orientation interference; generic low-pass phase estimation is not the only algebraic route.", "Disjoint branchwise isotypic deletion is unnecessary for the filter identity, so the local-filter no-go does not apply.", "No speedup is claimed until postfilter scaling, retained hidden-label information, and decoding are proved."],
            metrics=["orientation_subspace_fourier_filter_theorem_count", "exact_common_core_annihilation_theorem_count", "finite_control_count", "physical_w4_control_count", "finite_validation_failure_count", "common_range_control_count", "maximum_common_range_dimension", "minimum_retained_trace_fraction", "maximum_retained_trace_fraction", "maximum_common_range_annihilation_residual", "polynomial_controlled_invariant_projector_count", "all_n_postfilter_norm_theorem_count", "polynomial_hidden_permutation_decoder_count", "new_quantum_algorithm_count"],
            dependencies=[
                "self_dual_wreath_orientation_subspace_filter.py",
            ],
            next_actions=[
                "Run qsearch.py code-wreath-orientation-subspace-filter.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-PAIR-COMMON-COVERING-TRANSITION",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Exp Code Self Dual Wreath Pair Common Covering Transition",
            status="planned",
            hypothesis="Proved that pair-common ranges return at a fixed Hamming radius; the later variance theorem resolves distance three for independent Plancherel sources.",
            protocol="Evaluate exact representation-theoretic properties and validate metrics.",
            positive_signal="Proved that pair-common ranges return at a fixed Hamming radius; the later variance theorem resolves distance three for independent Plancherel sources.",
            falsifiers=["Natural pairwise transversality cannot hold through every fixed Hamming radius.", "Sellke's theorem alone does not resolve ordinary three-factor Kronecker positivity; the independent Plancherel variance theorem does.", "Pair-common support alone is not a spectral-edge obstruction."],
            metrics=["sellke_constant_covering_transition_theorem_count", "hamming_three_kronecker_positivity_reduction_theorem_count", "finite_covering_control_count", "finite_covering_control_failure_count", "finite_target_with_common_range_count", "independent_plancherel_hamming_three_positivity_theorem_count", "arbitrarily_coupled_hamming_three_positivity_theorem_count", "natural_node_frame_edge_theorem_count", "new_quantum_algorithm_count"],
            dependencies=[
                "self_dual_wreath_pair_common_covering_transition.py",
            ],
            next_actions=[
                "Run qsearch.py code-wreath-pair-common-covering-transition.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-PAIR-CORE-RANK-CONCENTRATION",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Exp Code Self Dual Wreath Pair Core Rank Concentration",
            status="planned",
            hypothesis="Proved that balanced natural pair cores have uniformly concentrated positive rank and occupy density 1-o(1), invalidating extrapolation from sparse finite live graphs.",
            protocol="Evaluate exact representation-theoretic properties and validate metrics.",
            positive_signal="Proved that balanced natural pair cores have uniformly concentrated positive rank and occupy density 1-o(1), invalidating extrapolation from sparse finite live graphs.",
            falsifiers=["Sparse W6 live pair-core support is pre-asymptotic and cannot justify an all-depth star classification.", "Near-Hamming orientation edges may be absent but have asymptotically negligible pair density.", "Dense pair-core ranks still do not identify residual carrier-channel topology."],
            metrics=["exact_pair_core_rank_factorization_theorem_count", "exact_rank_control_count", "exact_rank_control_failure_count", "finite_sparse_live_graph_control_count", "density_scaling_row_count", "conditioned_concentration_certified_row_count", "finite_conditioned_concentration_onset_n", "tail_n", "tail_copy_count", "tail_balanced_pair_fraction", "tail_log2_conditioned_failure_upper_bound", "natural_live_pair_core_density_one_theorem_count", "dense_residual_channel_theorem_count", "natural_endpoint_comparability_theorem_count", "new_quantum_algorithm_count"],
            dependencies=[
                "self_dual_wreath_pair_core_rank_concentration.py",
            ],
            next_actions=[
                "Run qsearch.py code-wreath-pair-core-rank-concentration.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-PAIR-POLAR-SAMPLER",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Exp Code Self Dual Wreath Pair Polar Sampler",
            status="planned",
            hypothesis="Proved that every pair of wreath orientation invariant projectors has a constant-conditioned polar sampler for n>=5. This rescues one noncommuting merge primitive but leaves recursive angle stability and the nonabelian sieve boundary unresolved.",
            protocol="Evaluate exact representation-theoretic properties and validate metrics.",
            positive_signal="Proved that every pair of wreath orientation invariant projectors has a constant-conditioned polar sampler for n>=5. This rescues one noncommuting merge primitive but leaves recursive angle stability and the nonabelian sieve boundary unresolved.",
            falsifiers=["Do not treat noncommutation or exact common ranges as a barrier to a single orientation-projector merge.", "Do not infer a global sampler from pair conditioning; measure the principal angles of recursively merged ranges first.", "Do not promote a pairwise merge tree before proving it lies outside the Moore--Russell--Sniady sieve model."],
            metrics=["constant_condition_pair_polar_theorem_count", "polynomial_pair_polar_circuit_schema_count", "finite_pair_control_count", "finite_pair_validation_failure_count", "finite_noncommuting_pair_control_count", "maximum_finite_pair_support_condition_number", "tail_n", "tail_support_condition_number_upper_bound", "tail_normalized_analysis_singular_gap_lower_bound", "recursive_angle_stability_theorem_count", "sieve_model_escape_theorem_count", "global_nonorthogonal_sampler_count", "polynomial_hidden_permutation_decoder_count", "new_quantum_algorithm_count"],
            dependencies=[
                "self_dual_wreath_pair_polar_sampler.py",
            ],
            next_actions=[
                "Run qsearch.py code-wreath-pair-polar-sampler.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-PAIR-POLAR-TRANSPORT-NETWORK",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Exp Code Self Dual Wreath Pair Polar Transport Network",
            status="planned",
            hypothesis="Compiled all finite affine fiber transports through paths of at most two pair polars; coherent GPE removes the edge-correlation barrier, leaving path selection, coverage, and holonomy.",
            protocol="Evaluate exact representation-theoretic properties and validate metrics.",
            positive_signal="Compiled all finite affine fiber transports through paths of at most two pair polars; coherent GPE removes the edge-correlation barrier, leaving path selection, coverage, and holonomy.",
            falsifiers=["Direct affine-generator overlap is not necessary; a short pair-overlap path can transport the fiber.", "The stacked pair sampler does not compile pair-overlap transport, but coherent GPE implements that polar without inverse-dimension amplification.", "Fixed W3 diameter and sign gauges do not imply polynomial diameter or abelian gauges at growing multiplicity."],
            metrics=["finite_pair_polar_transport_network_count", "finite_transport_validation_failure_count", "finite_qsvt_correlation_control_failure_count", "maximum_finite_transport_graph_diameter", "minimum_finite_used_edge_correlation", "w3_two_edge_transport_count", "w3_negative_sign_gauge_path_count", "w5_common_range_direct_transport_count", "all_n_polynomial_transport_diameter_theorem_count", "all_n_inverse_polynomial_transport_correlation_theorem_count", "gpe_direct_pair_correlation_bypass_theorem_count", "coherent_transport_path_compiler_count", "hierarchical_orientation_polar_sampler_count", "new_quantum_algorithm_count"],
            dependencies=[
                "self_dual_wreath_pair_polar_transport_network.py",
            ],
            next_actions=[
                "Run qsearch.py code-wreath-pair-polar-transport-network.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-PAIR-TRANSPORT-DEGREE-OBSTRUCTION",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Exp Code Self Dual Wreath Pair Transport Degree Obstruction",
            status="planned",
            hypothesis="Proved an Omega(d_alpha) QSVT degree obstruction for pair-overlap polar transport and isolated low-dimensional fiber alignment or direct recoupling as the only viable bypasses.",
            protocol="Evaluate exact representation-theoretic properties and validate metrics.",
            positive_signal="Proved an Omega(d_alpha) QSVT degree obstruction for pair-overlap polar transport and isolated low-dimensional fiber alignment or direct recoupling as the only viable bypasses.",
            falsifiers=["The constant-conditioned stacked pair sampler does not make cross-overlap polar transport constant-conditioned.", "Finite low-dimensional transport paths do not scale without a carrier-dimension theorem.", "The access-model degree bound is not a general circuit lower bound."],
            metrics=["pair_overlap_polar_bernstein_theorem_count", "finite_control_count", "finite_analytic_control_failure_count", "scaling_record_count", "superpolynomial_worst_carrier_row_count", "tail_n", "tail_log2_maximum_irrep_dimension_lower_bound", "tail_log2_pair_transport_degree_lower_bound", "candidate_high_dimension_fiber_alignment_theorem_count", "explicit_recoupling_bypass_lower_bound_count", "general_quantum_circuit_lower_bound_count", "hierarchical_orientation_polar_sampler_count", "new_quantum_algorithm_count"],
            dependencies=[
                "self_dual_wreath_pair_transport_degree_obstruction.py",
            ],
            next_actions=[
                "Run qsearch.py code-wreath-pair-transport-degree-obstruction.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-PAIR-TRANSPORT-NATIVE-MASS-BOUNDARY",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Exp Code Self Dual Wreath Pair Transport Native Mass Boundary",
            status="planned",
            hypothesis="Transferred the natural d_alpha^4 carrier law exactly to conditional native pair-frame mass and proved that normalized cross-overlap QSVT remains superpolynomial on that mass.",
            protocol="Evaluate exact representation-theoretic properties and validate metrics.",
            positive_signal="Transferred the natural d_alpha^4 carrier law exactly to conditional native pair-frame mass and proved that normalized cross-overlap QSVT remains superpolynomial on that mass.",
            falsifiers=["The natural carrier distribution is not merely an unrelated rank proxy at a conditional active pair node.", "Trace weighting does not rescue normalized cross-overlap access: it emphasizes the high-dimensional, low-correlation carriers.", "This obstruction cannot be promoted to the stacked pair sampler or the complete PGM without changing the operator or proving a global incidence-weight identity."],
            metrics=["fixed_pair_native_trace_theorem_count", "finite_native_trace_control_count", "finite_native_trace_control_failure_count", "natural_q4_native_pair_law_theorem_count", "polynomial_carrier_mass_bound_theorem_count", "branchwise_normalized_qsvt_boundary_theorem_count", "quadratic_cheap_mass_decreasing_trend_count", "tail_n", "tail_quadratic_dimension_mass_log2", "tail_branchwise_degree_lower_bound_log2", "direct_recoupling_transport_theorem_count", "companion_gpe_pair_transport_theorem_count", "complete_pgm_pair_mass_transfer_theorem_count", "new_quantum_algorithm_count"],
            dependencies=[
                "self_dual_wreath_pair_transport_native_mass_boundary.py",
            ],
            next_actions=[
                "Run qsearch.py code-wreath-pair-transport-native-mass-boundary.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-PARTIAL-SUPPORT-CHILD-EMBEDDING",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Exp Code Self Dual Wreath Partial Support Child Embedding",
            status="planned",
            hypothesis="Reconstructed child embeddings from sparse leaf Grams and used a globally distinct S6 affine node to falsify the scalar-fiber compiler, replacing it with a matrix partial-support target.",
            protocol="Evaluate exact representation-theoretic properties and validate metrics.",
            positive_signal="Reconstructed child embeddings from sparse leaf Grams and used a globally distinct S6 affine node to falsify the scalar-fiber compiler, replacing it with a matrix partial-support target.",
            falsifiers=["Affine orientation support does not imply scalar coefficient fibers.", "Global source distinctness does not remove noncommuting recoupling effects.", "Commuting short metrics do not imply commuting mask-resolved component effects.", "The finite W3/W5 scalar affine compiler cannot be extrapolated as the all-depth mechanism.", "The direct S6 trivial/sign pattern cannot supply positive asymptotic natural mass."],
            metrics=["gram_only_child_embedding_reconstruction_theorem_count", "finite_control_count", "finite_control_failure_count", "scalar_affine_positive_control_count", "natural_collision_free_scalar_affine_falsifier_count", "natural_partial_support_common_dimension", "natural_partial_support_maximum_full_scalar_residual", "natural_partial_support_cross_effect_commutator_norm", "matrix_partial_support_gpe_compiler_count", "positive_native_mass_partial_support_theorem_count", "companion_direct_s6_source_mass_no_go_count", "recursive_orientation_polar_sampler_count", "new_quantum_algorithm_count"],
            dependencies=[
                "self_dual_wreath_partial_support_child_embedding.py",
            ],
            next_actions=[
                "Run qsearch.py code-wreath-partial-support-child-embedding.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-PARTIAL-SUPPORT-SOURCE-MASS-BOUNDARY",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Exp Code Self Dual Wreath Partial Support Source Mass Boundary",
            status="planned",
            hypothesis="Proved that the direct S6 scalar-affine falsifier and every polynomial-dimensional-anchor variant have factorially vanishing natural source mass, leaving high-dimensional matrix supports as the only relevant obstruction.",
            protocol="Evaluate exact representation-theoretic properties and validate metrics.",
            positive_signal="Proved that the direct S6 scalar-affine falsifier and every polynomial-dimensional-anchor variant have factorially vanishing natural source mass, leaving high-dimensional matrix supports as the only relevant obstruction.",
            falsifiers=["A finite natural counterexample need not have positive asymptotic source mass.", "The direct S6 matrix-support mechanism is anchored by factorially rare one-dimensional irreps.", "Collision-free conditioning cannot rescue a source event when its own probability tends to one.", "Low-dimensional-anchor exclusion is not a scalar-on-bulk theorem for typical high-dimensional recoupling."],
            metrics=["low_dimension_plancherel_counting_bound_theorem_count", "exact_control_count", "exact_control_failure_count", "direct_s6_trivial_sign_mass_boundary_theorem_count", "polynomial_dimension_anchor_mass_boundary_theorem_count", "tail_n", "tail_log2_conditioned_direct_s6_mass_upper_bound", "tail_log2_conditioned_polynomial_anchor_mass_upper_bound", "finite_direct_s6_n12_benchmark_certified_row_count", "finite_polynomial_anchor_n12_benchmark_certified_row_count", "high_dimension_partial_support_mass_theorem_count", "matrix_partial_support_gpe_compiler_count", "new_quantum_algorithm_count"],
            dependencies=[
                "self_dual_wreath_partial_support_source_mass_boundary.py",
            ],
            next_actions=[
                "Run qsearch.py code-wreath-partial-support-source-mass-boundary.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-BOOLEAN-GRAPH-STOPPING-CORE-PRESSURE",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Exp Code Self Dual Wreath Boolean Graph Stopping Core Pressure",
            status="planned",
            hypothesis="Proved a uniform local-face pressure no-go for every systematic codimension-two stopping core, including arbitrary nonlinear proper four-colorings without a parity factor.",
            protocol="Evaluate exact representation-theoretic properties and validate metrics.",
            positive_signal="Proved a uniform local-face pressure no-go for every systematic codimension-two stopping core, including arbitrary nonlinear proper four-colorings without a parity factor.",
            falsifiers=["Nonlinearity alone does not evade bounded local word obstructions.", "Growing algebraic degree does not force a growing residual word.", "Codimension-two graph-code density does not preserve leading mass.", "A single Boolean face can dequantize an entire scalable family.", "Removing the explicit parity check does not remove every local obstruction."],
            metrics=["all_depth_boolean_graph_core_no_go_theorem_count", "classified_boolean_face_case_count", "face_case_failure_count", "classified_proper_four_color_face_case_count", "exceptional_proper_four_color_face_case_count", "proper_four_color_face_failure_count", "stored_graph_code_control_count", "stored_nonlinear_graph_code_control_count", "stored_proper_four_coloring_control_count", "stored_non_parity_factor_coloring_control_count", "control_failure_count", "maximum_stored_information_width", "maximum_stored_algebraic_degree", "minimum_stored_local_face_pressure_margin", "uniform_true_pressure_margin_lower_bound", "new_quantum_algorithm_count"],
            dependencies=[
                "self_dual_wreath_boolean_graph_stopping_core_pressure.py",
            ],
            next_actions=[
                "Run qsearch.py code-wreath-boolean-graph-stopping-core-pressure.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-COMPONENT-AGGREGATE-FRAME-INDETERMINACY",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Exp Code Self Dual Wreath Component Aggregate Frame Indeterminacy",
            status="planned",
            hypothesis="Proved that identical aggregate frames of all orders can have zero or asymptotically constant component commutator mass.",
            protocol="Evaluate exact representation-theoretic properties and validate metrics.",
            positive_signal="Proved that identical aggregate frames of all orders can have zero or asymptotically constant component commutator mass.",
            falsifiers=["Uncompressed sibling-frame degree-four freeness cannot determine compressed component M_4.", "Even an exact aggregate resolvent cannot determine the component algebra.", "The missing theorem must retain individual orientation leaves through normalization and common-span compression.", "The mutually unbiased constant is a generic counterexample, not natural wreath evidence."],
            metrics=["aggregate_frame_all_order_indeterminacy_theorem_count", "mutually_unbiased_constant_component_gap_theorem_count", "finite_control_count", "finite_control_failure_count", "tail_dimension", "tail_noncommuting_normalized_gap", "limiting_noncommuting_normalized_gap", "natural_leaf_resolved_compressed_M4_theorem_count", "new_quantum_algorithm_count"],
            dependencies=[
                "self_dual_wreath_component_aggregate_frame_indeterminacy.py",
            ],
            next_actions=[
                "Run qsearch.py code-wreath-component-aggregate-frame-indeterminacy.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-COMPONENT-COMMUTATOR-COLLISION-FREE-TRANSFER",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Exp Code Self Dual Wreath Component Commutator Collision Free Transfer",
            status="planned",
            hypothesis="Proved the sharp bounded-variable transfer of component M4 through global-distinct conditioning and reduced the natural mass problem to the independent-Plancherel Green gap alone.",
            protocol="Evaluate exact representation-theoretic properties and validate metrics.",
            positive_signal="Proved the sharp bounded-variable transfer of component M4 through global-distinct conditioning and reduced the natural mass problem to the independent-Plancherel Green gap alone.",
            falsifiers=["A positive expectation cannot be transferred by dividing by collision-free mass; the sharp lower bound subtracts off-event mass.", "Finite n<=48 collision-free rows remain too preasymptotic to certify the illustrative moment scale.", "Any constant or inverse-polynomial independent M4 does survive asymptotically because the collision error is n^{-omega(1)}.", "No independent natural M4, compiler, decoder, or speedup is proved."],
            metrics=["sharp_bounded_variable_conditioning_theorem_count", "inverse_polynomial_M4_collision_free_transfer_theorem_count", "sharpness_control_count", "sharpness_control_failure_count", "finite_scaling_row_count", "finite_nonvacuous_transfer_row_count", "tail_n", "tail_global_distinct_probability", "natural_independent_plancherel_M4_lower_bound_theorem_count", "new_quantum_algorithm_count"],
            dependencies=[
                "self_dual_wreath_component_commutator_collision_free_transfer.py",
            ],
            next_actions=[
                "Run qsearch.py code-wreath-component-commutator-collision-free-transfer.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-COMPONENT-COMMUTATOR-HAAR-BENCHMARK",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Exp Code Self Dual Wreath Component Commutator Haar Benchmark",
            status="planned",
            hypothesis="Derived the exact finite complex-Haar component commutator gap and showed its sparse-outcome limit remains constant at fixed aspect.",
            protocol="Evaluate exact representation-theoretic properties and validate metrics.",
            positive_signal="Derived the exact finite complex-Haar component commutator gap and showed its sparse-outcome limit remains constant at fixed aspect.",
            falsifiers=["Outcome-count growth alone does not suppress the component commutator gap in the Haar model.", "The exact formula respects the one-dimensional and two-outcome commuting boundaries.", "The positive Haar constant is a quantitative benchmark, not natural wreath evidence.", "A natural zero gap would require a representation-specific crossing-moment collapse that must be exhibited explicitly."],
            metrics=["exact_complex_haar_commutator_formula_theorem_count", "sparse_block_positive_haar_limit_theorem_count", "exact_control_count", "exact_control_failure_count", "natural_aspect_sparse_haar_normalized_gap", "surrogate_total_regular_master_M4_scale", "surrogate_physical_support_scale", "natural_compressed_component_M4_lower_bound_theorem_count", "new_quantum_algorithm_count"],
            dependencies=[
                "self_dual_wreath_component_commutator_haar_benchmark.py",
            ],
            next_actions=[
                "Run qsearch.py code-wreath-component-commutator-haar-benchmark.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-COMPONENT-COMMUTATOR-TRACE-MASS-BRIDGE",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Exp Code Self Dual Wreath Component Commutator Trace Mass Bridge",
            status="planned",
            hypothesis="Reduced natural noncommutative component support to one physically normalized scalar fourth-moment gap with no edge or center-valued rank premise.",
            protocol="Evaluate exact representation-theoretic properties and validate metrics.",
            positive_signal="Reduced natural noncommutative component support to one physically normalized scalar fourth-moment gap with no edge or center-valued rank premise.",
            falsifiers=["Nonscalarity and non-reciprocal spectra do not force a positive commutator moment gap.", "A scalar commutator fourth-moment lower bound is not diluted away from physical support; half of it transfers universally.", "Uncompressed sibling-frame degree-four freeness is not the missing compressed-component calculation.", "No natural M_4 lower bound, component compiler, MRS separation, decoder, or speedup is proved."],
            metrics=["exact_component_commutator_fourth_moment_identity_theorem_count", "universal_commutator_operator_bound_theorem_count", "scalar_moment_to_physical_support_bridge_theorem_count", "finite_control_count", "finite_control_failure_count", "trine_normalized_fourth_moment_gap", "natural_compressed_component_M4_lower_bound_theorem_count", "natural_noncommutative_component_physical_mass_theorem_count", "new_quantum_algorithm_count"],
            dependencies=[
                "self_dual_wreath_component_commutator_trace_mass_bridge.py",
            ],
            next_actions=[
                "Run qsearch.py code-wreath-component-commutator-trace-mass-bridge.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-COMPONENT-GREEN-RIDGE-STABILITY",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Exp Code Self Dual Wreath Component Green Ridge Stability",
            status="planned",
            hypothesis="Proved leaf-count-independent stability of component commutator mass and reduced the natural Green pseudoinverse to a bounded ridge calculation plus one scalar trace-weighted polar ratio.",
            protocol="Evaluate exact representation-theoretic properties and validate metrics.",
            positive_signal="Proved leaf-count-independent stability of component commutator mass and reduced the natural Green pseudoinverse to a bounded ridge calculation plus one scalar trace-weighted polar ratio.",
            falsifiers=["A naive q or q^2 approximation loss is not intrinsic for normalized projection syntheses.", "Uniform operator-norm frame edges are sufficient but not necessary; the exact average target is the trace-weighted polar ratio R_eta.", "Ridge normalization still depends on the sibling common metric and is not automatically stable.", "No natural ridge gap, average approximation estimate, compiler, decoder, or speedup is proved."],
            metrics=["outcome_count_free_component_M4_stability_theorem_count", "green_ridge_synthesis_perturbation_theorem_count", "average_ridge_to_exact_M4_transfer_theorem_count", "trace_weighted_polar_ridge_transfer_theorem_count", "povm_control_count", "povm_control_failure_count", "ridge_control_count", "ridge_control_failure_count", "natural_average_synthesis_error_theorem_count", "natural_positive_ridge_M4_theorem_count", "new_quantum_algorithm_count"],
            dependencies=[
                "self_dual_wreath_component_green_ridge_stability.py",
            ],
            next_actions=[
                "Run qsearch.py code-wreath-component-green-ridge-stability.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-COMPONENT-HAMMING-ORBIT-REDUCTION",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Exp Code Self Dual Wreath Component Hamming Orbit Reduction",
            status="planned",
            hypothesis="Used exact source-relabeling symmetry to collapse the annealed final-root component M4 to a binomial average of q^2-rescaled typical-Hamming pair commutators.",
            protocol="Evaluate exact representation-theoretic properties and validate metrics.",
            positive_signal="Used exact source-relabeling symmetry to collapse the annealed final-root component M4 to a binomial average of q^2-rescaled typical-Hamming pair commutators.",
            falsifiers=["Blockwise orientation translation covariance is false and is not needed for the annealed scalar reduction.", "The exponential leaf-pair count collapses to K-1 Hamming strata under the natural source law.", "The decisive individual pair scale is q^-2, not constant or q^-1.", "No positive typical-pair gap, exact natural M4, compiler, decoder, or speedup is proved."],
            metrics=["annealed_component_hamming_orbit_reduction_theorem_count", "binomial_rescaled_pair_M4_identity_theorem_count", "global_distinct_hamming_profile_transfer_theorem_count", "physical_control_count", "physical_control_failure_count", "relabeling_control_count", "relabeling_control_failure_count", "natural_typical_hamming_pair_gap_theorem_count", "new_quantum_algorithm_count"],
            dependencies=[
                "self_dual_wreath_component_hamming_orbit_reduction.py",
            ],
            next_actions=[
                "Run qsearch.py code-wreath-component-hamming-orbit-reduction.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-COMPONENT-LEAF-RESOLVED-GREEN-NORMAL-FORM",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Exp Code Self Dual Wreath Component Leaf Resolved Green Normal Form",
            status="planned",
            hypothesis="Derived the exact leaf-marked Green-function trace word whose natural AABB-minus-ABAB gap would prove component noncommutative mass.",
            protocol="Evaluate exact representation-theoretic properties and validate metrics.",
            positive_signal="Derived the exact leaf-marked Green-function trace word whose natural AABB-minus-ABAB gap would prove component noncommutative mass.",
            falsifiers=["Aggregate frame moments and resolvents omit the leaf labels required by component M4.", "The sibling common projection and child pseudoinverse remain inside the exact Green kernel.", "A positive Haar gap does not evaluate the natural leaf-marked Green word.", "No natural gap, globally distinct transfer, compiler, decoder, or speedup is proved."],
            metrics=["leaf_resolved_green_word_normal_form_theorem_count", "component_M4_green_AABB_minus_ABAB_identity_theorem_count", "regular_master_subgroup_walk_green_reduction_theorem_count", "finite_control_count", "finite_control_failure_count", "natural_leaf_marked_green_gap_theorem_count", "natural_compressed_component_M4_lower_bound_theorem_count", "new_quantum_algorithm_count"],
            dependencies=[
                "self_dual_wreath_component_leaf_resolved_green_normal_form.py",
            ],
            next_actions=[
                "Run qsearch.py code-wreath-component-leaf-resolved-green-normal-form.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-CONTIGUOUS-ALL-A-SUPPORT-PRESSURE",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Exp Code Self Dual Wreath Contiguous All A Support Pressure",
            status="planned",
            hypothesis="Removed the zero-base and linear-support restrictions for scalar pressure by combining exact two-fiber entropy and integer generator bounds with a uniform outer conjugacy-fiber count.",
            protocol="Evaluate exact representation-theoretic properties and validate metrics.",
            positive_signal="Removed the zero-base and linear-support restrictions for scalar pressure by combining exact two-fiber entropy and integer generator bounds with a uniform outer conjugacy-fiber count.",
            falsifiers=["Nonzero bases need not preserve the zero-base presentation topology.", "The maximum fiber entropy, not the sum, is the available scalar loss.", "Unsigned homomorphism counts do not certify target-character signal.", "No conclusion transfers automatically to B or interleaved word families."],
            metrics=["checked_arbitrary_support_pair_count_through_width_three", "arbitrary_support_pair_control_failure_count", "finite_S3_control_count", "finite_S3_control_failure_count", "minimum_crossing_pressure_margin", "growing_width_arbitrary_support_scalar_pressure_theorem_count", "integer_suffix_branch_pressure_strengthening_theorem_count", "mixed_target_character_theorem_count", "new_quantum_algorithm_count"],
            dependencies=[
                "self_dual_wreath_contiguous_all_a_support_pressure.py",
            ],
            next_actions=[
                "Run qsearch.py code-wreath-contiguous-all-a-support-pressure.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-CONTIGUOUS-FRAME-TARGET-FACTORIZATION",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Exp Code Self Dual Wreath Contiguous Frame Target Factorization",
            status="planned",
            hypothesis="Extended arbitrary-support scalar pressure to mixed contiguous frames and reduced every surviving target character exactly to a weighted frame-word character problem; scalar saturation itself now provably forces target identity.",
            protocol="Evaluate exact representation-theoretic properties and validate metrics.",
            positive_signal="Extended arbitrary-support scalar pressure to mixed contiguous frames and reduced every surviving target character exactly to a weighted frame-word character problem; scalar saturation itself now provably forces target identity.",
            falsifiers=["Outer elimination preserves a nonuniform centralizer weight.", "Genus-zero split patterns cannot carry a nontrivial target.", "Additional support relations can erase split-only target topology.", "Identity-frame lifts only make the obsolete real entropy margin vanish.", "The integer suffix-branch bound restores a uniform gap on that lift.", "Power-boundary pruning defeats the generic bound but not the true presentation.", "The S3 lift does not establish asymptotic S_n solution mass.", "No result here supplies a positive component moment or decoder."],
            metrics=["checked_frame_type_base_pair_count", "outer_factorization_failure_count", "target_factorization_failure_count", "maximum_split_target_genus", "representative_selected_target_survival_count", "growing_width_mixed_frame_scalar_pressure_theorem_count", "exact_target_to_frame_character_reduction_theorem_count", "target_survival_strict_pressure_tradeoff_theorem_count", "growing_width_uniform_target_survival_pressure_gap_theorem_count", "exact_S3_target_survival_identity_frame_lift_count", "target_survival_lift_failure_count", "minimum_target_survival_lift_pressure_margin", "minimum_real_entropy_lift_certificate_margin", "real_entropy_certificate_gap_falsified_count", "integer_suffix_branch_identity_lift_gap_closed_count", "uniform_certificate_pressure_gap_falsified_count", "power_boundary_integer_certificate_gap_falsified_count", "power_boundary_actual_presentation_escape_count", "weighted_frame_character_theorem_count", "new_quantum_algorithm_count"],
            dependencies=[
                "self_dual_wreath_contiguous_frame_target_factorization.py",
            ],
            next_actions=[
                "Run qsearch.py code-wreath-contiguous-frame-target-factorization.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-EXCEPTIONAL-BLOCK-GRAPH-CORE-PRESSURE",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Exp Code Self Dual Wreath Exceptional Block Graph Core Pressure",
            status="planned",
            hypothesis="Classified the globally all-cancelling graph code as a fixed genus-two law times free check generators and proved it subleading.",
            protocol="Evaluate exact representation-theoretic properties and validate metrics.",
            positive_signal="Classified the globally all-cancelling graph code as a fixed genus-two law times free check generators and proved it subleading.",
            falsifiers=["All local face cancellations do not remove the base surface law.", "Growing free check rank does not create pressure gain.", "Reverse block repetition is not an actual power-boundary escape."],
            metrics=["all_depth_exceptional_block_graph_no_go_theorem_count", "stored_block_graph_control_count", "control_failure_count", "maximum_stored_information_width", "maximum_stored_check_width", "minimum_stored_true_pressure_margin", "uniform_true_pressure_margin_lower_bound", "new_quantum_algorithm_count"],
            dependencies=[
                "self_dual_wreath_exceptional_block_graph_core_pressure.py",
            ],
            next_actions=[
                "Run qsearch.py code-wreath-exceptional-block-graph-core-pressure.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-FRAME-SUBWORD-ENTROPY",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Exp Code Self Dual Wreath Frame Subword Entropy",
            status="planned",
            hypothesis="Reduced zero-based contiguous all-A support profiles to ordered frame-subword groups and proved the entropy bound at every width by exact XOR re-rooting, entropy induction, and an explicit integer suffix-branch generator theorem.",
            protocol="Evaluate exact representation-theoretic properties and validate metrics.",
            positive_signal="Reduced zero-based contiguous all-A support profiles to ordered frame-subword groups and proved the entropy bound at every width by exact XOR re-rooting, entropy induction, and an explicit integer suffix-branch generator theorem.",
            falsifiers=["Edge/private-pivot metrics are incomplete even when pressure is safe.", "Finite coverage is not the all-u proof; the re-rooting induction is.", "The real entropy exponent is not optimal for non-power-of-two supports.", "The reduction requires zero in the different-coordinate support.", "Mixed B boundaries and target characters remain outside scope."],
            metrics=["exhaustively_checked_zero_support_count_through_width_four", "exhaustive_pressure_failure_count_through_width_four", "width_five_stress_support_count", "width_five_stress_failure_count", "conditional_frame_subword_pressure_theorem_count", "complete_width_four_zero_based_pressure_theorem_count", "growing_width_frame_subword_entropy_theorem_count", "growing_width_suffix_branch_generator_theorem_count", "nonzero_base_pressure_theorem_count", "mixed_target_character_theorem_count", "new_quantum_algorithm_count"],
            dependencies=[
                "self_dual_wreath_frame_subword_entropy.py",
            ],
            next_actions=[
                "Run qsearch.py code-wreath-frame-subword-entropy.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-LEAF-MARKED-GREEN-WORD-NORMAL-FORM",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Exp Code Self Dual Wreath Leaf Marked Green Word Normal Form",
            status="planned",
            hypothesis="Reduced the exact natural sibling-common Green pair functional to partially pinned A/B word-map strata and removed the common span as a separate probabilistic input.",
            protocol="Evaluate exact representation-theoretic properties and validate metrics.",
            positive_signal="Reduced the exact natural sibling-common Green pair functional to partially pinned A/B word-map strata and removed the common span as a separate probabilistic input.",
            falsifiers=["The exact common span is not an independent random object beyond the sibling frames.", "Leaf marking does not require enumerating all q leaves; Hamming distance leaves two coordinate counts.", "All-left marked words are exactly target independent.", "No positive normalized Green pair gap or component M4 follows without growing-word control."],
            metrics=["exact_leaf_marked_word_normal_form_theorem_count", "exact_hamming_two_count_reduction_theorem_count", "all_left_target_character_cancellation_theorem_count", "basis_free_common_green_algebra_theorem_count", "direct_moment_control_count", "direct_moment_control_failure_count", "uncompressed_pair_control_count", "uncompressed_pair_control_failure_count", "green_algebra_control_count", "green_algebra_control_failure_count", "tail_n", "tail_uncompressed_q_squared_rescaled_pair_trace", "growing_marked_word_rigidity_theorem_count", "natural_positive_green_pair_gap_theorem_count", "new_quantum_algorithm_count"],
            dependencies=[
                "self_dual_wreath_leaf_marked_green_word_normal_form.py",
            ],
            next_actions=[
                "Run qsearch.py code-wreath-leaf-marked-green-word-normal-form.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-LINEAR-CODE-SUPPORT-PRESSURE",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Exp Code Self Dual Wreath Linear Code Support Pressure",
            status="planned",
            hypothesis="Proved an exact all-degree crossing-pressure bound for contiguous all-A words with binary linear-code supports by RREF Tietze elimination and an F_(u-r)*Z^2 factorization.",
            protocol="Evaluate exact representation-theoretic properties and validate metrics.",
            positive_signal="Proved an exact all-degree crossing-pressure bound for contiguous all-A words with binary linear-code supports by RREF Tietze elimination and an F_(u-r)*Z^2 factorization.",
            falsifiers=["The theorem does not justify a rank-to-S_n lift for arbitrary supports.", "Affine translates need not contain linear RREF codewords.", "B frames and surviving mixed target characters are outside scope.", "No positive component moment, decoder, or quantum speedup follows."],
            metrics=["checked_linear_code_pair_count", "linear_code_pair_certificate_failure_count", "minimum_crossing_pressure_margin", "finite_S3_control_count", "finite_S3_control_failure_count", "growing_degree_linear_code_pressure_theorem_count", "arbitrary_support_pressure_theorem_count", "mixed_target_character_theorem_count", "new_quantum_algorithm_count"],
            dependencies=[
                "self_dual_wreath_linear_code_support_pressure.py",
            ],
            next_actions=[
                "Run qsearch.py code-wreath-linear-code-support-pressure.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-MARKED-PRESSURE-OBSTRUCTION-SEARCH",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Exp Code Self Dual Wreath Marked Pressure Obstruction Search",
            status="planned",
            hypothesis="Localized growing-degree marked-word pressure proof debt with deterministic best-first support mutation and exact presentation records.",
            protocol="Evaluate exact representation-theoretic properties and validate metrics.",
            positive_signal="Localized growing-degree marked-word pressure proof debt with deterministic best-first support mutation and exact presentation records.",
            falsifiers=["Literal word shape is not stable under free-group automorphisms; Nielsen certificates are required.", "Finite S3 solution exponents cannot promote an unresolved presentation into a counterexample.", "A beam search cannot prove a universal growing-degree pressure theorem.", "Marked-word degree alone cannot suppress a hard core because exact constant-zero lifts preserve pressure.", "Genus-zero mixed finalists conceal the target-character frontier; searches must retain separated B runs and support-uncancelled boundaries.", "Stopping at the requested pressure threshold can hide a stronger surface exponent and mis-rank the obstruction."],
            metrics=["searched_profile_count", "search_run_count", "unresolved_finalist_count", "constant_zero_trailing_lift_control_count", "constant_zero_trailing_lift_failure_count", "constant_zero_trailing_lift_theorem_count", "degree_five_reducible_finalist_count", "degree_six_reducible_finalist_count", "degree_six_positive_literal_pressure_profile_count", "mixed_frame_positive_split_genus_finalist_count", "mixed_frame_unresolved_scalar_pressure_finalist_count", "mixed_frame_nonzero_finite_character_bias_finalist_count", "mixed_frame_maximum_finite_character_bias", "mixed_frame_symbolically_cancelled_target_finalist_count", "mixed_frame_symbolically_uncancelled_target_finalist_count", "mixed_frame_maximum_strong_pressure_excess", "mixed_frame_full_exponent_margin_finalist_count", "residual_target_cancellation_control_count", "residual_target_cancellation_control_failure_count", "cyclic_relator_target_cancellation_theorem_count", "two_partition_ribbon_improved_finalist_count", "maximum_strong_pressure_excess", "asymptotic_counterexample_count", "growing_degree_pressure_theorem_count", "natural_positive_green_pair_gap_theorem_count", "new_quantum_algorithm_count"],
            dependencies=[
                "self_dual_wreath_marked_pressure_obstruction_search.py",
            ],
            next_actions=[
                "Run qsearch.py code-wreath-marked-pressure-obstruction-search.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-MARKED-RELATION-TOPOLOGY",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Exp Code Self Dual Wreath Marked Relation Topology",
            status="planned",
            hypothesis="Compressed fixed-degree marked moments from source-coordinate sequences to support-profile group presentations, recovered the free-versus-commutator crossing topology exactly, and certified the expected pressure gap for every profile through degree two.",
            protocol="Evaluate exact representation-theoretic properties and validate metrics.",
            positive_signal="Compressed fixed-degree marked moments from source-coordinate sequences to support-profile group presentations, recovered the free-versus-commutator crossing topology exactly, and certified the expected pressure gap for every profile through degree two.",
            falsifiers=["Copy-depth growth is not the remaining combinatorial bottleneck at fixed marked-word degree.", "The crossing correction is nonabelian: abelianized relation rank misses its commutator residual.", "Support-profile compression remains double exponential in growing frame-token degree.", "The first degree-three literal-classifier failures are hidden surface, commutator, or centralized-torsion presentations, not pressure counterexamples.", "Those four degree-three controls are adversarial witnesses, not an exhaustive degree-three theorem.", "No positive Green pair gap, component M4, compiler, decoder, or speedup is proved."],
            metrics=["exact_support_profile_expansion_theorem_count", "fixed_degree_copy_depth_compression_theorem_count", "solution_preserving_tietze_reduction_theorem_count", "uncompressed_noncrossing_free_pair_topology_theorem_count", "uncompressed_crossing_commutator_topology_theorem_count", "support_expansion_control_count", "support_expansion_control_failure_count", "relation_topology_control_count", "relation_topology_control_failure_count", "fixed_degree_pressure_control_count", "one_frame_token_complete_pressure_separation_theorem_count", "two_frame_token_unclassified_profile_count", "two_frame_token_pressure_uncertified_profile_count", "two_frame_token_pressure_separation_theorem_count", "two_frame_token_free_basis_commutator_certificate_count", "degree_three_adversarial_pressure_control_count", "degree_three_adversarial_pressure_failure_count", "centralized_primitive_cube_presentation_theorem_count", "noncrossing_S3_solution_count", "crossing_S3_solution_count", "growing_degree_profile_classification_theorem_count", "natural_positive_green_pair_gap_theorem_count", "new_quantum_algorithm_count"],
            dependencies=[
                "self_dual_wreath_marked_relation_topology.py",
            ],
            next_actions=[
                "Run qsearch.py code-wreath-marked-relation-topology.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-MIXED-SPLIT-TARGET-GENUS",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Exp Code Self Dual Wreath Mixed Split Target Genus",
            status="planned",
            hypothesis="Reduced mixed target characters to a cyclic-run surface genus and localized the remaining obstruction to support-conditioned handles.",
            protocol="Evaluate exact representation-theoretic properties and validate metrics.",
            positive_signal="Reduced mixed target characters to a cyclic-run surface genus and localized the remaining obstruction to support-conditioned handles.",
            falsifiers=["Separated B runs leave commutator handles, so mixed target cancellation is not universal.", "Finite character screens cannot replace the relative surface theorem with support relations."],
            metrics=["checked_split_pattern_count", "split_target_genus_control_failure_count", "maximum_verified_orientable_genus", "finite_character_control_failure_count", "new_quantum_algorithm_count"],
            dependencies=[
                "self_dual_wreath_mixed_split_target_genus.py",
            ],
            next_actions=[
                "Run qsearch.py code-wreath-mixed-split-target-genus.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-NATURAL-LEAF-COMMUTATOR-TRACE-PROFILE",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Exp Code Self Dual Wreath Natural Leaf Commutator Trace Profile",
            status="planned",
            hypothesis="Proved the exact Theta(q^-2) natural leaf-pair commutator trace, its quenched density-one balanced-pair profile, and constant aggregate all-distinct leaf trace mass.",
            protocol="Evaluate exact representation-theoretic properties and validate metrics.",
            positive_signal="Proved the exact Theta(q^-2) natural leaf-pair commutator trace, its quenched density-one balanced-pair profile, and constant aggregate all-distinct leaf trace mass.",
            falsifiers=["The natural leaf commutator witness is not confined to negligible trace rank.", "The tiny annealed pair expectation is not supported only on rare source collisions.", "The pair trace exactly matches the q^-2 scale required by the Hamming component reduction.", "No generic or natural Green-normalization transfer, component M4, compiler, decoder, or speedup is proved."],
            metrics=["exact_natural_leaf_pair_trace_formula_theorem_count", "quenched_density_one_leaf_trace_profile_theorem_count", "constant_aggregate_uncompressed_leaf_trace_theorem_count", "global_distinct_leaf_trace_density_transfer_theorem_count", "exact_control_count", "exact_control_failure_count", "tail_n", "tail_q_squared_rescaled_pair_trace", "tail_aggregate_leaf_trace_lower_bound", "natural_component_M4_lower_bound_theorem_count", "new_quantum_algorithm_count"],
            dependencies=[
                "self_dual_wreath_natural_leaf_commutator_trace_profile.py",
            ],
            next_actions=[
                "Run qsearch.py code-wreath-natural-leaf-commutator-trace-profile.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-PARITY-STOPPING-CORE-PRESSURE",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Exp Code Self Dual Wreath Parity Stopping Core Pressure",
            status="planned",
            hypothesis="Constructed the maximal-density nonpeelable parity core, preserved its finite target signal, and proved it uniformly subleading.",
            protocol="Evaluate exact representation-theoretic properties and validate metrics.",
            positive_signal="Constructed the maximal-density nonpeelable parity core, preserved its finite target signal, and proved it uniformly subleading.",
            falsifiers=["Avoiding support-difference peeling is not enough.", "Near-zero generic pressure margin is not an actual presentation escape.", "Persistent S3 standard bias does not imply asymptotic leading mass.", "Maximal even-parity stopping cores collapse to one involution.", "Odd-parity and support-swapped extremal cores do not escape.", "Every single-fiber maximum-density stopping core is a parity coset."],
            metrics=["all_depth_parity_stopping_core_no_go_theorem_count", "stored_nonpeelable_core_control_count", "stored_extremal_parity_coset_control_count", "hypercube_rigidity_control_count", "control_failure_count", "maximum_independent_set_rigidity_failure_count", "maximum_stored_core_width", "minimum_stored_generic_integer_margin", "minimum_stored_involution_pressure_margin", "uniform_true_pressure_margin_lower_bound", "finite_S3_standard_bias_control_count", "new_quantum_algorithm_count"],
            dependencies=[
                "self_dual_wreath_parity_stopping_core_pressure.py",
            ],
            next_actions=[
                "Run qsearch.py code-wreath-parity-stopping-core-pressure.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-PERIODIC-FRAME-FIBER-COUNTERFAMILY",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Exp Code Self Dual Wreath Periodic Frame Fiber Counterfamily",
            status="planned",
            hypothesis="Constructed an exact nonidentity periodic frame family with vanishing support-only margin, then proved its full infinite subsequence uniformly subleading by suffix-branch rank collapse.",
            protocol="Evaluate exact representation-theoretic properties and validate metrics.",
            positive_signal="Constructed an exact nonidentity periodic frame family with vanishing support-only margin, then proved its full infinite subsequence uniformly subleading by suffix-branch rank collapse.",
            falsifiers=["Identity padding is not the only nonuniform-margin mechanism.", "Boolean support growth does not establish group-solution growth.", "Finite S3 target survival does not establish normalized S_n signal.", "The first four full presentations have stronger loss than the support bound.", "The entire k=6m+1 family has at most three frame generators."],
            metrics=["exact_periodic_transfer_theorem_count", "infinite_nonidentity_margin_vanishing_counterfamily_count", "stored_period_scaling_count", "stored_vanishing_margin_period_count", "minimum_stored_scalar_certificate_margin", "materialized_presentation_control_count", "support_bound_tight_presentation_count", "all_period_frame_rank_collapse_theorem_count", "uniform_subleading_Sn_mass_theorem_count", "asymptotic_Sn_mass_theorem_count", "nonvanishing_Sn_target_character_theorem_count", "new_quantum_algorithm_count"],
            dependencies=[
                "self_dual_wreath_periodic_frame_fiber_counterfamily.py",
            ],
            next_actions=[
                "Run qsearch.py code-wreath-periodic-frame-fiber-counterfamily.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-HIGH-CODIMENSION-FACE-WORD-FRONTIER",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Exp Code Self Dual Wreath High Codimension Face Word Frontier",
            status="planned",
            hypothesis="Reduced high-codimension systematic stopping-code proof debt to uniform S_n control of cubic-overlap face words.",
            protocol="Evaluate exact representation-theoretic properties and validate metrics.",
            positive_signal="Reduced high-codimension systematic stopping-code proof debt to uniform S_n control of cubic-overlap face words.",
            falsifiers=["Growing primitive-power degree cannot arise from one face.", "Reverse block-union is the only freely trivial face mechanism.", "Singleton and quadratic incidence are already uniformly subleading."],
            metrics=["high_power_escape_exclusion_theorem_count", "maximum_face_word_generator_occurrence", "maximum_possible_proper_power_degree", "stored_scaling_width_count", "maximum_exhaustive_check_width", "stored_cubic_overlap_representative_count", "width_three_cubic_finite_control_failure_count", "certificate_failure_count", "new_quantum_algorithm_count"],
            dependencies=[
                "self_dual_wreath_high_codimension_face_word_frontier.py",
            ],
            next_actions=[
                "Run qsearch.py code-wreath-high-codimension-face-word-frontier.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-PERIODIC-FRAME-RANK-COLLAPSE",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Exp Code Self Dual Wreath Periodic Frame Rank Collapse",
            status="planned",
            hypothesis="Closed the periodic nonidentity family by proving a universal three-generator frame bound and uniform scalar-pressure loss.",
            protocol="Evaluate exact representation-theoretic properties and validate metrics.",
            positive_signal="Closed the periodic nonidentity family by proving a universal three-generator frame bound and uniform scalar-pressure loss.",
            falsifiers=["Vanishing support-size margins need not survive presentation structure.", "Exponential automaton fibers need not produce leading group-solution mass.", "Finite nonidentity target bias is irrelevant after uniform scalar suppression."],
            metrics=["all_period_suffix_branch_rank_theorem_count", "universal_frame_generator_upper_bound", "full_Sn_solution_exponent_upper_bound", "minimum_checked_scalar_pressure_margin", "proved_uniform_scalar_pressure_margin_lower_bound", "periodic_family_leading_Sn_mass_count", "periodic_family_surviving_candidate_count", "new_quantum_algorithm_count"],
            dependencies=[
                "self_dual_wreath_periodic_frame_rank_collapse.py",
            ],
            next_actions=[
                "Run qsearch.py code-wreath-periodic-frame-rank-collapse.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-PETZ-PGM-OBSTRUCTION",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Exp Code Self Dual Wreath Petz Pgm Obstruction",
            status="planned",
            hypothesis="Mapped the standard PGM-to-Petz implementation to the wreath ensemble and isolated its independent sqrt(n!) environment-dimension charge.",
            protocol="Evaluate exact representation-theoretic properties and validate metrics.",
            positive_signal="Mapped the standard PGM-to-Petz implementation to the wreath ensemble and isolated its independent sqrt(n!) environment-dimension charge.",
            falsifiers=["A generic Petz implementation does not turn constant PGM success into a polynomial algorithm for n! hypotheses.", "The factorial charge is not caused by label storage: it arises from generic Stinespring normalization/amplification.", "The primary theorem leaves group-covariant factorization open, so it cannot be cited as a structured impossibility result."],
            metrics=["generic_pgm_to_petz_reduction_count", "generic_environment_dimension_obstruction_count", "complementary_rank_bypass_failure_count", "scaling_record_count", "superpolynomial_generic_charge_record_count", "maximum_n", "maximum_log2_generic_environment_sqrt_charge", "algebraic_covariance_compressed_factorization_count", "polynomial_covariance_compressed_stinespring_circuit_count", "structured_multiplicity_whitening_count", "new_quantum_algorithm_count"],
            dependencies=[
                "self_dual_wreath_petz_pgm_obstruction.py",
            ],
            next_actions=[
                "Run qsearch.py code-wreath-petz-pgm-obstruction.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-PGM-QUANTUM-SAMPLING-REDUCTION",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Exp Code Self Dual Wreath Pgm Quantum Sampling Reduction",
            status="planned",
            hypothesis="Reduced the remaining wreath PGM operation to a controlled polar isometry and its orientation form to nonorthogonal quantum sampling. Known-fiber controls show that tiny singular scale is not inherently hard, while exact S4 sectors rule out the direct commuting-fiber strategy used by successful semidirect HSP PGMs.",
            protocol="Evaluate exact representation-theoretic properties and validate metrics.",
            positive_signal="Reduced the remaining wreath PGM operation to a controlled polar isometry and its orientation form to nonorthogonal quantum sampling. Known-fiber controls show that tiny singular scale is not inherently hard, while exact S4 sectors rule out the direct commuting-fiber strategy used by successful semidirect HSP PGMs.",
            falsifiers=["Do not cite factorial eigenvalue scale alone as evidence that the PGM is computationally hard.", "Do not mutate classical matrix-sum or subset-sum fiber samplers without handling noncommuting invariant subspaces.", "The next positive mechanism must directly synthesize the polar of an overlapping projector frame, ideally from recoupling or an efficiently navigable average-case witness structure."],
            metrics=["covariant_carrier_polar_reduction_theorem_count", "orientation_nonorthogonal_sampling_reduction_theorem_count", "commuting_fiber_sampler_special_case_theorem_count", "carrier_control_count", "carrier_validation_failure_count", "explicit_fiber_control_count", "explicit_fiber_validation_failure_count", "wreath_orientation_control_count", "wreath_orientation_validation_failure_count", "noncommuting_wreath_sector_count", "maximum_wreath_pair_commutator_norm", "tail_n", "tail_black_box_polar_query_cost_log2_lower", "structured_nonorthogonal_sampler_count", "polynomial_hidden_permutation_decoder_count", "new_quantum_algorithm_count"],
            dependencies=[
                "self_dual_wreath_pgm_quantum_sampling_reduction.py",
            ],
            next_actions=[
                "Run qsearch.py code-wreath-pgm-quantum-sampling-reduction.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-PGM-SPECTRAL-WINDOW",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Exp Code Self Dual Wreath Pgm Spectral Window",
            status="planned",
            hypothesis="Proved an all-n constant-condition spectral subspace retaining constant PGM correct probability.",
            protocol="Evaluate exact representation-theoretic properties and validate metrics.",
            positive_signal="Proved an all-n constant-condition spectral subspace retaining constant PGM correct probability.",
            falsifiers=["Neither the minimum frame eigenvalue nor an asymptotically growing condition number is necessary for constant average success.", "The unresolved algorithmic content is now exactly a structured multiplicity-window projector or equivalently rescaled frame access.", "Raw spectral filtering remains closed despite the well-conditioned retained subspace."],
            metrics=["two_sided_spectral_window_theorem_count", "constant_condition_constant_success_theorem_count", "finite_control_count", "finite_control_failure_count", "scaling_record_count", "retained_condition_number_upper_bound", "minimum_windowed_pgm_success_lower_bound", "maximum_discarded_mass_upper_bound", "structured_window_projector_circuit_count", "polynomial_covariant_pgm_circuit_count", "new_quantum_algorithm_count"],
            dependencies=[
                "self_dual_wreath_pgm_spectral_window.py",
            ],
            next_actions=[
                "Run qsearch.py code-wreath-pgm-spectral-window.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-PGM-SUCCESS-THEOREM",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Exp Code Self Dual Wreath Pgm Success Theorem",
            status="planned",
            hypothesis="Converted the exact projector-frame second moment into an all-n PGM success lower bound above one half at the information threshold.",
            protocol="Evaluate exact representation-theoretic properties and validate metrics.",
            positive_signal="Converted the exact projector-frame second moment into an all-n PGM success lower bound above one half at the information threshold.",
            falsifiers=["The multiregister PGM success probability is no longer an open information-theoretic obligation for the hidden bridge ensemble.", "A second frame moment can certify PGM success through Tr sqrt(B), even though it cannot upper-bound the frame operator norm.", "Constant information-theoretic success does not remove the factorial coherent frame-whitening implementation barrier."],
            metrics=["pgm_success_lower_bound_theorem_count", "constant_information_threshold_success_theorem_count", "bounded_error_with_constant_extra_copies_theorem_count", "holder_control_count", "holder_control_failure_count", "physical_branch_control_count", "physical_branch_bound_failure_count", "scaling_record_count", "threshold_record_count", "minimum_threshold_pgm_success_lower_bound", "minimum_two_extra_copy_success_lower_bound", "maximum_n", "polynomial_pgm_implementation_count", "new_quantum_algorithm_count"],
            dependencies=[
                "self_dual_wreath_pgm_success_theorem.py",
            ],
            next_actions=[
                "Run qsearch.py code-wreath-pgm-success-theorem.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-PGM-TRUNCATION-ROBUSTNESS",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Exp Code Self Dual Wreath Pgm Truncation Robustness",
            status="planned",
            hypothesis="Proved that constant PGM success only requires inversion above a controlled Theta(1/n!) cutoff, independent of lambda_min(B).",
            protocol="Evaluate exact representation-theoretic properties and validate metrics.",
            positive_signal="Proved that constant PGM success only requires inversion above a controlled Theta(1/n!) cutoff, independent of lambda_min(B).",
            falsifiers=["The actual minimum nonzero frame eigenvalue is not an algorithmic proof obligation for bounded average success.", "Raw normalization-one QSVT remains factorial because the robust cutoff is still Theta(1/n!).", "The decisive positive target is now a polynomially normalized representation-specific encoding of n!B or its multiplicity blocks."],
            metrics=["minimum_eigenvalue_independent_truncation_theorem_count", "gentle_pgm_success_robustness_theorem_count", "finite_control_count", "finite_control_failure_count", "scaling_record_count", "minimum_truncated_threshold_success_lower_bound", "maximum_n", "tail_cutoff_log2", "polynomial_rescaled_frame_encoding_count", "polynomial_multiplicity_inverse_count", "new_quantum_algorithm_count"],
            dependencies=[
                "self_dual_wreath_pgm_truncation_robustness.py",
            ],
            next_actions=[
                "Run qsearch.py code-wreath-pgm-truncation-robustness.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-PHYSICAL-ORIENTATION-INTERFERENCE",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Exp Code Self Dual Wreath Physical Orientation Interference",
            status="planned",
            hypothesis="Lifted orientation-character interference from the dual Gram description to a direct polynomial physical-carrier circuit and proved its exact coset trace-transfer identity; the companion Plancherel theorem proves typical information retention.",
            protocol="Evaluate exact representation-theoretic properties and validate metrics.",
            positive_signal="Lifted orientation-character interference from the dual Gram description to a direct polynomial physical-carrier circuit and proved its exact coset trace-transfer identity; the companion Plancherel theorem proves typical information retention.",
            falsifiers=["The factorial direct physical-to-Gram access obstruction is not an obstruction to branch-register interference.", "Selected orientation common cores can be rejected directly and covariantly at unit scale.", "No speedup follows until retained information, residual norm, outcome measurement, decoding, and classical separation close."],
            metrics=["direct_physical_orientation_filter_theorem_count", "orientation_coset_trace_transfer_theorem_count", "covariantized_acceptance_theorem_count", "finite_control_count", "finite_validation_failure_count", "minimum_finite_retained_trace_fraction", "maximum_finite_retained_trace_fraction", "maximum_covariant_acceptance_spread", "maximum_trace_transfer_residual", "scaling_record_count", "all_n_retained_information_theorem_count", "postfilter_polynomial_frame_norm_theorem_count", "polynomial_hidden_permutation_decoder_count", "new_quantum_algorithm_count"],
            dependencies=[
                "self_dual_wreath_physical_orientation_interference.py",
            ],
            next_actions=[
                "Run qsearch.py code-wreath-physical-orientation-interference.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-PHYSICAL-PGM-INTERTWINER",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Exp Code Self Dual Wreath Physical Pgm Intertwiner",
            status="planned",
            hypothesis="Compiled the physical-output transfer as coherent generalized S_n Fourier sampling and proved that the physical PGM is exactly the orientation polar adjoint after this row-copy isometry.",
            protocol="Evaluate exact representation-theoretic properties and validate metrics.",
            positive_signal="Compiled the physical-output transfer as coherent generalized S_n Fourier sampling and proved that the physical PGM is exactly the orientation polar adjoint after this row-copy isometry.",
            falsifiers=["Equal Gram data alone remains nonconstructive, but the wreath factor has additional covariant structure that supplies W.", "A Kronecker multiplicity basis is not an implementation prerequisite for physical transfer.", "Do not claim a decoder until the global hierarchical orientation polar is implemented with polynomial resources."],
            metrics=["physical_pgm_intertwiner_theorem_count", "finite_control_count", "finite_validation_failure_count", "physical_output_intertwiner_count", "coherent_cross_sector_transfer_count", "kronecker_multiplicity_basis_required_count", "factorial_amplitude_amplification_required_count", "hierarchical_orientation_polar_sampler_count", "polynomial_physical_pgm_circuit_count", "tail_n", "tail_information_threshold_copy_count", "new_quantum_algorithm_count"],
            dependencies=[
                "self_dual_wreath_physical_pgm_intertwiner.py",
            ],
            next_actions=[
                "Run qsearch.py code-wreath-physical-pgm-intertwiner.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-PLANCHEREL-KRONECKER-POSITIVITY",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Exp Code Self Dual Wreath Plancherel Kronecker Positivity",
            status="planned",
            hypothesis="Proved that ordinary Kronecker coefficients of three independent Plancherel partitions are positive with probability 1-o(1).",
            protocol="Evaluate exact representation-theoretic properties and validate metrics.",
            positive_signal="Proved that ordinary Kronecker coefficients of three independent Plancherel partitions are positive with probability 1-o(1).",
            falsifiers=["Exponential conjugacy-class count does not prevent the reciprocal class-size variance from vanishing.", "The independent Plancherel theorem must not be stated for arbitrary couplings or uniform partitions.", "Typical Hamming-three support does not determine the natural frame edge."],
            metrics=["exact_plancherel_kronecker_variance_theorem_count", "independent_plancherel_typical_positivity_theorem_count", "hamming_three_common_support_corollary_count", "finite_control_count", "finite_control_failure_count", "maximum_exact_control_n", "tail_scaling_n", "tail_zero_probability_upper_bound", "tail_n_squared_scaled_bound", "arbitrarily_coupled_positivity_theorem_count", "uniform_partition_positivity_theorem_count", "natural_node_frame_edge_theorem_count", "new_quantum_algorithm_count"],
            dependencies=[
                "self_dual_wreath_plancherel_kronecker_positivity.py",
            ],
            next_actions=[
                "Run qsearch.py code-wreath-plancherel-kronecker-positivity.",
            ],
        ),
        ExperimentRecord(
            ...
            status="planned",
            hypothesis=(
                "Pure equal-pair commutator products admit Frobenius/Kronecker contraction, while complete mixed "
                "third moments require a four-class recoupling kernel indexed by r, q, r^-1q, and [r,q]."
            ),
            protocol=(
                "Decompose tensor-product characters and apply the exact Frobenius commutator formula; validate "
                "against direct pair averages; construct finite four-class kernels through S_6; measure class-triple "
                "splitting and support; and distinguish the pure contraction from the unresolved mixed kernel."
            ),
            positive_signal=(
                "A polynomial recoupling or spin-network construction of the mixed four-class kernel, or a direct "
                "contraction of its action on every naturally occupied physical character portfolio."
            ),
            falsifiers=[
                "The Frobenius pure commutator average disagrees with direct pair enumeration.",
                "The unconditioned commutator distribution is reused for class-conditioned mixed terms.",
                "A finite four-class kernel built from (n!)^2 pairs is described as scalable.",
                "Pure commutator assignments are described as complete third moments.",
                "Character multiplicities are described as a coherent recoupling circuit.",
            ],
            metrics=[
                "failed_pure_commutator_validation_count",
                "exact_pure_commutator_frobenius_contraction_count",
                "maximum_pure_commutator_scaling_n",
                "maximum_character_inner_product_term_bound",
                "maximum_refined_kernel_explicit_pair_count",
                "maximum_refined_class_quadruple_support_count",
                "maximum_split_class_triple_count",
                "polynomial_refined_kernel_construction_count",
                "mixed_class_commutator_contraction_count",
                "all_physical_irrep_sector_contraction_count",
            ],
            dependencies=[
                "self_dual_wreath_equal_commutator_audit.py",
                "symmetric_character.py",
                "Frobenius finite-group commutator formula",
                "Kronecker tensor-product multiplicities",
            ],
            next_actions=[
                "Run qsearch.py code-self-dual-wreath-commutators.",
                "Derive a representation-theoretic contraction for the four-class kernel.",
                "Search low-rank or stable-partition structure in finite refined kernels.",
                "Compile any classical recoupling formula into a coherent access model only after scaling is proved.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-STABLE-COMMUTATOR-RANK",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Stable-partition commutator rank and mass audit",
            status="planned",
            hypothesis=(
                "Bounded-tail stable partitions may have a low-rank four-class feature algebra, but any such "
                "contraction is useful only if those partitions carry nonvanishing natural physical-label mass."
            ),
            protocol=(
                "Generate all bounded-tail source partitions and physical descriptors; compute exact modular ranks "
                "of their Hadamard feature algebra on finite refined commutator kernels; derive the physical label "
                "law from independent Plancherel source partitions; and measure exact bounded-tail mass through n=64."
            ),
            positive_signal=(
                "A polynomial feature algebra whose covered source partitions carry asymptotically nonvanishing "
                "Plancherel and physical-label mass, followed by a coherent typical-sector recoupling transform."
            ),
            falsifiers=[
                "Finite low rank is treated as a growing-n theorem.",
                "Stable character-polynomial partitions are assumed to be Plancherel typical.",
                "Source Plancherel mass is confused with physical pair-label mass.",
                "One-copy stable mass is reported without threshold-copy compounding.",
                "A vanishing-mass conditional contraction is described as a decoder.",
            ],
            metrics=[
                "maximum_stable_feature_algebra_rank",
                "maximum_refined_kernel_support_count",
                "finite_full_kernel_separation_count",
                "fixed_tail_vanishing_mass_theorem_count",
                "minimum_log2_one_copy_physical_stable_mass",
                "minimum_log2_threshold_all_stable_mass",
                "typical_sector_coverage_count",
                "polynomial_typical_four_class_contraction_count",
                "polynomial_hidden_permutation_decoder_count",
            ],
            dependencies=[
                "self_dual_wreath_stable_commutator_rank.py",
                "self_dual_wreath_equal_commutator_audit.py",
                "hook-length dimension formula",
                "Plancherel measure on symmetric-group partitions",
            ],
            next_actions=[
                "Run qsearch.py code-self-dual-wreath-stable-rank.",
                "Reject bounded-tail-only recoupling if its natural mass vanishes.",
                "Construct finite rank probes on Plancherel-typical partitions instead.",
                "Search asymptotic limit-shape coordinates or local transition measures for compressed recoupling.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-TYPICAL-PARTITION-PORTFOLIO",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Constant-mass typical-partition portfolio audit",
            status="planned",
            hypothesis=(
                "A constant-natural-mass physical recoupling route requires a growing portfolio of Plancherel-"
                "typical source partitions, and no polynomial precertified catalog can supply that coverage."
            ),
            protocol=(
                "Sort exact Plancherel atoms; compute minimal prefixes reaching 50%, 80%, and 90% source mass; square "
                "their mass for physical pair-label coverage; apply the maximal-dimension theorem to catalog size; "
                "and measure finite four-class feature-algebra rank for 80%-mass portfolios through S_6."
            ),
            positive_signal=(
                "A single reversible recoupling rule polynomial in n and the sampled partition descriptions, with "
                "nonvanishing physical-label coverage and no explicit partition catalog."
            ),
            falsifiers=[
                "A finite list of maximum-dimension partitions is described as constant-mass coverage.",
                "Source catalog mass is not squared for physical pair labels.",
                "A superpolynomial catalog is described as an efficient uniform rule.",
                "Finite feature rank is described as a growing-n recoupling construction.",
                "Known QFT/YJM label primitives are substituted for internal multiplicity recoupling.",
            ],
            metrics=[
                "maximum_catalog_n",
                "maximum_ninety_percent_catalog_count",
                "maximum_ninety_percent_atom_lower_bound",
                "constant_mass_polynomial_catalog_no_go_theorem_count",
                "maximum_typical_feature_algebra_rank",
                "maximum_typical_refined_kernel_support_count",
                "uniform_partition_description_recoupling_rule_count",
                "polynomial_typical_four_class_contraction_count",
            ],
            dependencies=[
                "self_dual_wreath_typical_partition_portfolio.py",
                "self_dual_wreath_stable_commutator_rank.py",
                "aggarwal-elboim-maximal-dimension-2026",
                "Plancherel measure and hook-length dimensions",
            ],
            next_actions=[
                "Run qsearch.py code-self-dual-wreath-typical-portfolio.",
                "Transfer the existing recoupling capability ledger to the physical four-class kernel.",
                "Synthesize a partition-description-uniform spin-network contraction.",
                "Reject any proposal that enumerates typical partition labels or multiplicity bases.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-TYPICAL-RECOUPLING-TRANSFER",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Typical physical recoupling capability transfer audit",
            status="planned",
            hypothesis=(
                "Known symmetric-group quantum primitives transfer only partially to the physical four-class frame; "
                "the first missing breakthrough primitive can be isolated by a typed capability contract."
            ),
            protocol=(
                "Read the literature-backed recoupling capability ledger, typical source coverage, uniform-source "
                "collision probe, invariant contraction, and new wreath commutator artifacts; map each required "
                "physical primitive to proved scope; and reject every scope-mismatched transfer."
            ),
            positive_signal=(
                "A newly synthesized partition-description-uniform internal Kronecker/associator primitive that "
                "contracts the mixed four-class kernel and feeds a proved support-conditioned decoder."
            ),
            falsifiers=[
                "An S_n QFT is counted as an internal Kronecker transform.",
                "YJM target labels are counted as multiplicity basis resolution.",
                "Block encoding is counted as a spectral gap or eigenbasis.",
                "Stable-sector or explicit-catalog transforms are counted as typical natural coverage.",
                "A fixed separator with exact collisions is counted as uniform.",
            ],
            metrics=[
                "valid_partial_primitive_transfer_count",
                "decoder_blocking_missing_primitive_count",
                "internal_kronecker_transform_count",
                "kcopy_associator_count",
                "mixed_four_class_contraction_count",
                "uniform_typical_separator_rule_count",
                "uniform_typical_gap_theorem_count",
                "fixed_separator_exact_collision_count",
            ...
            next_actions=[
                "Run qsearch.py code-self-dual-wreath-recoupling-transfer.",
                "Synthesize the first missing typed primitive rather than another finite separator table.",
                "Search partition-dependent commutant generator rules against the registered exact collisions.",
                "Require end-to-end coherent access and decoding contracts for every proposal.",
            ],
        ),
        ExperimentRecord(
        ...
        ExperimentRecord(
            id="EXP-COSET-NATURAL-MULTICOPY-PGM",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Natural-input multi-copy carrier PGM benchmark",
            status="planned",
            hypothesis=(
                "A genuinely joint carrier measurement can extract hidden-"
                "involution information unavailable to separate strong "
                "Fourier and separate one-copy PGM measurements."
            ),
            protocol=(
                "For every naturally weighted Fourier source-label multiset "
                "on exact S_5 fixed-point-free controls, construct the global "
                "conditioned square-root measurement at one, two, and three "
                "copies. Average mutual information and Bayes recovery over "
                "the exact source law and compare with product one-copy PGMs "
                "and separate Young-basis carrier measurements."
            ),
            positive_signal=(
                "A source-natural collective information gain persists under "
                "scaling, admits a uniform polynomial carrier-sensitive "
                "circuit and compressed outcome, supports a polynomial hidden-"
                "involution decoder, and beats legal classical baselines."
            ),
            falsifiers=[
                "The gain disappears after natural source-label weighting.",
                "Separate one-copy PGMs or strong Fourier match the global measurement.",
                "The exact PGM requires dense exponentially large branch matrices.",
                "No polynomial decoder or classical separation follows from the outcomes.",
            ],
            metrics=[
                "natural_source_mass_control_count",
                "one_copy_global_product_pgm_consistency_count",
                "finite_collective_information_gain_row_count",
                "maximum_global_information_gain_over_product_pgm_bits",
                "maximum_global_information_gain_over_separate_young_bits",
                "maximum_global_bayes_gain_over_product_pgm",
            ...
            dependencies=[
                ...
                "coset_commutant_information_obstruction.py",
                "natural weak-Fourier source-label law",
                "finite mixed-state square-root measurements",
            ],
            next_actions=[
                "Express the exact PGM effects in carrier/commutant harmonic coordinates.",
                "Identify which joint observables account for gain over product PGM.",
                "Search a growing-width covariant circuit that approximates those observables.",
                "Demand an explicit hidden-involution decoder and classical baseline comparison.",
            ],
        ),
        ExperimentRecord(
            id="EXP-COSET-PGM-GAIN-LOCALIZATION",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Natural PGM gain localization and frame-spectrum audit",
            status="planned",
            hypothesis=(
                "The finite natural collective PGM gain is broad across source "
                "labels and arises from average frames with tractable spectral "
                "structure, making harmonic frame access a sharper target than "
                "bounded-support separator search."
            ),
            protocol=(
                "Enumerate every natural three-copy S_5 source multiset. "
                "Measure its weighted global-over-product PGM information "
                "gain, support rank, distinct nonzero average-frame spectrum, "
                "condition number, and exact finite inverse-root interpolation "
                "degree. Audit gain concentration under the full source law."
            ),
            positive_signal=(
                "Gain remains broad and source-natural while an all-n harmonic "
                "average-frame block encoding has polynomial condition and "
                "inverse-root degree, composes with a growing-width covariant "
                "measurement, and supports polynomial hidden-label decoding."
            ),
            falsifiers=[
                "Most gain comes from negligible natural source mass.",
                "One selected source branch dominates the aggregate signal.",
                "Average frames are already exponentially ill-conditioned at finite size.",
                "Finite low interpolation degree has no all-n harmonic implementation.",
            ],
            metrics=[
                "source_branch_count",
                "positive_information_gain_branch_count",
                "positive_gain_natural_source_probability",
                "maximum_single_branch_gain_fraction",
                "gain_80_percent_natural_source_probability",
                "maximum_frame_condition_number",
                "maximum_distinct_nonzero_frame_eigenvalue_count",
            ...
            dependencies=[
                "coset_pgm_gain_localization.py",
                "coset_natural_multicopy_pgm_benchmark.py",
                "harmonic analysis of diagonal conjugation actions",
                "uniform block encodings and polynomial inverse-root transforms",
            ],
            next_actions=[
                "Derive the average frame as an element of a typed diagonal-action commutant algebra.",
                "Identify a polynomial-description basis for its multiplicity blocks.",
                "Test condition and distinct-spectrum growth on adjacent sizes without selected source postselection.",
                "Compile a covariant seed measurement and hidden-involution decoder from the frame inverse.",
            ],
        ),
        ExperimentRecord(
            id="EXP-COSET-PGM-AVERAGE-FRAME-BLOCK-ENCODING",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Coset PGM average-frame block-encoding boundary",
            status="planned",
            hypothesis=(
                "The natural multi-copy PGM average frame has a compact "
                "all-k class-average representation, but a direct projected "
                "LCU may lose exponentially in absolute spectral normalization "
                "at the required growing width."
            ),
            protocol=(
                "Prove the subset class-average expansion for arbitrary copy "
                "count, verify direct frame reconstruction and projected-LCU "
                "proportionality on every natural S_5 three-copy source branch, "
                "and audit generic amplification under an explicit hard-sector "
                "character-ratio envelope at k=ceil(n log2 n)."
            ),
            positive_signal=(
                "A uniform natural-source theorem and representation-specific "
                "block encoding or spectral amplifier avoids the direct "
                "2^-k average scale, composes in polynomial gates, and feeds a "
                "covariant hidden-involution decoder."
            ),
            falsifiers=[
                "The subset expansion fails exact finite controls.",
                "Uniform conjugacy-class or Young-representation access is unavailable.",
                "Direct projected-LCU normalization needs superpolynomial amplification.",
                "The hard-sector character-ratio envelope fails under the natural source law.",
            ],
            metrics=[
                "all_k_subset_expansion_identity_count",
                "finite_subset_expansion_failure_count",
                "finite_projected_lcu_failure_count",
                "conditional_polynomial_projected_lcu_schema_count",
                ...
                "normalization_free_average_frame_block_encoding_count",
                "polynomial_structured_spectral_amplification_count",
                "polynomial_hidden_involution_decoder_count",
            ],
            dependencies=[
                "coset_pgm_average_frame_block_encoding.py",
                "uniform coherent involution-class preparation",
                "controlled Young-representation circuits",
                "QSVT or representation-specific spectral transforms",
            ],
            next_actions=[
                "Prove the character-ratio envelope for naturally sampled growing-width source tuples.",
                "Search harmonic preconditioners that rescale all occupied frame sectors simultaneously.",
                "Test direct covariant seed measurements that bypass inverse-root frame amplification.",
                "Charge every class-state preparation, projection, amplification, and decoder operation.",
            ],
        ),
        ExperimentRecord(
            id="EXP-COSET-NATURAL-CHARACTER-RATIO-CONCENTRATION",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Natural-source character-ratio concentration theorem",
            status="planned",
            hypothesis=(
                "Column orthogonality forces naturally sampled weak-Fourier "
                "source labels into the small-character-ratio envelope "
                "simultaneously across k=ceil(n log2 n) copies."
            ),
            protocol=(
                "Derive the exact natural source law from the coset Fourier "
                "distribution, transfer the Plancherel character-ratio second "
                "moment using |r|<=1, apply Markov and an iid union bound, and "
                "verify source mass and column orthogonality on complete "
                "partition tables through n=16."
            ),
            positive_signal=(
                "A rigorous complete-source theorem resolves the ratio "
                "assumption in the average-frame normalization audit and "
                "correctly distinguishes that obstruction from a general "
                "collective-measurement lower bound."
            ),
            falsifiers=[
                "The weak source law depends on the individual hidden involution.",
                "The Plancherel second moment differs from inverse conjugacy-class size.",
                "Natural reweighting invalidates the second-moment tail bound.",
                "The growing-width union bound is not inverse-polynomial.",
            ],
            metrics=[
                "column_orthogonality_identity_count",
                "natural_second_moment_bound_theorem_count",
                "iid_growing_width_union_bound_theorem_count",
                "uniform_natural_source_character_ratio_theorem_count",
                "finite_column_orthogonality_failure_count",
                "finite_natural_source_mass_failure_count",
                "inverse_polynomial_tuple_envelope_row_count",
                "tail_tuple_failure_log2_upper_bound",
                "pgm_measurement_circuit_count",
                "polynomial_hidden_involution_decoder_count",
            ],
            dependencies=[
                "coset_natural_character_ratio_concentration.py",
                "symmetric-group character column orthogonality",
                "natural weak-Fourier source-label distribution",
                "fixed-point-free involution conjugacy-class counting",
            ],
            next_actions=[
                "Use the theorem to make the direct frame-normalization obstruction unconditional on natural tail inputs.",
                "Do not misstate the result as a lower bound for arbitrary collective measurements.",
                "Search structured harmonic amplification and direct covariant seed measurements.",
            ],
        ),
        ExperimentRecord(
            id="EXP-COSET-COVARIANT-PROJECTOR-SUBPOVM",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Whitening-free covariant projector sub-POVM",
            status="planned",
            hypothesis=(
                "Because conditioned involution carrier states are normalized "
                "projectors, a covariant sub-POVM proportional to those states "
                "can retain collective information without inverse-root frame "
                "whitening or absolute spectral amplification."
            ),
            protocol=(
                "Prove the normalized-projector identity, define the maximal "
                "scalar covariant sub-POVM with an explicit failure effect, "
                "derive q=Tr(F^2)/||F||>=1/kappa(F), and compare its complete "
                "natural S_5 one-to-three-copy channel with global PGM, product "
                "one-copy PGM, and separate Young-basis measurements."
            ),
            positive_signal=(
                "A uniform harmonic Naimark dilation implements the projector "
                "orbit with compressed outcomes, the natural all-n frame "
                "condition is polynomial, and a decoder beats separate strong "
                "Fourier and legal classical baselines."
            ),
            falsifiers=[
                "Conditioned carrier states are not normalized projectors.",
                "The scaled covariant effects fail POVM completeness.",
                "Conclusive probability still depends on absolute frame scale.",
                "The finite channel is dominated by separate strong Fourier or lacks a scalable orbit measurement.",
            ],
            metrics=[
                "normalized_projector_theorem_count",
                "covariant_subpovm_validity_theorem_count",
                "conclusive_probability_frame_formula_theorem_count",
                "inverse_condition_conclusive_lower_bound_theorem_count",
                "finite_subpovm_information_gain_over_product_pgm_row_count",
                "finite_subpovm_information_gain_over_separate_young_row_count",
                ...
                "direct_uniform_label_projector_dilation_schema_count",
                "structured_maximal_effect_amplification_count",
                "all_n_polynomial_frame_condition_theorem_count",
                "uniform_covariant_natural_subpovm_circuit_count",
                "compressed_hidden_label_outcome_count",
                "polynomial_hidden_involution_decoder_count",
                "classical_separation_theorem_count",
            ],
            dependencies=[
                "coset_covariant_projector_subpovm.py",
                "coset_pgm_gain_localization.py",
                "harmonic synthesis of covariant orbit POVMs",
                "compressed nonabelian outcome transforms",
            ],
            next_actions=[
                "Express the projector orbit analysis operator in diagonal-action harmonic coordinates.",
                "Construct a uniform Naimark dilation without enumerating hidden involutions.",
                "Prove natural all-n frame condition bounds or identify a preconditioned subfamily.",
                "Build and baseline a polynomial hidden-involution decoder from compressed outcomes.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-PROJECTOR-SUBPOVM",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Physical wreath projector sub-POVM transfer",
            status="planned",
            hypothesis=(
                "The whitening-free normalized-projector sub-POVM transfers "
                "exactly to code-equivalence bridge involutions, while its "
                "performance depends on a new natural all-sector wreath frame-"
                "condition theorem rather than symmetric-group controls."
            ),
            protocol=(
                "Transfer the group-general projector and condition-only "
                "conclusive theorem to W_n=(S_n x S_n) semidirect Z_2, combine "
                "it with exact wreath first/second moments and physical finite "
                "frame blocks, and separate valid algebraic transfer from "
                "invalid numerical S_n performance transfer."
            ),
            positive_signal=(
                "The natural information-threshold wreath frame has polynomial "
                "condition on all occupied sectors and admits a uniform "
                "covariant Naimark dilation, compressed permutation outcome, "
                "and polynomial hidden-permutation decoder."
            ),
            falsifiers=[
                "Bridge hidden states are not normalized projectors.",
                "S_n finite information is imported without a wreath source-law proof.",
                "Global wreath moments give only a factorially weak conclusive bound.",
                "Selected finite equal-pair block conditions do not cover natural sectors.",
            ],
            metrics=[
                "group_general_normalized_projector_theorem_count",
                "wreath_projector_subpovm_transfer_theorem_count",
                "condition_only_conclusive_formula_theorem_count",
                "tail_moment_only_conclusive_log2_lower_bound",
                "tail_direct_uniform_projector_conclusive_log2_probability",
                "direct_uniform_label_projector_dilation_schema_count",
                "structured_maximal_effect_dilation_count",
                "selected_information_threshold_block_count",
                ...
                "compressed_permutation_outcome_transform_count",
                "polynomial_hidden_permutation_decoder_count",
                "classical_code_equivalence_separation_count",
            ],
            dependencies=[
                "self_dual_wreath_projector_subpovm_transfer.py",
                "self_dual_wreath_pgm_polar_audit.py",
                "self_dual_wreath_physical_frame_blocks.py",
                "harmonic analysis of the physical wreath bridge orbit",
            ],
            next_actions=[
                "Derive operator-norm or condition bounds for the complete natural wreath frame.",
                "Decompose the projector analysis operator into physical harmonic carrier blocks.",
                "Construct a compressed permutation-outcome Naimark dilation without orbit enumeration.",
                "Compose the output with public-generator code-equivalence verification and classical baselines.",
            ],
        ),
        ExperimentRecord(
            id="EXP-CODE-SELF-DUAL-WREATH-SUBPOVM-MOMENTS",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Wreath sub-POVM high-order moment certificate",
            status="planned",
            hypothesis=(
                "High-order physical frame trace moments can certify natural "
                "projector-sub-POVM conclusive probability without full frame "
                "diagonalization or a condition-number fit."
            ),
            protocol=(
                "Prove q>=Tr(B^2)/(Tr(B)Tr(B^p)^(1/p)), evaluate orders "
                "2,3,4,8,16 over every naturally occupied complete W_3 "
                "threshold block, compare with exact conclusive probability, "
                "and derive a factor-two moment-order target from the growing "
                "wreath Hilbert-dimension bound."
            ),
            positive_signal=(
                "A growing-order contraction on arbitrary all-unequal tuples "
                "proves inverse-polynomial natural conclusive probability "
                "after discarding the vanishing equal event, and composes with "
                "a structured maximal-effect Naimark dilation and decoder."
            ),
            falsifiers=[
                "The moment certificate exceeds exact conclusive probability.",
                "Finite low-order certificates collapse under natural source weighting.",
                "Required moment order is superpolynomial in n.",
                "Polynomial order has no polynomial all-unequal contraction or measurement circuit.",
            ],
            metrics=[
                "moment_to_conclusive_lower_bound_theorem_count",
                "naturally_occupied_w3_block_count",
            ...
            dependencies=[
                ...
                "self_dual_wreath_character_moments.py",
                "self_dual_wreath_natural_unequal_dominance.py",
                "growing-order symmetric-group character contractions",
            ],
            next_actions=[
                "Extend class-compressed moment contractions beyond fixed order on arbitrary all-unequal physical tuples.",
                "Exploit moment generating functions or recurrences rather than enumerating p hidden labels.",
                "Control the vanishing discarded equal event and prove a natural averaged conclusive bound.",
                "Keep success certification separate from maximal-effect Naimark synthesis.",
            ],
        ),
        ExperimentRecord(
        ...
        ExperimentRecord(
            ...
            protocol=(
                "Build disjoint/shared-point transposition-pair and support-stratified transposition/3-cycle orbit sums, verify "
            ...
            metrics=[
                ...
                "maximum_low_support_portfolio_noncommuting_target_count",
                "minimum_finite_low_support_portfolio_lcu_normalized_gap",
            ...
            next_actions=[
                "Derive exact spectra for the support-three two-generator Hamiltonian as functions of partitions.",
        ...
        ExperimentRecord(
            id="EXP-COSET-STABLE-SHAPE-SECOND-MOMENT-CERTIFICATE",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Exact stable-shape Racah second-moment certificate",
            status="planned",
            hypothesis=(
                "The universal 17-class relative-orbit proof for Tr(H^2) extends to every stable intermediate "
                "shape, completing all five open multiplicity-two characteristic polynomials."
            ),
            protocol=(
                "Evaluate each relative orbit class with the shape-specific falling character polynomial, close "
                "every size below the symbolic stability range by exact pattern counts, and apply Newton's second "
                "identity with the proved trace formulas."
            ),
            positive_signal=(
                "Nine exact second-moment formulas agree with every sparse coefficient; five quadratic spectra "
                "become exact and only the multiplicity-three determinant remains."
            ),
            falsifiers=[
                "A second moment fails to simplify to a degree-at-most-six polynomial.",
                "An exact finite endpoint disagrees with the symbolic formula.",
                "A Newton coefficient disagrees with an independent sparse characteristic polynomial.",
                "Exact quadratic polynomials are promoted without normalized root separation.",
                "Exact spectra are promoted as coherent circuits or decoders.",
            ],
            metrics=[
                "exact_all_n_shape_second_moment_theorem_count",
                "new_exact_open_shape_second_moment_theorem_count",
                "new_exact_complete_quadratic_shape_polynomial_count",
                "finite_probe_second_coefficient_comparison_count",
                "finite_probe_second_coefficient_agreement_count",
                "exact_endpoint_verified_count",
                "remaining_open_shape_characteristic_coefficient_family_count",
                "new_normalized_gap_theorem_count",
            ],
            dependencies=[
                "coset_stable_shape_second_moment_certificate.py",
                "coset_stable_second_moment_certificate.py",
                "coset_stable_shape_trace_certificate.py",
                "17 relative simultaneous-conjugacy classes",
            ],
            next_actions=[
                "Prove the multiplicity-three (3,1)-tail determinant.",
                "Factor and prove positivity of all five quadratic discriminants.",
                "Prove normalized separation for the cubic shape.",
                "Compile the common orbit LCU and complete coupling-tree transitions.",
            ],
        ),
        ExperimentRecord(
            id="EXP-COSET-STABLE-SHAPE-CUBIC-DETERMINANT-CERTIFICATE",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Exact final stable-shape cubic determinant certificate",
            status="planned",
            hypothesis=(
                "The 129-class third-moment proof extends to the lone multiplicity-three (n-4,3,1) intermediate "
                "shape and completes the stable nine-shape spectral classification."
            ),
            protocol=(
                "Enumerate the exact relative simultaneous-conjugacy classes for H^3, reduce the (3,1)-tail "
                "falling character correlations to rational pattern coefficients, close n=8..16 exactly, and "
                "apply Newton's third identity."
            ),
            positive_signal=(
                "The degree-nine determinant formula passes all exact endpoints and sparse references, leaving "
                "zero unknown stable-shape characteristic coefficients."
            ),
            falsifiers=[
                "The 129 relative classes fail their raw orbit-count checksum.",
                "The third power trace fails to simplify to a degree-at-most-nine polynomial.",
                "Any exact endpoint or sparse determinant disagrees.",
                "Complete spectra are promoted without normalized root gaps or circuits.",
                "Spectral completeness is promoted as a hidden-involution decoder or speedup.",
            ],
            metrics=[
                "relative_orbit_class_count",
                "raw_canonical_pattern_count",
                "exact_endpoint_verified_count",
                "finite_sparse_determinant_match_count",
                "exact_cubic_shape_determinant_theorem_count",
                "exact_complete_stable_shape_polynomial_count",
                "remaining_open_shape_characteristic_coefficient_family_count",
                "new_normalized_gap_theorem_count",
            ],
            dependencies=[
                "coset_stable_shape_cubic_determinant_certificate.py",
                "coset_stable_third_moment_certificate.py",
                "coset_stable_shape_second_moment_certificate.py",
                "exact 129-class pattern checkpoint",
            ],
            next_actions=[
                "Prove normalized root separation for the five quadratic and one cubic open shapes.",
                "Compile the common orbit Hamiltonian as a uniform coherent block encoding.",
                "Synthesize all nine left/right shape-label transitions.",
                "Test complete outcomes for hidden-involution information against classical baselines.",
            ],
        ),
        ExperimentRecord(
            id="EXP-COSET-STABLE-SHAPE-QUADRATIC-GAP-CERTIFICATE",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Exact complementary quadratic-shape gap certificate",
            status="planned",
            hypothesis=(
                "The five exact multiplicity-two characteristic polynomials have uniformly positive discriminants "
                "and inverse-polynomial gaps after common orbit-LCU normalization."
            ),
            protocol=(
                "Factor each exact discriminant e1^2-4e2, substitute n=m+8 to prove positivity by nonnegative "
                "coefficients, and divide the raw square-root gap by alpha=n(n-1)(n-2)."
            ),
            positive_signal=(
                "All five discriminants are positive for every n>=8 and every normalized gap is at least 12/n^3."
            ),
            falsifiers=[
                "A discriminant vanishes or becomes negative in the stable range.",
                "The declared LCU normalization omits orbit terms.",
                "A raw gap is confused with a normalized phase-estimation gap.",
                "Five quadratic gaps are promoted as the unresolved cubic gap.",
                "Gap theorems are promoted as circuits, transitions, or decoders.",
            ],
            metrics=[
                "quadratic_shape_count",
                "positive_discriminant_theorem_count",
                "new_normalized_gap_theorem_count",
                "minimum_uniform_raw_gap_lower_bound",
                "uniform_inverse_polynomial_gap_exponent",
                "remaining_open_stable_shape_gap_family_count",
                "new_coherent_shape_label_count",
            ],
            dependencies=[
                "coset_stable_shape_quadratic_gap_certificate.py",
                "coset_stable_shape_second_moment_certificate.py",
                "exact quadratic discriminants",
                "common orbit-LCU normalization",
            ],
            next_actions=[
                "Prove normalized root separation for the multiplicity-three cubic shape.",
                "Compile one uniform common-orbit block encoding for all six complementary shapes.",
                "Synthesize left/right transitions across all nine shapes.",
                "Test complete label outcomes against hidden-involution classical baselines.",
            ],
        ),
        ExperimentRecord(
            id="EXP-COSET-STABLE-SHAPE-CUBIC-GAP-CERTIFICATE",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Exact final cubic stable-shape gap certificate",
            status="planned",
            hypothesis=(
                "The exact multiplicity-three discriminant is positive in the stable range and yields an "
                "inverse-polynomial normalized minimum root separation."
            ),
            protocol=(
                "Factor the exact cubic discriminant, prove positivity after n=m+8, bound all roots by a Cauchy "
                "coefficient estimate, convert the discriminant product to a minimum pair gap, and divide by the "
                "common orbit-LCU normalization."
            ),
            positive_signal=(
                "The cubic normalized gap is inverse polynomial, completing all six complementary and all seven "
                "nontrivial stable-shape gap families."
            ),
            falsifiers=[
                "The cubic discriminant vanishes or becomes negative for n>=8.",
                "The Cauchy coefficient bound does not dominate all roots.",
                "The discriminant-to-minimum-gap inequality omits a pairwise factor.",
                "The LCU normalization is understated.",
                "Complete gap theorems are promoted as circuits, transitions, decoders, or speedups.",
            ],
            metrics=[
                "exact_cubic_discriminant_identity_count",
                "positive_cubic_discriminant_theorem_count",
                "new_normalized_gap_theorem_count",
            ...
            dependencies=[
                "coset_stable_shape_cubic_gap_certificate.py",
                "exact cubic determinant certificate",
                "exact five-quadratic gap certificate",
                "Cauchy root bound and cubic discriminant identity",
            ],
            next_actions=[
                "Compile one uniform common-orbit block encoding across all six complementary shapes.",
                "Prove coherent phase-estimation precision and labels for every shape.",
                "Synthesize complete left/right nine-shape transitions.",
                "Test resulting outcomes for hidden-involution information against classical baselines.",
            ],
        ),
        ExperimentRecord(
            id="EXP-COSET-STABLE-SHAPE-COHERENT-LABEL-CERTIFICATE",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Uniform coherent labels on all stable shapes",
            status="planned",
            hypothesis=(
                "The original ordered-triple block encoding generalizes uniformly to every fixed padded stable "
                "shape, and the complete gap family permits coherent shape-local eigenlabels."
            ),
            protocol=(
                "Use one reversible distinct-triple PREPARE and shape-controlled Young-basis SELECT for "
                "rho_eta((a b)) tensor rho_W((a b c)); apply phase estimation at each exact gap precision, assuming "
                "the input is already routed into the declared eta tensor W -> xi channel."
            ),
            positive_signal=(
                "All seven nontrivial shape-local multiplicity eigenlabels are appended coherently with polynomial "
                "cost under one common block-encoding architecture."
            ),
            falsifiers=[
                "Ordered triples do not index every orbit term exactly once.",
                "Shape-controlled Young representation actions lack a polynomial implementation.",
                "A source normalized gap is missing.",
                "The theorem silently assumes routing or a coupling-tree transition.",
                "Local eigenlabels are promoted as an associator or decoder.",
            ],
            metrics=[
                "nontrivial_stable_shape_count",
                "common_shape_controlled_block_encoding_count",
                "new_coherent_shape_label_count",
                "all_nontrivial_stable_shape_coherent_label_count",
                "maximum_phase_estimation_query_exponent",
                "channel_routing_circuit_count",
                "coupling_tree_transition_circuit_count",
                "complete_racah_associator_count",
            ],
            dependencies=[
                "coset_stable_shape_coherent_label_certificate.py",
                "all stable-shape normalized gap certificates",
                "CAP-SN-QFT",
                "CAP-BOUNDED-SUPPORT-COMMUTANT-BLOCK-ENCODING",
            ],
            next_actions=[
                "Construct coherent routing/projectors into all nine intermediate/final channels.",
                "Synthesize the full left/right coupling-tree transition isometry.",
                "Estimate transition kernels and hidden-involution information at scaling sizes.",
                "Attack any decoder signal with classical representation-theory baselines.",
            ],
        ),
        ExperimentRecord(
        ...
        ExperimentRecord(
            id="EXP-COSET-TYPICAL-COMMUTANT-MOMENT-AUDIT",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Character-only typical-irrep commutant moment audit",
            status="planned",
            hypothesis=(
                "Bounded-support simultaneous-conjugacy orbit averages retain non-scalar action on naturally relevant "
                "high-dimensional Kronecker multiplicity blocks, rather than being an artifact of fixed stable tails."
            ),
            protocol=(
                "Use exact character projection to compute first and second multiplicity-space moments of normalized "
                "transposition/3-cycle and disjoint-transposition orbit averages on maximum-dimension source partitions. "
                "Record exact zero-variance blocks, finite portfolio coverage, and every missing asymptotic obligation."
            ),
            positive_signal=(
                "An all-n class-algebra formula proves non-scalarity and inverse-polynomial joint spectral separation "
                "uniformly for sampled partition descriptions, followed by a coherent transform and decoder."
            ),
            falsifiers=[
                "A searched orbit average has exactly zero variance on a typical multiplicity block.",
                "Finite nonzero variance is promoted as a minimum-gap theorem.",
                "The generator portfolio does not have simple joint spectrum.",
                "Exact factorial enumeration is counted as an efficient implementation.",
                "No observable-outcome information or hidden-involution decoder is supplied.",
            ],
            metrics=[
                "nontrivial_multiplicity_block_count",
                "finite_non_scalar_covered_count",
                "finite_uncovered_count",
                "minimum_positive_variance",
                "finite_centered_covariance_rank_two_count",
                "finite_multiplicity_two_simple_spectrum_count",
                "uniform_typical_commutant_non_scalar_theorem_count",
                "uniform_typical_commutant_gap_theorem_count",
            ...
            dependencies=[
                ...
                "simultaneous-conjugacy orbit algebra",
            ],
            next_actions=[
                "Derive class-algebra contractions that replace permutation enumeration for arbitrary n.",
                "Test whether the finite covariance-rank portfolio generates a noncommutative full matrix algebra.",
                "Prove inverse-polynomial joint spectral separation on Plancherel-typical labels.",
                "Specify a coherent label-adaptive block encoding and an outcome-information theorem.",
            ],
        ),
        ExperimentRecord(
            id="EXP-COSET-TYPICAL-CLASS-CONTRACTION-SCALING",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Marked-class contraction scaling for typical commutants",
            status="planned",
            hypothesis=(
                "A fixed bounded-support commutant generator that splits a maximum-dimension n=7 sector remains "
                "non-scalar on every typical Kronecker multiplicity block as n grows."
            ),
            protocol=(
                "Replace factorial group enumeration by exact marked-support injections into conjugacy-class "
                "representatives. Classify the primary generator's second moment into 17 relative pair types, "
                "verify against factorial controls through n=8, and enumerate exact scalar blocks through n=10."
            ),
            positive_signal=(
                "A fixed generator portfolio admits all-n class-algebra formulas, simple joint spectrum, and an "
                "inverse-polynomial minimum separation theorem on naturally weighted typical labels."
            ),
            falsifiers=[
                "The primary generator has exactly zero eigenvalue variance in any typical multiplicity block.",
                "Finite n=7 splitting is extrapolated without checking adjacent sizes.",
                "Class-compressed finite evaluation is presented as a coherent circuit.",
                "Separate non-scalarity is substituted for simple joint spectrum.",
                "No hidden-involution outcome law or decoder is supplied.",
            ],
            metrics=[
                "maximum_n",
                "fixed_relative_pair_type_count",
                "total_nontrivial_multiplicity_block_count",
                "total_exact_scalar_block_count",
                "finite_portfolio_non_scalar_covered_count",
                "finite_portfolio_common_scalar_block_count",
                "finite_portfolio_covariance_rank_two_count",
                "single_primary_generator_uniformity_falsification_count",
                "all_n_symbolic_class_contraction_formula_count",
                "uniform_portfolio_joint_gap_theorem_count",
                "typical_label_hidden_involution_decoder_count",
            ],
            dependencies=[
                "symmetric_marked_class_contraction.py",
                "coset_typical_class_contraction_scaling.py",
                "exact symmetric-group characters",
                "simultaneous-conjugacy pair classification",
            ],
            next_actions=[
                "Derive mixed higher moments or finite commutator norms for the support-three portfolio.",
                "Derive symbolic recurrences for the 17 primary relative pair types.",
                "Search exact joint-spectrum collisions rather than separate variance only.",
                "Prove normalized gaps on a natural-label set before synthesizing phase estimation.",
            ],
        ),
        ExperimentRecord(
            id="EXP-COSET-TYPICAL-PORTFOLIO-COLLISION-CERTIFICATE",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Exact higher-moment collision certificate for the typical commutant portfolio",
            status="planned",
            hypothesis=(
                "A fixed linear combination of the two support-three orbit averages that splits every audited n<=7 "
                "multiplicity block retains simple spectrum on the first n=8 scalar-primary targets."
            ),
            protocol=(
                "Compute exact TT1 orbit-word power traces through order four on the n=8 multiplicity-four targets, "
                "recover characteristic polynomials by Newton identities, and combine them with the exact TC2 scalar certificate."
            ),
            positive_signal=(
                "At least one fixed portfolio has square-free characteristic polynomials and inverse-polynomial "
                "minimum gaps on every naturally relevant target, with an all-n proof and coherent decoder."
            ),
            falsifiers=[
                "The TT1 characteristic polynomial has a repeated root on a TC2-scalar block.",
                "Any coefficient rule that splits n<=7 becomes degenerate at n=8.",
                "Nonzero variance is substituted for simple spectrum.",
                "A third finite generator is added without repeating the collision test.",
                "No all-n gap or hidden-involution decoder is supplied.",
            ],
            metrics=[
                "exact_collision_target_count",
                "exact_four_moment_certificate_count",
                "repeated_zero_eigenvalue_target_count",
                "two_generator_linear_span_simple_spectrum_target_count",
                "finite_common_coefficient_rules_falsified_at_n8",
                "minimum_required_portfolio_generator_count_on_certified_targets",
                "tested_third_generator_count",
                "disjoint_third_generator_repeated_root_target_count",
                "tested_three_generator_span_simple_spectrum_target_count",
                "uniform_three_generator_joint_gap_theorem_count",
                "typical_label_hidden_involution_decoder_count",
            ],
            dependencies=[
                "coset_typical_portfolio_collision_certificate.py",
                "exact orbit-word dynamic program",
                "symmetric-group character projection",
                "Newton identities",
            ],
            next_actions=[
                "Reject the TC2/TT1 linear span as a uniform resolver.",
                "Reject the disjoint-transposition extension and test TC-intersection-one on the certified collision blocks.",
                "Derive higher-moment recurrences only for portfolios that survive exact collisions.",
                "Keep all-n gap, natural-access, and decoder obligations blocked.",
            ],
        ),
        ExperimentRecord(
            id="EXP-COSET-TYPICAL-INDEPENDENT-THIRD-GENERATOR-CERTIFICATE",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Exact independent third-generator collision repair certificate",
            status="planned",
            hypothesis=(
                "The transposition/3-cycle intersection-one orbit average supplies an independent direction that "
                "removes the exact n=8 repeated-root collisions left by TC2, TT1, and the disjoint-transposition extension."
            ),
            protocol=(
                "Contract all TT1/TC1 orbit words through degree four on every n=8 target of multiplicity at most four, "
                "recover the parameterized characteristic polynomial by Newton identities, factor its discriminant, "
                "and prove positivity for every nonzero real coefficient using an exact z=c^2 decomposition."
            ),
            positive_signal=(
                "A fixed bounded-support portfolio has square-free characteristic polynomials on every naturally "
                "weighted typical block for all n, inverse-polynomial normalized gaps, a coherent implementation, and a decoder."
            ),
            falsifiers=[
                "The parameterized discriminant vanishes identically or at a nonzero real coefficient.",
                "Sign-twist transfer between the conjugate targets fails.",
                "The repair fails on another target at n=8 or an adjacent size.",
                "A finite simple-spectrum certificate is presented as an all-n gap theorem.",
                "No coherent transform or hidden-involution decoder is supplied.",
            ],
            metrics=[
                "independent_third_generator_target_count",
                "exact_parameterized_characteristic_polynomial_certificate_count",
                "exact_parameterized_four_moment_certificate_count",
                "nonzero_coefficient_simple_spectrum_target_count",
                "certified_n8_collision_target_repair_count",
                "certified_n8_low_multiplicity_target_count",
                "certified_n8_low_multiplicity_simple_spectrum_target_count",
                "n8_nontrivial_multiplicity_target_count",
                "n8_unaudited_higher_multiplicity_target_count",
                "maximum_exactly_audited_kronecker_multiplicity",
                "n8_exact_target_coverage_fraction",
                "all_n_simple_spectrum_theorem_count",
                "all_typical_target_coverage_theorem_count",
                "inverse_polynomial_gap_theorem_count",
                "coherent_typical_multiplicity_transform_count",
                "typical_label_hidden_involution_decoder_count",
            ],
            dependencies=[
                "coset_typical_independent_third_generator_certificate.py",
                "exact orbit-word dynamic program",
                "symmetric-group character projection",
                "Newton identities and exact discriminants",
            ],
            next_actions=[
                "Develop a higher-multiplicity transfer method for the 14 n=8 targets beyond fourth moments.",
                "Search adjacent n for exact repeated roots and coefficient incompatibility.",
                "Derive normalized gap bounds only after all finite collision tests survive.",
                "Keep coherent implementation and decoder obligations blocked.",
            ],
        ),
        ExperimentRecord(
            id="EXP-COSET-TYPICAL-HIGH-MULTIPLICITY-TRANSFER",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Exact full n=8 orbit-state transfer",
            status="planned",
            hypothesis=(
                "The fixed low-support operator TT1+TC1 that survives every n=8 block through multiplicity four "
                "has simple spectrum on every nontrivial n=8 multiplicity target through multiplicity 17."
            ),
            protocol=(
                "Propagate exact integer weights on the simultaneous-conjugacy quotient of permutation pairs through "
                "degree 17 with arbitrary-precision weights, contract each distribution with symmetric-group characters, and recover each complete "
                "characteristic polynomial and discriminant by Newton identities."
            ),
            positive_signal=(
                "The same fixed coefficients survive every n=8 target, adjacent sizes, and an all-n square-freeness "
                "and inverse-polynomial normalized-gap proof with coherent implementation and decoding."
            ),
            falsifiers=[
                "The exact transfer state counts disagree with the Python degree-four word audit.",
                "Any target characteristic polynomial has a nontrivial gcd with its derivative.",
                "A coefficient-one n=8 theorem is presented as parameter robustness or an all-n theorem.",
                "Adjacent sizes and normalized gap scaling are silently treated as audited.",
                "No all-n gap, coherent transform, or hidden-involution decoder is supplied.",
            ],
            metrics=[
                "maximum_exact_transfer_degree",
                "maximum_exact_transfer_state_count",
                "total_simultaneous_conjugacy_pair_state_count",
                "certified_n8_target_count",
                "certified_n8_simple_spectrum_target_count",
                "n8_nontrivial_multiplicity_target_count",
                "n8_unaudited_multiplicity_above_six_target_count",
                "n8_unaudited_target_count",
                "maximum_certified_kronecker_multiplicity",
                "n8_exact_target_coverage_fraction",
                "exact_transfer_recomputed_count",
                "all_n8_fixed_coefficient_simple_spectrum_theorem_count",
                "parameterized_all_coefficient_theorem_count",
                "all_n_simple_spectrum_theorem_count",
                "inverse_polynomial_gap_theorem_count",
                "coherent_typical_multiplicity_transform_count",
                "typical_label_hidden_involution_decoder_count",
            ...
            dependencies=[
                "tools/pair_orbit_transfer.cpp",
                "coset_typical_high_multiplicity_transfer.py",
                "exact symmetric-group characters",
                "Newton identities and exact discriminants",
            ],
            next_actions=[
                "Test the same fixed coefficient on the n=9 maximum-dimension source.",
                "Search parameter robustness and exact gap decay across n.",
                "Derive a class-algebra recurrence if adjacent sizes survive.",
                "Keep all-n gaps, implementation, and decoding blocked.",
            ],
        ),
        ExperimentRecord(
            id="EXP-COSET-TYPICAL-FIXED-SEPARATOR-GAP-SCALING",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Fixed typical-separator normalized gap scaling",
            status="planned",
            hypothesis=(
                "The fixed normalized orbit-average operator TT1+TC1 that separates all n=8 targets retains simple "
                "spectrum with inverse-polynomial normalized minimum gaps on maximum-dimension sources as n grows."
            ),
            protocol=(
                "Restrict the two normalized orbit averages to every dense multiplicity block at n=5,6,7, use exact "
                "quotient-transfer characteristic polynomials and rational root isolation at n=8, charge LCU "
                "normalization two, and separate finite descriptive trends from an all-n theorem."
            ),
            positive_signal=(
                "Adjacent sizes survive exact collision tests and admit a symbolic class-algebra recurrence proving "
                "a uniform inverse-polynomial normalized gap on naturally weighted typical labels."
            ),
            falsifiers=[
                "Any finite block has a repeated eigenvalue.",
                "The exact n=8 root-isolation lower bound is nonpositive.",
                "A four-point fitted exponent is presented as a theorem.",
                "Orbit-average LCU normalization is omitted.",
                "No coherent transform or hidden-involution decoder is supplied.",
            ],
            metrics=[
                "minimum_n",
                ...
                "n8_exact_target_count",
                "n8_certified_minimum_raw_gap_lower_bound",
                "n8_certified_minimum_lcu_normalized_gap_lower_bound",
                "descriptive_log_log_gap_fit_exponent",
                "all_n_simple_spectrum_theorem_count",
            ...
            dependencies=[
                "coset_typical_fixed_separator_gap_scaling.py",
                "coset_typical_high_multiplicity_transfer.py",
                "dense Young--Jucys--Murphy restrictions",
                "exact rational root isolation",
            ],
            next_actions=[
                "Run the same fixed separator on the n=9 maximum-dimension source.",
                "Record exact normalized gaps and the first repeated-root counterexample.",
                "Derive a symbolic recurrence only if adjacent sizes survive.",
                "Keep coherent implementation and decoding blocked.",
            ],
        ),
        ExperimentRecord(
            id="EXP-COSET-TYPICAL-N9-LOW-MULTIPLICITY-PROBE",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Exact n=9 low-multiplicity separator probe",
            status="planned",
            hypothesis=(
                "The fixed normalized TT1+TC1 separator that resolves every n=8 target remains square-free on the "
            ...
            metrics=[
                ...
                "maximum_exact_transfer_degree",
                "maximum_exact_transfer_state_count",
                "low_multiplicity_target_count",
                "low_multiplicity_simple_spectrum_target_count",
                "n9_nontrivial_multiplicity_target_count",
                "n9_unaudited_higher_multiplicity_target_count",
                "maximum_certified_kronecker_multiplicity",
                "maximum_n9_kronecker_multiplicity",
                "n9_exact_target_coverage_fraction",
                "certified_low_multiplicity_minimum_raw_gap_lower_bound",
                "certified_low_multiplicity_minimum_lcu_normalized_gap_lower_bound",
                "degree6_unique_left_translation_count",
                "degree6_unique_right_translation_count",
                "degree6_temporary_character_table_bytes",
                ...
                "scalable_n9_character_contraction_count",
                "exact_transfer_recomputed_count",
            ...
            dependencies=[
                ...
                "coset_typical_n9_low_multiplicity_probe.py",
                "exact vectorized S_9 character translations",
        ...
        ExperimentRecord(
            id="EXP-COSET-TYPICAL-TRANSFER-SUPPORT-GROWTH",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Exact support-growth barrier for typical transfer contraction",
            status="planned",
            hypothesis=(
                "The bounded-support marked-class evaluator can be extended "
                "termwise to the transfer degrees needed for typical "
                "Kronecker multiplicity blocks."
            ),
            protocol=(
                "Aggregate exact arbitrary-precision TT1+TC1 quotient-transfer "
                "weights by the active support of each simultaneous-conjugacy "
                "pair at selected degrees for n=8,9,10. Hash-gate the certificate "
                "to the transfer kernel and account for the marked-injection cost."
            ),
            positive_signal=(
                "A fixed support bound captures every nonzero weighted transfer "
                "state needed for high-multiplicity traces, or a new collective "
                "recurrence handles the full-support sector without n! work."
            ),
            falsifiers=[
                "Full-support pair states have positive high-degree weight.",
                "Most exact transfer weight moves to support n or n-1.",
                "Marked-support injection enumeration degenerates to n!.",
                "A finite support profile is presented as an all-n concentration theorem.",
                "No collective full-support contraction recurrence is supplied.",
            ],
            metrics=[
                "audited_size_count",
                "exact_support_checkpoint_count",
                ...
                "scalable_full_support_character_contraction_count",
                "all_n_support_concentration_theorem_count",
            ],
            dependencies=[
                "tools/pair_orbit_transfer.cpp",
                "coset_transfer_support_growth.py",
                "research/certificates/coset_transfer_support_growth_certificate.json",
                "symmetric_marked_class_contraction.py",
            ],
            next_actions=[
                "Replace termwise marked injection with a full-support representation recurrence.",
                "Validate any recurrence against the exact n=8 and n=9 traces.",
                "Use it on the n=10 multiplicity-six target before extending the finite ladder.",
                "Keep all algorithmic speedup gates blocked.",
            ],
        ),
        ExperimentRecord(
            id="EXP-COSET-TYPICAL-INVARIANT-CONTRACTION",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Sparse invariant-space contraction beyond explicit S_n rows",
            status="planned",
            hypothesis=(
                "Diagonal-S_n invariant tensors allow finite typical multiplicity "
                "blocks to be contracted without materializing factorial character "
                "rows or every orbit term."
            ),
            protocol=(
                "Use a direct Coxeter invariant solve as a control. Then isolate "
                "one target-tableau multiplicity fiber in V_lambda tensor V_lambda "
                "with a diagonal Jucys-Murphy content penalty, propagate it through "
                "all target tableaux, trace one TT1 and TC1 representative, reproduce "
                "the exact n=10 cubic, then probe the multiplicity-six block."
            ),
            positive_signal=(
                "Exact control traces are reproduced, a previously inaccessible "
                "block is reached without S_10 rows, and the invariant solve exposes "
                "a branching or centralizer recurrence with polynomial complexity."
            ),
            falsifiers=[
                "The sparse restriction disagrees with exact class-Fourier traces.",
                "The Coxeter nullspace residual or block symmetry residual is large.",
                "The multiplicity-six block has a repeated numerical eigenvalue.",
                "The numerical solve is promoted as an exact square-free certificate.",
                "Exponential invariant-vector dimension is presented as polynomial scaling.",
            ],
            metrics=[
                "numerically_contracted_primary_target_count",
                "conjugate_target_coverage_count",
                "maximum_numerical_kronecker_multiplicity",
                "maximum_invariant_vector_dimension",
                "yjm_fiber_vector_dimension",
                "maximum_vector_dimension_reduction_factor",
                "maximum_estimated_arpack_basis_bytes",
                "maximum_single_tableau_fiber_bytes",
                "all_tableau_fibers_materialized_count",
                "direct_coxeter_multiplicity6_timeout_seconds",
                "next_multiplicity15_estimated_arpack_basis_bytes",
                ...
                "explicit_s10_character_table_bytes_avoided",
                "factorial_group_row_storage_removed_count",
                "exact_multiplicity6_certificate_count",
                "polynomial_invariant_contraction_theorem_count",
            ],
            dependencies=[
                "coset_typical_invariant_contraction.py",
                "coset_stable_subspace_transition_probe.py",
                "coset_jucys_murphy_label_transform.py",
                "symmetric_yjm_multiplicity_contraction.py",
                "research/certificates/coset_typical_invariant_contraction_certificate.json",
            ],
            next_actions=[
                "Convert the multiplicity-six numerical block to an exact certificate.",
                "Derive a Young-branching recurrence for the invariant nullspace.",
                "Extend the first-collision search only under exact residual and scaling gates.",
                "Keep all-n, coherent-transform, decoder, and speedup gates blocked.",
            ],
        ),
        ExperimentRecord(
            id="EXP-COSET-TYPICAL-YJM-PROJECTOR-TRACE",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Exact YJM-projector multiplicity trace identity",
            status="planned",
            hypothesis=(
                "A joint-content tableau projector converts exact typical "
                "Kronecker multiplicity-block moments into ordinary rational "
                "representation traces without constructing the numerical block."
            ),
            protocol=(
                "Construct the exact diagonal Jucys-Murphy Lagrange projector, "
                "evaluate Tr(P_T H^d) in rational pair-group algebra on n=5 and "
                "n=6 controls, recover exact characteristic polynomials, and "
                "measure pair-state saturation to test evaluator scalability."
            ),
            positive_signal=(
                "The projector identity is exact and a Young-tower, centralizer, "
                "or tensor-network recurrence evaluates its traces without "
                "enumerating a constant fraction of the (n!)^2 pair state space."
            ),
            falsifiers=[
                "The projector trace differs from the Kronecker multiplicity.",
                "Exact power traces disagree with independent invariant contractions.",
                "The control characteristic polynomial is not square-free.",
                "Literal pair-group expansion saturates factorial state space.",
                "An exact identity is presented as an efficient evaluator without a scaling theorem.",
            ],
            metrics=[
                "exact_yjm_projector_trace_identity_theorem_count",
                "exact_control_count",
                "exact_projector_trace_count",
                "exact_power_trace_count",
                "exact_square_free_control_count",
                "n6_degree2_pair_state_count",
                "n6_explicit_pair_state_space_size",
                "n6_degree2_pair_state_saturation_fraction",
                "n10_explicit_pair_state_space_size",
                "explicit_pair_algebra_scaling_falsification_count",
                "young_tower_compressed_exact_trace_evaluator_count",
                "exact_n10_multiplicity6_power_trace_count",
                "exact_n10_multiplicity6_characteristic_polynomial_count",
                "polynomial_exact_trace_evaluator_count",
            ],
            dependencies=[
                "symmetric_yjm_projector_trace.py",
                "coset_typical_yjm_projector_certificate.py",
                "research/certificates/coset_typical_yjm_projector_certificate.json",
                "exact symmetric-group characters",
                "diagonal Jucys-Murphy joint-content spectrum",
            ],
            next_actions=[
                "Derive a Young-tower recurrence for Tr(P_T H^d).",
                "Avoid explicit pair-group distributions at every recurrence layer.",
                "Use the recurrence to certify the n=10 multiplicity-six sextic.",
                "Keep all-n, coherent-transform, decoder, and speedup gates blocked.",
            ],
        ),
        ExperimentRecord(
            id="EXP-COSET-TYPICAL-MODULAR-YJM-CONTRACTION",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Exact finite-field YJM multiplicity contraction",
            status="planned",
            hypothesis=(
                "Young rational seminormal form and a finite-field content "
                "projector can certify typical multiplicity blocks exactly "
                "without factorial pair-group expansion."
            ),
            protocol=(
                "Represent adjacent transpositions over a good prime, project "
                "random vectors with the exact content-penalty spectral "
                "polynomial, propagate the target-tableau fiber with rational "
                "Gram weights, and contract TT1+TC1 modulo the prime."
            ),
            positive_signal=(
                "Independent controls reproduce exact rational traces and a "
                "compiled multi-prime implementation reaches the n=10 "
                "multiplicity-six sextic with CRT reconstruction."
            ),
            falsifiers=[
                "The modular projected rank differs from the Kronecker multiplicity.",
                "Modular traces disagree with exact rational controls.",
                "A selected prime divides a denominator or nonzero penalty.",
                "The pure-Python projector cost is presented as operationally ready for multi-prime n=10.",
                "Finite exact certification is presented as polynomial all-n scaling.",
            ],
            metrics=[
                "exact_modular_control_count",
                "exact_modular_power_trace_count",
                "exact_modular_trace_disagreement_count",
                "maximum_control_projected_rank",
                "pair_group_states_materialized_count",
                "n10_tensor_dimension",
                "n10_pair_group_state_space_size",
                "n10_pair_group_to_tensor_dimension_reduction_factor",
                "n10_projector_polynomial_degree",
                "n10_sparse_axis_actions_for_all_trials",
                "compiled_modular_projector_kernel_count",
                "n10_modular_prime_residue_count",
                "n10_modular_power_trace_residue_count",
                "n10_direct_exact_square_free_target_count",
                "n10_exact_square_free_target_count",
                "n10_inferred_by_conjugate_sign_duality_count",
                "exact_conjugate_sign_duality_theorem_count",
                "conjugate_sign_duality_validation_pair_count",
                "n10_exact_collision_target_count",
                "n10_maximum_exact_certified_multiplicity",
                "n10_nontrivial_multiplicity_target_count",
                "n10_unaudited_target_count",
                "n10_ladder_declared_target_count",
                "n10_ladder_remaining_declared_target_count",
                "exact_n10_multiplicity6_square_free_certificate_count",
                "exact_n10_multiplicity6_power_trace_count",
                "exact_n10_multiplicity6_characteristic_polynomial_count",
                "crt_rational_reconstruction_count",
                "polynomial_in_n_state_dimension_theorem_count",
            ],
            dependencies=[
                "symmetric_modular_yjm_contraction.py",
                "coset_typical_modular_yjm_contraction.py",
                "research/certificates/coset_typical_modular_yjm_contraction_certificate.json",
                "Young rational seminormal form",
                "finite-field spectral projection and CRT",
            ],
            next_actions=[
                "Compile and parallelize the modular content-penalty projector.",
                "Compute the n=10 multiplicity-six block over independently checked good primes.",
                "CRT-reconstruct exact power traces and certify a square-free sextic.",
                "Keep the exponential state-dimension, transform, decoder, and speedup gates blocked.",
            ],
        ),
        ExperimentRecord(
            id="EXP-COSET-TYPICAL-MODULAR-GAP-BOUND",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Exact denominator and discriminant gap bound",
            status="planned",
            hypothesis=(
                "Finite square-freeness plus exact YJM and orbit-average "
                "denominators yields a normalized gap bound strong enough for "
                "efficient phase estimation."
            ),
            protocol=(
                "Clear all power-trace denominators with the YJM Lagrange "
                "projector and separator orbit denominators, propagate through "
                "Newton identities, and combine the nonzero integer discriminant "
                "with the operator norm bound."
            ),
            positive_signal=(
                "The resulting all-n formula simplifies to an inverse-polynomial "
                "normalized gap on a natural typical family, or a stronger "
                "separator-specific recurrence supplies such a bound."
            ),
            falsifiers=[
                "The safest exact lower bound is astronomically smaller than inverse polynomial.",
                "Finite square-freeness is presented without quantitative real-root separation.",
                "An existence bound is presented as the observed gap.",
                "LCU normalization is omitted.",
                "No all-n separator structure controls coefficient height.",
            ],
            metrics=[
                "exact_denominator_root_separation_theorem_count",
                "square_free_target_bound_count",
                "maximum_bounded_multiplicity",
                "maximum_characteristic_clearing_denominator_digits",
                "maximum_gap_bound_denominator_digits",
                "strongest_lcu_normalized_gap_lower_bound_log10",
                "weakest_lcu_normalized_gap_lower_bound_log10",
                "inverse_polynomial_normalized_gap_theorem_count",
                "polynomial_phase_estimation_precision_certificate_count",
                "all_n_simple_spectrum_theorem_count",
                "coherent_typical_multiplicity_transform_count",
                "typical_label_hidden_involution_decoder_count",
            ],
            dependencies=[
                "coset_typical_modular_gap_bound.py",
                "coset_typical_modular_yjm_contraction.py",
                "symmetric_yjm_projector_trace.py",
                "exact YJM Lagrange denominators",
                "integer polynomial discriminant identity",
            ],
            next_actions=[
                "Search for separator-specific coefficient-height cancellation.",
                "Measure real normalized gaps on the exact modular ladder.",
                "Prove an inverse-polynomial bound or reject phase estimation as the implementation route.",
                "Keep transform, decoder, and speedup claims blocked.",
            ],
        ),
        ExperimentRecord(
            id="EXP-COSET-TYPICAL-N10-GAP-TREND",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Real-gap trend on exactly square-free n=10 blocks",
            status="planned",
            hypothesis=(
                "The exactly square-free typical separator blocks retain stable "
                "LCU-normalized real gaps as Kronecker multiplicity increases."
            ),
            protocol=(
                "Use sparse YJM eigensolves for modularly certified "
                "multiplicity-six and multiplicity-eight targets, stream all "
                "target tableaux, record residuals and normalized gaps, and "
                "audit a declared floating-point budget without promoting it."
            ),
            positive_signal=(
                "Observed gaps remain stable across a growing exact ladder and "
                "lead to a machine-verified interval or separator-specific "
                "inverse-polynomial all-n theorem."
            ),
            falsifiers=[
                "Normalized gaps drop sharply with multiplicity.",
                "Floating-point magnitudes are presented as exact.",
                "A declared error budget is presented as machine-verified.",
                "Two blocks at one n are fitted as an asymptotic law.",
                "No inverse-polynomial all-n gap theorem is supplied.",
            ],
            metrics=[
                "numerical_gap_target_count",
                "exact_square_free_numerical_gap_target_count",
                "minimum_audited_multiplicity",
                "maximum_audited_multiplicity",
                "multiplicity6_lcu_normalized_gap",
                "multiplicity8_lcu_normalized_gap",
                "multiplicity8_to_multiplicity6_gap_ratio",
                "multiplicity6_to_multiplicity8_gap_drop_factor",
                "declared_budget_survival_count",
                "machine_verified_roundoff_bound_count",
                "inverse_polynomial_normalized_gap_theorem_count",
                "all_n_gap_trend_sample_count",
                "coherent_typical_multiplicity_transform_count",
                "typical_label_hidden_involution_decoder_count",
            ],
            dependencies=[
                "coset_typical_n10_gap_trend.py",
                "symmetric_yjm_multiplicity_contraction.py",
                "research/certificates/coset_typical_n10_m8_numerical_gap_certificate.json",
                "exact modular square-free certificates",
            ],
            next_actions=[
                "Add multiplicity-nine and multiplicity-fifteen numerical gaps only if the eigensolve cost is justified.",
        ...
        ExperimentRecord(
            id="EXP-COSET-TYPICAL-PARITY-CLASS-CONTRACTION",
            candidate_id="CODE-COSET-COLLECTIVE",
            title="Exact n=8 parity-separator holdout",
            status="planned",
            hypothesis=(
                "The frozen TC2+CT1-2CT2 discovery-set rule remains "
                "non-scalar on every ordered-source Kronecker block at n=8."
            ),
            protocol=(
                "Compress bounded-support first and second moments by "
                "simultaneous-conjugacy class, contract exact unequal-source "
                "characters, and audit every nontrivial block through n=8."
            ),
            positive_signal=(
                "No exact scalar block occurs, every multiplicity-two block "
                "splits, and higher exact moments support square-freeness."
            ),
            falsifiers=[
                "Any exact zero-variance n=8 block is found.",
                "Class contractions disagree with factorial controls.",
                "Positive variance above multiplicity two is promoted as a simple-spectrum proof.",
                "Finite survival is promoted as an all-n gap theorem.",
            ],
            metrics=[
                "all_source_exact_block_count",
                "exact_non_scalar_block_count",
                "exact_scalar_obstruction_count",
                "multiplicity_two_block_count",
                "exact_multiplicity_two_simple_spectrum_count",
                "maximum_kronecker_multiplicity",
                "maximum_relative_pair_support",
                "factorial_control_count",
                "all_n_square_free_theorem_count",
                "inverse_polynomial_normalized_gap_theorem_count",
            ],
            dependencies=[
                "coset_typical_parity_class_contraction.py",
                "coset_typical_parity_complete_separator.py",
                "symmetric_marked_class_contraction.py",
                "exact symmetric-group characters",
            ],
            next_actions=[
                "Retire the frozen rule after any exact holdout obstruction.",
                "Classify obstruction orbits under source swap and sign twist.",
                "Prioritize growing-width carrier-sensitive decoding over repeated finite separator fitting.",
            ],
        ),
        ExperimentRecord(

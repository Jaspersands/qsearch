...
def write_cluster_locality_no_go_report(
    ...
    if write_registry:
        ...
        upsert_negative_result(
            NegativeResultRecord(
                id="NEG-SELF-DUAL-WREATH-CLUSTER-LOCALITY-NO-GO",
                source=registry_experiment_id,
                claim=(
                    "Disjoint product isotypic filters acting on sublinear source labels per cluster can restore polynomial frame bounds."
                ),
                reason_invalid=(
                    "Quantitative locality lower bounds prove sublinear disjoint cluster filters leave a superpolynomial common-family spike."
                ),
                lesson=(
                    "High-retention filters must use linear-scale, overlapping, or genuinely global coordination."
                ),
                applies_to=[
        ...
        upsert_experiment_result(
            ExperimentResultRecord(
                ...
                artifacts={
                    "self_dual_wreath_cluster_locality_no_go": str(path)
                },
            )
        )
    return payload

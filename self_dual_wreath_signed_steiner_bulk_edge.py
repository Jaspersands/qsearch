...
def write_signed_steiner_bulk_edge_report(
    ...
    if write_registry:
        ...
        upsert_negative_result(
            NegativeResultRecord(
                id="NEG-SELF-DUAL-WREATH-SIGNED-STEINER-BULK-EDGE",
                source=registry_experiment_id,
                claim=(
                    "Initial negative claim for EXP-CODE-SELF-DUAL-WREATH-SIGNED-STEINER-BULK-EDGE."
                ),
                reason_invalid=(
                    "Falsified or refined by exact theorem evaluation."
                ),
                lesson=(
                    "Lesson from exact theorem analysis for EXP-CODE-SELF-DUAL-WREATH-SIGNED-STEINER-BULK-EDGE."
                ),
                applies_to=[
        ...
        upsert_experiment_result(
            ExperimentResultRecord(
                ...
                artifacts={
                    "self_dual_wreath_signed_steiner_bulk_edge": str(path)
                },
            )
        )
    return payload

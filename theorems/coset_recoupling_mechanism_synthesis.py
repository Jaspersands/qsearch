...
TEMPLATES = (
    MechanismTemplate(
        ...
        (
            ...
            "The explicit coherent subset-phase program now has a verified conditional Walsh contraction and charged primitive counts, not a growing-degree classifier. Preserve source-selector correlations, beat disjoint-pair and pair-plus-total controls, and do not confuse fixed-group copy scaling with degree scaling. Fixed O(log n) mask marginals remain obstructed under the one-query contract; arbitrary full-bit decisions are not covered.",
            "Explicit parity and threshold terminal rules now exist. Any fixed parity reduces to an input-independent random two-subset Hadamard test; bounded Fourier l1 norm transfers the obstruction to source-corrected all-zero rules. Seek a genuinely collective large-norm output rule with proved signal, not a source-selected-support shortcut. Cheap majority evaluation has exponential Fourier norm and is not excluded by this argument.",
            "Source-selected parity is now executable and its exact character contraction reaches S6. Corrected negative-character selection loses to even one pair for every k>=2 by an exact prefix and moment-tail certificate. Do not present escaping the fixed-parity theorem as evidence of success; the finite-degree failure does not rule out arbitrary source-based classifiers at growing degree.",
            "Matching source-phase correction followed by source discard now has a separate full-output, large-copy binomial-mixture bound, including majority regardless of Fourier norm. At S1024 k=17528 its distance is at most 2^-2174. Retain additional source information or address the unresolved intermediate copy window explicitly; neither is evidence of success without a new joint-law baseline check.",
            "The newer categorical-mixture argument retains ALL source irreps and Walsh bits and permits arbitrary fixed common central unitary phases. At S1024 k=17528 even the full paired transcript has T<=2^-1663, review pending. Retaining sources or complex phases alone is not an escape at this scaling. Target the unresolved intermediate participating-copy window, genuinely source-adaptive phases, multiple coherent queries or a different final measurement, with a fresh actual-law derivation and stronger baselines.",
            "The streamed centralizer-orbit probe reaches S8 in the intermediate copy window. Its negative-character sign-category/Walsh ideal table loses to even coarse pair counts at k=7,8,10,12,14,16; source counts explain most of its signal. Do not keep proposing scalar features of this same paired transcript at those controls: its ideal distance already loses. Full irrep retention or a genuinely changed measurement requires a new unconditioned law, and the finite failure is not an all-degree theorem.",
        ),

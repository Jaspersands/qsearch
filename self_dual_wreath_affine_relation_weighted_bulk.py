...
def write_affine_relation_weighted_bulk_report(
    ...
    if write_registry:
        ...
        upsert_negative_result(
            NegativeResultRecord(
                id="NEG-SELF-DUAL-WREATH-AFFINE-RELATION-WEIGHTED-BULK",
                source=registry_experiment_id,
                claim=(
                    "Naive initial assumption for EXP-CODE-SELF-DUAL-WREATH-AFFINE-RELATION-WEIGHTED-BULK."
                ),
                reason_invalid=(
                    "Falsified or refined by exact theorem evaluation."
                ),
                lesson=(
                    "Lesson from exact theorem analysis for EXP-CODE-SELF-DUAL-WREATH-AFFINE-RELATION-WEIGHTED-BULK."
                ),
                applies_to=[
        ...
        upsert_experiment_result(
            ExperimentResultRecord(
                ...
                artifacts={
                    "self_dual_wreath_affine_relation_weighted_bulk": str(path)
                },
            )
        )
    return payload

...
def _dcp_random_label_decoder_proposal_for_conjecture(conjecture: dict[str, Any]) -> dict[str, Any] | None:
    ...
    return {
        ...
        "rejection_filters": [
            ...
            "For the measured low-subset-sum prefix, audit retained label information against GRZ and the review-pending joint-slice extension. Dependence on measured low sum z alone is not an escape: check conditional range sizes with the separate proof. Other measurements or extra label-sensitive quantum workspace need their own argument.",
            "For explicit pairings, prove inverse-polynomial natural-label-averaged noise-weighted half-period edge mass, not canonical witness rate. Supply efficient F or clean P and inverse; exponential fiber tables are only references. The prelabel-fault gauge must discard old labels and coins before program selection. Short-move coverage and constant-noise bounds apply only to the specified readouts.",
            "Affine-marked isolated edges and capped balanced supports have derived coverage bounds. The sorting-network finder's time and coherent storage are exponential, but a separate shared-geometric Grover edge readout is now list-free and polynomial-space: it needs neither unique partners nor vertex isolation. Its time remains exponential. Target faster structured evolution, not another coverage interface. For the isolated architecture only, increasing raw density also increases syndrome constraints. Do not impose deterministic-partner or exponential-list requirements on the direct coherent-edge route.",
            ...
            "Do not reject arbitrary quantum relation solvers merely for noncanonical witnesses: the conditional symmetric double-evaluation lift is an allowed interface. Reject naive single-evaluation uses with unhandled which-path workspace; solver efficiency, coverage and full noise composition remain separate obligations.",

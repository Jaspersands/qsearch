...
def _dcp_random_label_decoder_proposal_for_conjecture(conjecture: dict[str, Any]) -> dict[str, Any] | None:
    ...
    return {
        ...
        "rejection_filters": [
            ...
            "Affine-marked isolated edges now have review-pending polynomial source coverage. A balanced-support meet-in-the-middle finder is implemented with a fixed reversible sorting network, but its time and coherent storage are exponential. Beat this charged frontier using structure, not another existence argument. Conditional isolation degree D/(N*2^ell) remains in (1/8,1/4] at D>=N; increasing raw density also increases syndrome constraints. A valid clean proposal need not decide uniqueness on every input, but must retain useful reciprocal coverage.",

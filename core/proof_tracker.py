...
def _binary_carrier_instrument_lemmas(candidate_id: str) -> list[LemmaRecord]:
    ...
    return [LemmaRecord(
        ...
        id=f"LEMMA-{candidate_id}-CORRECTED-WALSH-OUTPUT-MIXTURE-BOUND", candidate_id=candidate_id,
        statement="For a common real central reflection fixed before source labels and matching source-phase correction, discarding sources and physical inputs gives an exchangeable full Walsh output. Its distance is at most 3k/(2 sqrt(M)) plus bulk and exceptional off-coset remainder bounds, vanishing in the stated large-copy regime. The intermediate window and further source-based readouts are not covered.",
        depends_on=[f"LEMMA-{candidate_id}-COHERENT-SUBSET-PHASE-QUERY", "PO-INPUT-MODEL", "PO-SUCCESS"],
        status="derived-corrected-output-mixture-bound-review-pending" if checked and report.get("claim_gate", {}).get("corrected_weight_contraction_verified") and report.get("claim_gate", {}).get("corrected_full_output_large_copy_bound_derived") else "blocked-corrected-output-mixture-evidence-missing",
        falsification_test="Verify every finite weight probability, coset-diagonal positive mixture, signed remainder and shared-h covariance. Use central squared-amplitude overlap and unitary cross cancellation; do not replace the small-k law with the mixture or drop its error. Discarding sources after correction is essential; neither all copy counts nor arbitrary complex phases are certified. No classical sampler or novelty follows.",
    ), LemmaRecord(
        id=f"LEMMA-{candidate_id}-FULL-SOURCE-WALSH-MIXTURE-BOUND", candidate_id=candidate_id,
        statement="For one uniform full-mask query with a common central unitary and irrep partition fixed before sources, retaining the complete paired source/Walsh transcript and discarding physical inputs gives T <= 2k sqrt(r/M)+|G| min(1,1/2+3/sqrt(L))^k, L=n(n-1)/2. Complex phases and all irrep labels, using r<=2^(n-1), are covered in the stated large participating-copy regime; the intermediate window is unresolved.",
        depends_on=[f"LEMMA-{candidate_id}-COHERENT-SUBSET-PHASE-QUERY", "PO-INPUT-MODEL", "PO-MEASUREMENT", "PO-SUCCESS"],
        status="derived-full-source-walsh-bound-review-pending" if checked and report.get("claim_gate", {}).get("source_category_contraction_verified") and report.get("claim_gate", {}).get("complex_source_mixtures_verified") and report.get("claim_gate", {}).get("full_source_walsh_large_copy_bound_derived") else "blocked-full-source-walsh-mixture-evidence-missing",
        falsification_test="Check full paired categorical laws, natural source marginals, complex coset-diagonal mixtures, mixed projector overlaps and the category-count-INDEPENDENT column envelope. Never discard signed remainder mass or condition on rare sources. Test source-count rules against source-only and full pair baselines. All source labels are covered but multiple queries, nonuniform masks, source-adaptive phases, final coherent readouts and every participating-copy count are not. No efficient classical sampler, formal verification or novelty is supplied.",
    ), LemmaRecord(
        id=f"LEMMA-{candidate_id}-S8-SIGN-CATEGORY-INTERMEDIATE-FAILURE", candidate_id=candidate_id,
        statement="Exact simultaneous C(h)-conjugation orbits preserve the complete sign-category/Walsh spectrum, including signed mass and remainder. At S8 and tested k=7,8,10,12,14,16, the ideal negative-character sign-category readout loses to coarse disjoint-pair counts even though the raw coset-state support-rank comparison has substantially more information.",
        depends_on=[f"LEMMA-{candidate_id}-COHERENT-SUBSET-PHASE-QUERY", "PO-INPUT-MODEL", "PO-MEASUREMENT", "PO-SUCCESS"],
        status="exact-s8-finite-window-failure-review-pending" if checked and report.get("claim_gate", {}).get("streamed_source_orbit_spectra_verified") and report.get("claim_gate", {}).get("s8_sign_category_table_loses_to_pairs_in_tested_window") else "blocked-s8-orbit-evidence-missing",
        falsification_test="Require equality of all signed spectra against full S4/S6 sums, exact orbit multiplicities and shared-h conjugation. Independently check scalar weight marginals and character predicates. Full S_n conjugacy of only one endpoint is invalid. The S8 experiment retains sign categories, not every irrep label; no growing-degree/all-copy theorem or efficient support-rank classifier follows.",
    ), LemmaRecord(
        id=f"LEMMA-{candidate_id}-SOURCE-SELECTOR-QUANTUM-MIXTURE-BOUND", candidate_id=candidate_id,
        statement="One fixed common group-algebra unitary, a uniform mask, classical source records and physical discard are obstructed at every polynomial copy budget asymptotically. For t=floor(k/4), a two-sided typical-mask projection gives E_h T(Omega_0,Omega_h)<=2k sqrt(r/M)+2^(1-k/16)+D(6/sqrt(L))^t+4sqrt(D)2^(k/2)(2/sqrt(L))^t. The decision distance T(Omega_0,E_h Omega_h), but not the average individual distance, is also capped by the raw-copy bound. A raw prefix through 5n and a monotone typical-mask suffix cover every pre-input participating count through K. At S4096 every k<=4096^2 has T<=2^-571, including central-weight masks with charged budget-wide polynomial overlap. Arbitrary masks, source-adaptive operations and physical retention are not covered.",
        depends_on=["PO-INPUT-MODEL", "PO-MEASUREMENT", "PO-SUCCESS"],
        status="derived-source-selector-quantum-bound-review-pending" if checked and report.get("claim_gate", {}).get("source_selector_quantum_mixtures_verified") and report.get("claim_gate", {}).get("source_selector_quantum_large_copy_bound_derived") and report.get("claim_gate", {}).get("noncentral_group_algebra_controls_verified") and report.get("claim_gate", {}).get("typical_mask_projection_controls_verified") and report.get("claim_gate", {}).get("uniform_mask_polynomial_budget_obstruction_derived") else "blocked-source-selector-quantum-evidence-missing",
        falsification_test="Check all shared-hidden regular-basis channels and RIGHT coefficient translations. The class average cannot be replaced by one representative or interchanged with the trace norm. Verify BOTH endpoint orientations: a one-sided typical projection does not always kill the dominant tensor factors. Keep the unnormalized gentle cost, sum all natural source masses, and validate monotone remainder envelopes across all four k residues. Charge a filter lower bound uniform over copy counts; do not extrapolate the finite S1024 certificate, which remains vacuous. A raw-prefix and typical-suffix interval, not a sweep or a fixed k, must support the polynomial-budget claim. No independent review, novelty, arbitrary-mask theorem or classical sampler is supplied.",
    ), LemmaRecord(
        id=f"LEMMA-{candidate_id}-MASK-TAIL-FIDELITY-BOUND", candidate_id=candidate_id,
        statement="For the same shared-hidden standard coset inputs, one fixed common group-algebra query, classical source records and physical discard, an arbitrary fixed pure mask with mass <=pi below weight t gives E_h T<=2k sqrt(r/M)+2sqrt(pi)+D min(1,12/sqrt(L))^(t/2)+4sqrt(D) min(1,2/sqrt(L))^(t/2). The positive comparison is a channel image of the commuting Walsh law. Cross-partial-trace norms equal environmental root fidelities; source-weighted Jensen controls the remainder WITHOUT a uniform-overlap penalty. At polynomial k, t>=3n and negligible lower-tail mass this is asymptotically negligible. It does NOT obstruct arbitrary low-occupation masks or source-selected masks.",
        depends_on=[f"LEMMA-{candidate_id}-SOURCE-SELECTOR-QUANTUM-MIXTURE-BOUND", "PO-INPUT-MODEL", "PO-MEASUREMENT", "PO-SUCCESS"],
        status="derived-mask-tail-fidelity-bound-review-pending" if checked and report.get("claim_gate", {}).get("source_weighted_environment_fidelity_verified") and report.get("claim_gate", {}).get("arbitrary_mask_positive_comparison_channel_verified") and report.get("claim_gate", {}).get("mask_tail_fidelity_bound_derived") else "blocked-mask-tail-fidelity-evidence-missing",
        falsification_test="Verify the preparation-channel identity against the full regular-basis physical output without dropping its remainder. Check root rather than squared fidelity, singular local Gram matrices, zero/rare sources, both endpoints, ordered source tuples and subnormalized retained masks. A source-selected mask exceeds the fixed-mask Jensen bound, and weight-zero mass invalidates a mean-weight-only claim. The environment/purification measurement is a mathematical comparison, not available algorithmic access. Require independent proof and novelty review; never infer a t-copy reduction from masks of weight t across many positions.",
    ), LemmaRecord(
        id=f"LEMMA-{candidate_id}-SELECTOR-MASK-SYMMETRY-REDUCTION", candidate_id=candidate_id,
        statement="For a source-record-valued Schur selector channel jointly covariant under source/selector copy permutations, every fixed pure mask is information-dominated by the positive radial pure mask with the same Hamming-weight probabilities. A complete diagonal instrument proves concavity in mask probabilities, and phases are common output unitaries. The optimization has k+1 weight probabilities but retains cross-weight coherence, all natural source masses and the unrestricted final joint measurement. No efficient objective evaluation, global numerical certificate or all-mask obstruction follows.",
        depends_on=["PO-INPUT-MODEL", "PO-MEASUREMENT", "PO-SUCCESS"],
        status="derived-selector-mask-symmetry-review-pending" if checked and report.get("claim_gate", {}).get("selector_schur_covariance_controls_verified") and report.get("claim_gate", {}).get("selector_mask_diagonal_instrument_verified") and report.get("claim_gate", {}).get("selector_mask_radial_reduction_derived") else "blocked-selector-mask-symmetry-evidence-missing",
        falsification_test="Derive the physical Schur matrices from regular-basis effects and retain every shared hidden member after tensor products. Check simultaneous permutations of source records and selector indices, phase invariance, Kraus completeness on zero-probability coordinates and the concavity direction. Ordinary density-matrix averaging or dephasing across weights is invalid. Noncovariant channels falsify radial dominance; coherent weights can beat every fixed weight. Source-adaptive masks and efficient restricted final measurements are outside scope. Require independent proof and novelty review; optimizer convergence is not a theorem.",
    ), LemmaRecord(
        id=f"LEMMA-{candidate_id}-VACUUM-COHERENCE-GAIN-BOUND", candidate_id=candidate_id,
        statement="For one fixed common group-algebra query on standard mixed coset inputs with a shared uniform fixed-point-free involution, fixed irrep-coarsened classical sources, fixed source-independent mask and physical discard, coherence with the empty subset adds negligible class-decision information at polynomial copy budgets. Source-weighted character squares give v<=1/M, compressed-input chi-square bounds vacuum row weights <=n, and coefficient decay bounds weights >=n+1. Weighted-source Jensen bounds the gain over vacuum/nonempty pinching by sqrt(w0(1-w0))*max(B_low,B_high). This does not bound total information or coherences between nonempty low-weight states.",
        depends_on=["PO-INPUT-MODEL", "PO-MEASUREMENT", "PO-SUCCESS"],
        status="derived-vacuum-coherence-bound-review-pending" if checked and report.get("claim_gate", {}).get("source_character_square_bound_verified") and report.get("claim_gate", {}).get("vacuum_coherence_physical_controls_verified") and report.get("claim_gate", {}).get("vacuum_coherence_gain_bound_derived") else "blocked-vacuum-coherence-evidence-missing",
        falsification_test="Check exact source-weighted character-square sums including coarse and merged partitions. Reconstruct the physical (S,empty) column with the correct conjugate empty phase, class-average the shared hidden member only after tensor products, and bound all four coefficient terms. Keep source-only information and natural source masses. Check positive finite vacuum gains and zero gain with nonzero source distance. Verify integer geometric-envelope rounding and coverage of every nonempty weight without sweeping the copy budget. Source adaptation and nonempty/nonempty coherence are outside scope. Independent proof and novelty review remain required.",
    ), LemmaRecord(
        id=f"LEMMA-{candidate_id}-LOW-OCCUPATION-SUPPORT-BOUND", candidate_id=candidate_id,
        statement="For a fixed pure selector mask with support size <=N and maximum subset weight t in the one-common-query physical-discard architecture, each matrix entry has active union <=min(K,2t). Compressed-input chi-square, source-weighted Hilbert-Schmidt norm and selector rank <=N give the FULL mask decision distance T<=sqrt(N*(K+2^min(K,2t)-1)/(M-K))/2 for K<M. With N<=sum_(i<=t) binomial(K,i), K=n^2 and t=n/4 this is negligible asymptotically. It includes all intra-support coherences but not arbitrary low/high cross-sector mixtures.",
        depends_on=["PO-INPUT-MODEL", "PO-MEASUREMENT", "PO-SUCCESS"],
        status="derived-low-occupation-support-bound-review-pending" if checked and report.get("claim_gate", {}).get("source_character_square_bound_verified") and report.get("claim_gate", {}).get("low_occupation_union_controls_verified") and report.get("claim_gate", {}).get("low_occupation_support_bound_derived") else "blocked-low-occupation-support-evidence-missing",
        falsification_test="Check every physical (S,T) entry with the shared hidden member averaged after products, then the entire rank-charged mask. Counterexamples must prevent dropping sqrt(N), substituting the radial parameter count K+1 for output rank, or inferring a cross-sector bound from separate sector failures. Use exact support counts and outward integer rounding; nontrivial source mass cannot be discarded. Source-adaptive masks and low/high coherences remain outside scope. A compressed entry is not a full 2t-copy channel simulation. Independent proof and novelty review remain required.",
    ), LemmaRecord(
        id=f"LEMMA-{candidate_id}-OCCUPATION-BAND-LOCALIZATION", candidate_id=candidate_id,
        statement="For one fixed common group-algebra query on standard mixed coset inputs with a shared uniform fixed-point-free involution, irrep-coarsened classical sources, fixed source-independent mask and physical discard, masks with mass <=pi strictly between weights t and s have decision distance T <= min(1, raw, max(T_low, T_high) + sqrt(N_L)*B_cross/2 + 2*sqrt(pi)). For K=n^2, t=n/4, s=3n, negligible middle mass is obstructed asymptotically. The actual low/high cross block pays sqrt(N_L) and decays on high-only positions, not overlaps. This does not bound intermediate-band masks or changed physical/query access.",
        depends_on=[f"LEMMA-{candidate_id}-LOW-OCCUPATION-SUPPORT-BOUND", f"LEMMA-{candidate_id}-MASK-TAIL-FIDELITY-BOUND", "PO-INPUT-MODEL", "PO-MEASUREMENT", "PO-SUCCESS"],
        status="derived-occupation-band-bound-review-pending" if checked and all(report.get("claim_gate", {}).get(key) for key in ("source_character_square_bound_verified", "low_occupation_support_bound_derived", "mask_tail_fidelity_bound_derived", "cross_sector_double_character_controls_verified", "cross_sector_high_side_endpoint_verified", "occupation_band_localization_derived")) else "blocked-occupation-band-evidence-missing",
        falsification_test="Verify all four weighted cross-coefficient terms against double-character expansions on ordered pairs with |T|>|S|. Check high-side endpoint identities with hidden members averaged only after products, never factor the correlated average. In the S6 counterexample with shared g'=g=v, verify exclusive decay without charging overlap. Keep all source masses, the rectangular sqrt(N_L) rank charge and gentle middle cost, and verify outward integer rounding in composite envelopes. Separate sector bounds alone are insufficient. Necessary middle mass does not justify projecting away outside-sector coherence. Masks carrying nonnegligible intermediate-band mass, source adaptation, physical retention and multiple queries are not ruled out. Independent proof and novelty review remain required.",
    ), LemmaRecord(
        id=f"LEMMA-{candidate_id}-SOURCE-RESOLVED-SELECTOR-SCHUR-BOUNDARY", candidate_id=candidate_id,
        statement="A fixed radial source-selector channel decomposes under the stabilizer of its classical source tuple into determinant-twisted symmetric powers of local 2x2 kernels. Keep spin multiplicities, source-orbit multiplicities and coherent total weights. Total matrix entries are binomial(K+4r-1,4r-1), polynomial in copies only at fixed category count, excluding group contraction and precision. For full S_n irrep sources, q<=2p and maximal-dimension bounds make polynomially many source labels asymptotically all distinct under both hypotheses; the stabilizer reduction leaves a 2^K block. This is not a circuit lower bound or an intermediate-band obstruction.",
        depends_on=[f"LEMMA-{candidate_id}-SELECTOR-MASK-SYMMETRY-REDUCTION", "PO-INPUT-MODEL", "PO-MEASUREMENT", "PO-COMPLEXITY"],
        status="derived-source-resolved-selector-schur-boundary-review-pending" if checked and all(report.get("claim_gate", {}).get(key) for key in ("selector_mask_radial_reduction_derived", "source_resolved_selector_schur_spectra_verified", "full_source_selector_collision_free_boundary_derived")) else "blocked-source-resolved-selector-schur-evidence-missing",
        falsification_test="Compare complete physical spectra for every source histogram and hidden member, including lower spins and repeated multiplicities. Verify normalized Dicke/singlet tensor actions for singular nonnormal kernels. Check source-orbit counting, total weight coherence and no source renormalization. Establish iid q from conjugacy invariance, q<=2p and exact source collision laws; do not infer a finite crossover from unspecified asymptotic constants. Source erasure is not lossless. Global covariance moves source blocks; it does not stabilize an all-distinct ordered tuple. Additional within-block structure and alternative measurements remain open; standard Schur transforms do not supply the missing classifier.",
    ), LemmaRecord(
        id=f"LEMMA-{candidate_id}-SELECTOR-COEFFICIENT-MASS-BOUND", candidate_id=candidate_id,
        statement="In the fixed common-query physical-discard architecture, source-labelled cross maps have diamond norm <=1. Shifted character-square averaging over the shared hidden involution, tensor telescoping and coefficient triangle give E_h T<=2 K (sum_g |a_g|)^2 sqrt(p(n)/M) uniformly over ALL fixed selector states and independent reference extensions. Polynomial-support queries and q<=(1/4-epsilon)n log2 n typed involutory-rotation products are obstructed asymptotically. This excludes their intermediate masks too, but generic dense queries give a vacuous bound. Coefficient mass is not a general circuit lower bound.",
        depends_on=["PO-INPUT-MODEL", "PO-QUANTUM-MECHANISM", "PO-COMPLEXITY", "PO-MEASUREMENT"],
        status="derived-selector-coefficient-mass-bound-review-pending" if checked and all(report.get("claim_gate", {}).get(key) for key in ("source_character_square_bound_verified", "selector_cross_channel_isometry_controls_verified", "selector_coefficient_tensor_hybrid_controls_verified", "selector_shifted_character_square_average_verified", "selector_coefficient_mass_all_fixed_masks_bound_derived")) else "blocked-selector-coefficient-mass-evidence-missing",
        falsification_test="Reconstruct cross kernels from physical purifications and source projectors, proving rather than inferring norm-one cross-isometry factorizations. Check arbitrary reference-ancilla operators, exact group-shift character squares, shared-h tensor products and telescoping. Keep the squared coefficient-l1 factor and actual canonical word coefficients; l2 normalization does not make l1 one. Distinguish T_class from E_h T and never apply the raw-copy cap to the latter. Coefficient certificates and typed word length do not bound arbitrary circuit complexity. Source-adaptive preparation and large coefficient mass remain outside a useful bound. Require independent proof and novelty review.",
    )]

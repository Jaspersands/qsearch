...
def _binary_carrier_instrument_lemmas(candidate_id: str) -> list[LemmaRecord]:
    ...
    return [LemmaRecord(
        ...
        status="derived-occupation-band-bound-review-pending" if checked and report.get("claim_gate", {}).get("source_character_square_bound_verified") and report.get("claim_gate", {}).get("cross_sector_double_character_controls_verified") and report.get("claim_gate", {}).get("cross_sector_high_side_endpoint_verified") and report.get("claim_gate", {}).get("occupation_band_localization_derived") else "blocked-occupation-band-evidence-missing",
        falsification_test="Verify all four weighted cross-coefficient terms against double-character expansions on ordered pairs with |T|>|S|. Check high-side endpoint identities with hidden members averaged only after products. In the S6 counterexample with shared g'=g=v, verify exclusive decay without charging overlap. Keep all source masses and verify outward integer rounding in composite envelopes. Masks carrying nonnegligible intermediate-band mass, source adaptation, physical retention and multiple queries are not ruled out. Independent proof and novelty review remain required.",
    )]

...
def write_multiscale_polar_schedule_report(
    ...
    if write_registry:
        ...
        upsert_negative_result(
            NegativeResultRecord(
                id="NEG-SELF-DUAL-WREATH-MULTISCALE-POLAR-SCHEDULE",
                source=registry_experiment_id,
                claim=(
                    "Initial negative claim for EXP-CODE-SELF-DUAL-WREATH-MULTISCALE-POLAR-SCHEDULE."
                ),
                reason_invalid=(
                    "Falsified or refined by exact theorem evaluation."
                ),
                lesson=(
                    "Lesson from exact theorem analysis for EXP-CODE-SELF-DUAL-WREATH-MULTISCALE-POLAR-SCHEDULE."
                ),
                applies_to=[
        ...
        upsert_experiment_result(
            ExperimentResultRecord(
                ...
                artifacts={
                    "self_dual_wreath_multiscale_polar_schedule": str(path)
                },
            )
        )
    return payload

...
def write_hidden_involution_query_separation_report(
    ...
    **kwargs: Any,
    ...
    if write_registry:
        ...
        upsert_negative_result(
            NegativeResultRecord(
                id="NEG--HIDDEN-INVOLUTION-QUERY-SEPARATION-BOUNDARY",
                source=registry_experiment_id,
                claim=(
                    "Initial negative claim for EXP-COSET-HIDDEN-INVOLUTION-QUERY-SEPARATION-BOUNDARY."
                ),
                reason_invalid=(
                    "Falsified or refined by exact theorem evaluation."
                ),
                lesson=(
                    "Lesson from exact theorem analysis for EXP-COSET-HIDDEN-INVOLUTION-QUERY-SEPARATION-BOUNDARY."
                ),
                applies_to=[
        ...
        upsert_experiment_result(
            ExperimentResultRecord(
                ...
                artifacts={
                    "coset_hidden_involution_query_separation_boundary": str(path)
                },
            )
        )
    ...
    if write_registry:
        ...
        upsert_experiment_result(
            ExperimentResultRecord(
                ...
                falsifiers_triggered=_res_payload.get("falsifiers_triggered", []),
                artifacts={
                    "coset_hidden_involution_query_separation_boundary": str(output_path)
                },
            )
        )
